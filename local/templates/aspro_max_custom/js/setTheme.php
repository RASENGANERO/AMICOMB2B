...
if (isset($_REQUEST['site_dir'])) {
	$SITE_DIR = htmlspecialchars((string) $_REQUEST['site_dir']);
...
if ($arSite) {
	...
	if (strlen((string) $arSite['DOMAINS'])) {
		$arSite['DOMAINS'] = preg_replace('/\s+/', ',', (string) $arSite['DOMAINS']);
...
$strListDomains = implode(
	...
	array_unique(
		array_filter(
			$arDomains,
			fn($domain) => strlen(trim((string) $domain))
		)
	)
);
...
var arAsproOptions = window[solutionName] = ({
	...
	"THEME" : ({
		...
		'INSTAGRAMM_INDEX' : '<?=($arFrontParametrs[$arFrontParametrs['INDEX_TYPE'].'_INSTAGRAMM'] ?? 'Y')?>',
