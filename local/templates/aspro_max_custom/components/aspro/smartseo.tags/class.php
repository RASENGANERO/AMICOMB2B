...
use Aspro\Smartseo\Models\SmartseoFilterConditionUrlTable;
use Aspro\Smartseo\Models\SmartseoFilterTagItemTable;
use Aspro\Smartseo\Template\Entity\FilterRuleUrl;
use Aspro\Smartseo\Models\SmartseoFilterTagTable;
...
class SmartseoTagsComponent extends CBitrixComponent
{
    ...
    protected function getUrlsByTags(array $tags, $isAnotherSection = false)
    {
        ...
        if ($tags) {
            ...
            $filterConditionUrls = SmartseoFilterConditionUrlTable::getList([
                'select' => [
                    'ID',
                    'NEW_URL',
                    'PROPERTIES',
                ...
                'cache' => [
                    'ttl' => SmartseoFilterConditionUrlTable::getCacheTtl(),
                ]
            ])->fetchAll();
            ...
            if ($tagsWithGeneratedItems) {
                ...
                $res = SmartseoFilterTagItemTable::getList([
                    'select' => [
                        'ID',
                        'NAME',
                        'SORT',
            ...
            foreach ($tags as $tag) {
                foreach ($filterConditionUrls as $url) {
                    $element = new FilterRuleUrl($url['ID']);

                    if($tag['FILTER_CONDITION_ID'] != $url['FILTER_CONDITION_ID']) {
                        continue;
                    }

                    if($tag['TYPE'] == SmartseoFilterTagTable::TYPR_FILTER_CONDITION && $tag['RELATED_PROPERTY']) {
                    ...
                    if ($tag['ITEMS_GENERATED'] != 'Y') {
                        $result[] = [
                            'ID' => $url['ID'],
                            'URL' => $url['NEW_URL'],
                            'NAME' => HtmlFilter::encode(
                                Engine::process($element, $tag['TEMPLATE'])
                            ),
    ...
    protected function sortWithoutConditionGroups(&$result, $by){
        usort($result, function($urlA, $urlB) use ($by) {
            if ($urlA[$by] == $urlB[$by]) {
                ...
                if ($by == 'NAME') {
                    return strnatcasecmp((string) $urlA[$by], (string) $urlB[$by]);
                } else {
    ...
    protected function getTagListBySelfSection()
    {
        return $this->getTagTableRows([
            '=TYPE' => SmartseoFilterTagTable::TYLE_SELF_SECTION,
    ...
    protected function getTagListByParentCondition()
    {
        ...
        return $this->getTagTableRows([
            '=TYPE' => SmartseoFilterTagTable::TYPR_FILTER_CONDITION,
    ...
    {
        return $this->getTagTableRows([
            '=TYPE' => SmartseoFilterTagTable::TYPE_SECTION,
    ...
    {
        $rows = SmartseoFilterTagTable::getList([
              'select' => [
                    'ID',
                    'TYPE',
                    'FILTER_RULE_ID' => 'FILTER_CONDITION.FILTER_RULE.ID',
              ...
              'cache' => [
                    'ttl' => SmartseoFilterTagTable::getCacheTtl(),
              ]
          ])->fetchAll();
    ...
    protected function getVariableValues()
    {
        ...
        CComponentEngine::makeComponentVariableAliases($defaultVariableAliases404, $this->arParams['VARIABLE_ALIASES']);
