...
<?$arResult['SOCIAL_VKVIDEO'] = 'https://vkvideo.ru/@ami_com';?>
<div class="social-icons">
	<span class="small_title_social-text"><?=$arParams["TITLE_BLOCK"];?></span>
	<ul>
		...
		<?if(!empty($arResult['SOCIAL_VKVIDEO'])):?>
			<li class="vkvideo">
				<a class="vkvideo-url" href="<?=$arResult['SOCIAL_VKVIDEO']?>" target="_blank" rel="nofollow" title="<?=GetMessage('TEMPL_SOCIAL_VKVIDEO')?>">
					<?=GetMessage('TEMPL_SOCIAL_VKVIDEO')?>
				</a>
			</li>
		<?endif;?>
		...
		<?if(!empty($arResult['SOCIAL_VIBER']) || !empty($arResult["SOCIAL_VIBER_CUSTOM_DESKTOP"]) || !empty($arResult["SOCIAL_VIBER_CUSTOM_MOBILE"]) ):?>
			<?			
			$hrefDesktop = strlen(trim((string) $arResult["SOCIAL_VIBER_CUSTOM_DESKTOP"])) ? $arResult["SOCIAL_VIBER_CUSTOM_DESKTOP"] : 'viber://chat?number=+'.$arResult['SOCIAL_VIBER'];
			$hrefMobile = strlen(trim((string) $arResult["SOCIAL_VIBER_CUSTOM_MOBILE"])) ? $arResult["SOCIAL_VIBER_CUSTOM_MOBILE"] : 'viber://add?number='.$arResult['SOCIAL_VIBER'];
			?>	
		...
		<?if(!empty($arResult['SOCIAL_WHATS']) || !empty($arResult["SOCIAL_WHATS_CUSTOM"]) ):?>
			<?
			if( strlen(trim((string) $arResult["SOCIAL_WHATS_CUSTOM"])) ){
				$whatsHref = $arResult["SOCIAL_WHATS_CUSTOM"];
			} else {
				if(defined('LANG_CHARSET') && strtolower((string) LANG_CHARSET) == 'windows-1251'){
					$text = iconv("windows-1251","utf-8", (string) $arResult['SOCIAL_WHATS_TEXT']);
				} else {
					$text = $arResult['SOCIAL_WHATS_TEXT'];
				}
				$bWhatsText = !empty($arResult['SOCIAL_WHATS_TEXT']);
				$whatsText = $bWhatsText ? '?text='.rawurlencode((string) $text) : '';
