										...
										<div class="img shine">
											<?if($bIcons && $arSection["UF_CATALOG_ICON"]):?>
												...
												<a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="thumb 22">
													<?if(str_contains((string) $img["src"], ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
														<?=\Aspro\Functions\CAsproMax::showSVG([
															'PATH' => $img["src"]
														]);?>
													<?else:?>
														<img class="lazy" data-src="<?=$img["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($img["src"]);?>" alt="<?=($arSection["PICTURE"]["ALT"] ?: $arSection["NAME"])?>" title="<?=($arSection["PICTURE"]["TITLE"] ?: $arSection["NAME"])?>" />
													<?endif;?>
												</a>
											<?else:?>
												<?if($arSection["PICTURE"]["SRC"]):?>
													<?$img = CFile::ResizeImageGet($arSection["PICTURE"]["ID"], [ "width" => $nResizeWH, "height" => $nResizeWH ], BX_RESIZE_IMAGE_EXACT, true );?>
													<a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="thumb"><img class="lazy" data-src="<?=$img["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($img["src"]);?>" alt="<?=($arSection["PICTURE"]["ALT"] ?: $arSection["NAME"])?>" title="<?=($arSection["PICTURE"]["TITLE"] ?: $arSection["NAME"])?>" /></a>
												<?elseif($arSection["~PICTURE"]):?>
													<?$img = CFile::ResizeImageGet($arSection["~PICTURE"], [ "width" => $nResizeWH, "height" => $nResizeWH ], BX_RESIZE_IMAGE_EXACT, true );?>
													<a href="<?=$arSection["SECTION_PAGE_URL"]?>" class="thumb"><img class="lazy" data-src="<?=$img["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($img["src"]);?>" alt="<?=($arSection["PICTURE"]["ALT"] ?: $arSection["NAME"])?>" title="<?=($arSection["PICTURE"]["TITLE"] ?: $arSection["NAME"])?>" /></a>
												<?else:?>
