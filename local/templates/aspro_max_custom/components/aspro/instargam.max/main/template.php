...
if($arResult['ITEMS']):?>
	...
	$sTemplateMobile = ($arResult['MOBILE_TEMPLATE'] ?? '');
					...
					<?if(!$bWide):?>
						<?if($arResult['DOP_TEXT'] && !$bWideFirstBlock):?>
							<div class="with-text-block-wrapper">
								<div class="row">
									<div class="col-md-3">
										<h3><?=($arResult['TITLE'] ?: \Bitrix\Main\Localization\Loc::getMessage('TITLE'));?></h3>
										<?// intro text?>
										...
										<?endif;?>
										<a href="https://www.instagram.com/<?=$arResult['USER']['username']?>/" class="btn btn-transparent-border-color btn-sm"><?=($arResult['ALL_TITLE'] ?: \Bitrix\Main\Localization\Loc::getMessage('INSTAGRAM_ALL_ITEMS'));?></a>
									</div>
									<div class="col-md-9 instagram_body">
						<?else:?>
							<div class="top_block">
								<h3><?=($arResult['TITLE'] ?: \Bitrix\Main\Localization\Loc::getMessage('TITLE'));?></h3>
								<a class="pull-right font_upper muted" href="https://www.instagram.com/<?=$arResult['USER']['username']?>/" target="_blank"><?=CMax::showIconSvg("resume", SITE_TEMPLATE_PATH."/images/svg/social/Instagram.svg", "", "inline", true, false);?><span><?=($arResult['ALL_TITLE'] ?: \Bitrix\Main\Localization\Loc::getMessage('INSTAGRAM_ALL_ITEMS'));?></span></a>
							</div>
						<?endif;?>
					<?endif;?>
					<div class="instagram clearfix">
						...
						<div class="items row flexbox<?=($bNoMargin ? ' margin0 rounded3' : '');?> <?=$sTemplateMobile;?><?=($bSlider ? ' mobile-slider' : '');?> <?=($bSlider && !$bWideFirstBlock ? ' swipeignore mobile-overflow mobile-margin-16 ' : '');?>">
							<?if($bWideFirstBlock):?>
								...
								$arItem['LINK'] = $arItem['thumbnail_url'] ?: $arItem['media_url'];
								?>
								<div class="item custom">
									<div class="item-wrapper">
										...
										<a class="wrap scrollbar" href="<?=$arItem['permalink']?>" target="_blank" rel="nofollow">
											<span class="wrapper">
												<span class="date font_upper_md muted"><span><?=FormatDate('d F', strtotime((string) $arItem['timestamp']), 'SHORT');?></span></span>
												<?if($arItem['caption']):?>
