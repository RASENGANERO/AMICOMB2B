	...
	<div class="flexslider hovers">
		<ul class="slides">
			...
			<?foreach($arResult["ITEMS"][$arParams["BANNER_TYPE_THEME"]]["ITEMS"] as $i => $arItem):?>
				...
				$arItem["NAME"] = strip_tags((string) $arItem["~NAME"]);

				if(!$i && ($arItem["PROPERTIES"]["DARK_MENU_COLOR"]["VALUE"] != "Y"))
					$bBannerLight = true;

				// video options
				$videoSource = strlen((string) $arItem['PROPERTIES']['VIDEO_SOURCE']['VALUE_XML_ID']) ? $arItem['PROPERTIES']['VIDEO_SOURCE']['VALUE_XML_ID'] : 'LINK';
				...
				if($bShowVideo = $arItem['PROPERTIES']['SHOW_VIDEO']['VALUE_XML_ID'] === 'YES' && ($videoSource == 'LINK' ? strlen((string) $videoSrc) : strlen((string) $videoFileSrc))){
					$colorSubstrates = ($arItem['PROPERTIES']['COLOR_SUBSTRATES']['VALUE_XML_ID'] ?: '');
				...
				<li class="lazy box<?=($arItem["PROPERTIES"]["TEXTCOLOR"]["VALUE_XML_ID"] ? " ".$arItem["PROPERTIES"]["TEXTCOLOR"]["VALUE_XML_ID"] : "");?><?=($arItem["PROPERTIES"]["TEXT_POSITION"]["VALUE_XML_ID"] ? " ".$arItem["PROPERTIES"]["TEXT_POSITION"]["VALUE_XML_ID"] : " left");?><?=($bShowVideo ? ' wvideo' : '');?>" data-nav_color="<?=($arItem["PROPERTIES"]["NAV_COLOR"]["VALUE_XML_ID"] ?: "");?>" data-text_color="<?=($arItem["PROPERTIES"]["DARK_MENU_COLOR"]["VALUE"] != "Y" ? "light" : "");?>" data-slide_index="<?=$i?>" <?=($bShowVideo ? ' data-video_source="'.$videoSource.'"' : '')?><?=(strlen($videoPlayer) ? ' data-video_player="'.$videoPlayer.'"' : '')?><?=(strlen((string) $videoPlayerSrc) ? ' data-video_src="'.$videoPlayerSrc.'"' : '')?><?=($bVideoAutoStart ? ' data-video_autoplay="1"' : '')?><?=($bVideoDisableSound ? ' data-video_disable_sound="1"' : '')?><?=($bVideoLoop ? ' data-video_loop="1"' : '')?><?=($bVideoCover ? ' data-video_cover="1"' : '')?> id="<?=$this->GetEditAreaId($arItem['ID']);?>" style="background-image: url('<?=\Aspro\Functions\CAsproMax::showBlankImg($background)?>');" data-src="<?=$background;?>">
					<?if($arItem["PROPERTIES"]["URL_STRING"]["VALUE"]):?>
						<a class="target" href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>></a>
					<?endif;?>
									...
									<?ob_start();?>
										<td class="text <?=$arItem["PROPERTIES"]["TEXT_POSITION"]["VALUE_XML_ID"];?>">
											<?if($arItem["PROPERTIES"]["LINK_ITEM"]["VALUE"]):?>
												<?
												$hitProp = ($arParams["HIT_PROP"] ?? "HIT");
												$saleProp = ($arParams["SALE_PROP"] ?? "SALE_TEXT");

												$arSelect = ["ID", "IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "PROPERTY_vote_count", "PROPERTY_rating", "PROPERTY_vote_sum", "CATALOG_TYPE"];
												if($hitProp)
												...
												<div class="banner_title">
														...
														<div class="stickers">
															...
															<?if($hitProp && $arProduct['PROPERTY_'.$hitProp.'_VALUE']):?>
																<?foreach((array)$arProduct['PROPERTY_'.$hitProp.'_VALUE'] as $key => $value):?>
																	...
																	<div class="sticker_<?=($strTypeHitProp == "PROPERTY_VALUE" ? CUtil::translit($value, 'ru') : strtolower((string) $arTmpEnum["XML_ID"]));?>"><?=$value?></div>
																<?endforeach;?>
															<?endif;?>
														</div>
													<?endif;?>
													...
													<?endif;?>
														...
														<?if($arProduct["DETAIL_PAGE_URL"]):?>
															<a href="<?=$arProduct["DETAIL_PAGE_URL"]?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>>
														<?endif;?>
														<?=$arProduct["NAME"];?>
														<?if($arProduct["DETAIL_PAGE_URL"]):?>
															</a>
													...
													$arQuantityData = CMax::GetQuantityArray($totalCount, ['ID' => $arProduct["ID"]], "N", (($arProduct["OFFERS"] || $arProduct['CATALOG_TYPE'] == CCatalogProduct::TYPE_SET || !$arResult['STORES_COUNT']) ? false : true) );
													$strMeasure = '';
												...
												<div class="banner_buttons with_actions <?=$arProduct["CATALOG_TYPE"];?>">
													<a href="<?=$arProduct["DETAIL_PAGE_URL"]?>" class="<?=!empty($arItem["PROPERTIES"]["BUTTON1CLASS"]["VALUE"]) ? $arItem["PROPERTIES"]["BUTTON1CLASS"]["VALUE"] : "btn btn-default btn-lg"?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>>
											...
											<?else:?>
												<?
													$bShowButton1 = (strlen((string) $arItem['PROPERTIES']['BUTTON1TEXT']['VALUE']) && strlen((string) $arItem['PROPERTIES']['BUTTON1LINK']['VALUE']));
													$bShowButton2 = (strlen((string) $arItem['PROPERTIES']['BUTTON2TEXT']['VALUE']) && strlen((string) $arItem['PROPERTIES']['BUTTON2LINK']['VALUE']));
												?>
												<?if($arItem["NAME"]):?>
													<div class="banner_title">
														<?if(strlen((string) $arItem['PROPERTIES']['TOP_TEXT']['VALUE'])):?>
														...
														<?endif;?>
															...
															<?if($arItem["PROPERTIES"]["URL_STRING"]["VALUE"]):?>
																<a href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>>
															<?endif;?>
															<?=strip_tags((string) $arItem["~NAME"], "<br><br/>");?>
															<?if($arItem["PROPERTIES"]["URL_STRING"]["VALUE"]):?>
																</a>
															<?endif;?>
												...
												<?if($bShowButton1 || $bShowButton2 || ($bShowVideo && !$bVideoAutoStart)):?>
													<div class="banner_buttons">
														<?if($bShowVideo && !$bVideoAutoStart && !$bShowButton1 && !$bShowButton2):?>
															<span class="play btn-video small <?=(strlen((string) $arItem['PROPERTIES']['BUTTON_VIDEO_CLASS']['VALUE_XML_ID']) ? $arItem['PROPERTIES']['BUTTON_VIDEO_CLASS']['VALUE_XML_ID'] : 'btn-default')?>" title="<?=$buttonVideoText?>"></span>
														<?elseif($bShowButton1 || $bShowButton2):?>
															<?if($bShowVideo && !$bVideoAutoStart):?>
																<span class="btn <?=(strlen((string) $arItem['PROPERTIES']['BUTTON_VIDEO_CLASS']['VALUE_XML_ID']) ? $arItem['PROPERTIES']['BUTTON_VIDEO_CLASS']['VALUE_XML_ID'] : 'btn-default')?> btn-video" title="<?=$buttonVideoText?>"></span>
															<?endif;?>
															<?if($bShowButton1):?>
																<a href="<?=$arItem["PROPERTIES"]["BUTTON1LINK"]["VALUE"]?>" class="<?=!empty($arItem["PROPERTIES"]["BUTTON1CLASS"]["VALUE"]) ? $arItem["PROPERTIES"]["BUTTON1CLASS"]["VALUE"] : "btn btn-default btn-lg"?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>>
																	<?=$arItem["PROPERTIES"]["BUTTON1TEXT"]["VALUE"]?>
																</a>
															<?endif;?>
															<?if($bShowButton2):?>
																<a href="<?=$arItem["PROPERTIES"]["BUTTON2LINK"]["VALUE"]?>" class="<?=!empty( $arItem["PROPERTIES"]["BUTTON2CLASS"]["VALUE"]) ? $arItem["PROPERTIES"]["BUTTON2CLASS"]["VALUE"] : "btn btn-transparent-border-color btn-lg"?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>>
									...
									<td class="img <?=($bHasVideo ? 'with_video' : '');?>">
										<?if($bHasVideo):?>
											<div class="video_block">
												<span class="play btn btn-video  <?=(strlen((string) $arItem['PROPERTIES']['BUTTON_VIDEO_CLASS']['VALUE_XML_ID']) ? $arItem['PROPERTIES']['BUTTON_VIDEO_CLASS']['VALUE_XML_ID'] : 'btn-default')?>" title="<?=$buttonVideoText?>"></span>
											</div>
										<?elseif($arItem["PREVIEW_PICTURE"]):?>
											<?if(!empty($arItem["PROPERTIES"]["URL_STRING"]["VALUE"])):?>
												<a href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?>" <?=(strlen((string) $target) ? 'target="'.$target.'"' : '')?>>
											<?endif;?>
											<img class="lazy plaxy" data-src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arItem['PREVIEW_PICTURE']['SRC'])?>" alt="<?=($arItem['PREVIEW_PICTURE']['ALT'] ?: $arItem['NAME'])?>" title="<?=($arItem['PREVIEW_PICTURE']['TITLE'] ?: $arItem['NAME'])?>" />
											<?if(!empty($arItem["PROPERTIES"]["URL_STRING"]["VALUE"])):?>
												</a>
											<?endif;?>
										<?endif;?>									
