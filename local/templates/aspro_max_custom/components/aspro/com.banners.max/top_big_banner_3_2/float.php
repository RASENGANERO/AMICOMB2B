...
<div class="item normal_block <?=$arItem["PROPERTIES"]["BANNER_SIZE"]["VALUE_XML_ID"];?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
	<div class="item_inner darken-bg-animate">
		...
		<?endif;?>
		<div class="scale_block_animate img_block lazy" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg(($arItem["DETAIL_PICTURE"]["SRC"] ?: $arItem["PREVIEW_PICTURE"]["SRC"]));?>)" data-src="<?=($arItem["DETAIL_PICTURE"]["SRC"] ?: $arItem["PREVIEW_PICTURE"]["SRC"])?>"></div>
	</div>
</div>