...
<?if($arResult["ITEMS"]){?>
	...
	<div class="start_promo <?=($arResult["OTHER_BANNERS_VIEW"]=="Y" ? "other" : "normal_view");?> row margin0">
		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			...
			$isUrl=(strlen((string) $arItem["PROPERTIES"]["URL_STRING"]["VALUE"]) ? true : false);
			?>
			<?if($arItem["DETAIL_PICTURE"]["SRC"] || $arItem["PREVIEW_PICTURE"]["SRC"]):?>
				<div class="item s_<?=$i;?> <?=($isUrl ? "hover" : "");?> <?=($arItem["PROPERTIES"]["BANNER_SIZE"]["VALUE_XML_ID"] ?: "normal");?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					<?$arItem["FORMAT_NAME"]=strip_tags((string) $arItem["~NAME"]);?>
					<?if($isUrl){?>
										...
										<span class="wrap_outer_desc">
											...
											<?}?>
												<span class="inner_text_desc">
													<?=trim(strip_tags((string) $arItem["PREVIEW_TEXT"]))?>
												</span>
											<?if($isUrl){?>
												</a>
											<?}else{?>
					...
					<?endif;?>
					<div class="scale_block_animate img_block" style="background-image:url('<?=($arItem["DETAIL_PICTURE"]["SRC"] ?: $arItem["PREVIEW_PICTURE"]["SRC"])?>')"></div>
				</div>
				<?$i++;?>
			<?endif;?>
		<?endforeach;?>
