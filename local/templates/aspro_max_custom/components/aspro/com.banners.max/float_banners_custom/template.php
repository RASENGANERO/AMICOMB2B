					...
					<div class="row flexbox justify-center swipeignore mobile-overflow mobile-margin-16 mobile-compact c_<?=count($arResult['ITEMS'])?>">
						<?foreach($arResult['ITEMS'] as $arItem)
						{
							...
							if($bType2)
							{
								...
								{
									$col = ($arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'] ?: 3);
								}
								else
								{
									$col = 3;
							...
							?>
							<div class="main-banner-cust col-md-<?=$col;?> col-sm-6 col-xs-6 col-xxs-12 item-width-261">
								<div id="<?=$this->GetEditAreaId($arItem['ID']);?>" class="item hover_zoom">
									<?if(is_array($arItem['PREVIEW_PICTURE']) ):?>
										<div class="image shine rounded3">
											<div class="img_inner">
												<?if($sUrl):?>
													<a href="<?=$sUrl;?>" title="<?=($arItem['PREVIEW_PICTURE']['TITLE'] ?: $arItem['NAME']);?>">
												<?endif;?>
									...
									<?endif;?>
									<div class="main-banner-text-cust inner-text<?=($bType2 ? '' : ' text-center');?><?=($arItem['PREVIEW_TEXT'] ? ' animated' : '');?>">
