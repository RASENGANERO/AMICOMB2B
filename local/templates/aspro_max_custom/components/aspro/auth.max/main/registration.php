...
<?if(!$USER->IsAuthorized()):?>
	...
	$arShowFields = ["LOGIN", "LAST_NAME", "NAME", "SECOND_NAME", "EMAIL", "PERSONAL_PHONE"];
	$arRequiredFields = ["NAME"];
	$useBackUrl = 'Y';
	...
	if($bPhoneAuthSupported && $bPhoneAuthShow){
		...
		if($emailKey !== false){
			$arShowFields = array_merge(array_slice($arShowFields, 0, $emailKey + 1), ["PERSONAL_PHONE"], array_slice($arShowFields, $emailKey + 1));
		}
		else{
