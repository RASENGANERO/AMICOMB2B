...
<?
use Amikomnew;
//Получение кода и названия элемента
$resultBrand = CIBlockElement::GetByID($arResult['ID'])->Fetch();
$arResult['BRAND_CODE'] = $resultBrand['CODE'];
$arResult['BRAND_NAME'] = $resultBrand['NAME'];
$resultBrandCount = Amikomnew\BrandsDetail::getElementsNewsBrandsCount($arResult['ID']);
?>
...
<?foreach($arBlockOrder as $code):?>
	...
	<?elseif($code == 'map' && $templateData['MAP']):?>
        ...
        <div class="bordered rounded-4 map-view">
            ...
            if ($arStoreMap = explode(',', (string) $templateData['MAP'])) {
                $gps_S = $arStoreMap[0];
                $gps_N = $arStoreMap[1];
            ...
            ?>
                    <?$APPLICATION->IncludeComponent(
                        ...
                        [
                            ...
                            "CONTROLS" => [0=>"ZOOM",1=>"SMALLZOOM",3=>"TYPECONTROL",4=>"SCALELINE",],
                            "INIT_MAP_TYPE" => "ROADMAP",
                            "KEY" => "AKk9BlwBAAAAcf5CSgMAHyfAq9knnHW9nsNrwnKOBpJ8-FUAAAAAAAAAAABE8lP1ifeROCbNOEGuF0oRi1P0xQ==",
                            "MAP_DATA" => serialize(["yandex_lat" => $arShop["POINTS"]["LAT"], "yandex_lon" => $arShop["POINTS"]["LON"], "yandex_scale" => 15, "PLACEMARKS" => $arShop["PLACEMARKS"]]),
                            "MAP_HEIGHT" => "500",
                            "MAP_ID" => "MAP_project",
                            "MAP_WIDTH" => "100%",
                            "OPTIONS" => [0=>"ENABLE_DBLCLICK_ZOOM",1=>"ENABLE_DRAGGING",],
                            "ZOOM_BLOCK" => ["POSITION"=>"right center",],
	...
	<?elseif($code == 'brands' && $templateData['LINK_BRANDS']):?>
		<?ob_start()?>
			<?
			$GLOBALS['arrBrandsFilter'] = ['ID' => $templateData['LINK_BRANDS']];
	...
	<?elseif($code == 'projects' && $templateData['LINK_PROJECTS']):?>
		<?ob_start()?>
			<?
			$GLOBALS['arrProjectsFilter'] = ['ID' => $templateData['LINK_PROJECTS']];
	...
	<?elseif($code == 'reviews' && $templateData['LINK_REVIEWS']):?>
		<?ob_start()?>
			<?
			$GLOBALS['arrReviewsFilter'] = ['ID' => $templateData['LINK_REVIEWS']];
	...
	<?elseif($code == 'video' && $templateData['VIDEO']):?>
		<div class="wraps video-block ordered-block with-title">
			<div class="ordered-block__title option-font-bold font_lg ">
				<?=($arParams["T_VIDEO"] ?: GetMessage("T_VIDEO"));?>
			</div>
			<div class="hidden_print">
				<div class="video_block row">
					<?if(count($templateData['VIDEO']) > 1):?>
						<?foreach($templateData['VIDEO'] as $value):?>
							<div class="col-sm-6 col-xs-12">
								<?=str_replace('src=', 'width="660" height="457" src=', str_replace(['width', 'height'], ['data-width', 'data-height'], $value));?>
							</div>
						<?endforeach;?>
					<?else:?>
	...
	<?elseif($code == 'docs' && $templateData['DOCUMENTS']):?>
		<div class="wraps docs-block ordered-block with-title">
			<div class="ordered-block__title option-font-bold font_lg ">
				<?=($arParams["T_DOCS"] ?: GetMessage("T_DOCS"));?>
			</div>
				<div class="docs_wrap files_block">
					...
					<?foreach($templateData['DOCUMENTS'] as $docID):?>
						...
						<div class="col-md-3 col-sm-6 col-xs-12">
							<?
							$fileName = substr((string) $arItem['ORIGINAL_NAME'], 0, strrpos((string) $arItem['ORIGINAL_NAME'], '.'));
							$fileTitle = (strlen((string) $arItem['DESCRIPTION']) ? $arItem['DESCRIPTION'] : $fileName);
	...
	<?elseif($code == 'landings' && $templateData['LINK_LANDINGS']):?>
		<?ob_start();?>
			<?$GLOBALS['arrLandingsFilter'] = ['ID' => $templateData['LINK_LANDINGS']];
	...
	<?elseif($code == 'partners' && $templateData['LINK_PARTNERS']):?>
		<?ob_start();?>
			<?$GLOBALS['arrPartnersFilter'] = ['ID' => $templateData['LINK_PARTNERS']];
    ...
    <?//goods?>
	<?elseif($code == 'goods'):?>
		<?if((in_array('LINK_GOODS', $arParams['PROPERTY_CODE']) || ($arParams['SHOW_LINKED_PRODUCTS'] == 'Y' && strlen((string) $arParams['LINKED_PRODUCTS_PROPERTY'])))
			...
			global $arTheme;
			$catalogIBlockID = ($arParams["IBLOCK_CATALOG_ID"] ?: $arTheme["CATALOG_IBLOCK_ID"]["VALUE"]);
			$arItemsFilter = ["IBLOCK_ID" => $catalogIBlockID, "ACTIVE" => "Y", 'SECTION_GLOBAL_ACTIVE' => 'Y'];
			...
			if($arParams['SHOW_SECTIONS_FILTER']!="N"){
				$nTopCount = ["nTopCount" => 20000];
			} else {
				$nTopCount = ["nTopCount" => 10];
			}?>
			...
			$arItems = [];
			$arRes = CIBlockElement::GetList(['SORT'=>'ASC'],$arSectionsFilter,false,false,$arSelectFields);
			while($res = $arRes->Fetch()){
				
					$arItems[] = [
						'ID' => $res['ID'],
						'IBLOCK_ID' => 29,
						'IBLOCK_SECTION_ID' => $res['IBLOCK_SECTION_ID'],
						'IBLOCK_SECTION_ID_SELECTED' => $res['IBLOCK_SECTION_ID'],
						'NAME' => CIBlockSection::GetByID($res['IBLOCK_SECTION_ID'])->Fetch()['NAME']
					];
			...
			<?endif;?>
				...
				<?if($arParams['SHOW_SECTIONS_FILTER']!="N"):?>
					<div class="sections_wrap_detail">
						...
						if($arItems)
						{
							...
							if($arSectionsID):?>
									<?$GLOBALS["arDetailSections"] = ["ID" => $arSectionsID];?>
									
									<?$APPLICATION->IncludeComponent(
										"aspro:catalog.section.list.max",
										"sections-tags-brands",
										[
											"IBLOCK_TYPE" => "aspro_max_catalog",
				...
				<div class="assoc-block js-load-block tabs_slider loader_circle content_linked_goods" data-sections-ids="" data-block="assoc" data-file="<?=$APPLICATION->GetCurPage()?>">
					...
					<?if($arParams['IS_AJAX_SECTIONS']=="Y" || $arParams['FROM_AJAX'] == 'Y'){
						...
						$arFilterSectionsIDs = json_decode((string) $_REQUEST['ajax_section_id']);
						...
						<?if(CMax::checkAjaxRequest() || CMax::checkAjaxRequest2()):?>
							...
							<?include($_SERVER['DOCUMENT_ROOT'].SITE_DIR.'include/detail.linked_products_block_brands.php');?>
						<?endif;?>
					
				</div>

				<?
					$GLOBALS['filterBrandsDetail'] = [
						'PROPERTY_BRAND_NEWS' => $arResult['ID'],
					];
					$APPLICATION->IncludeComponent(
	"bitrix:news.list", 
	"amikom-news-list-brands", 
	[
		"IBLOCK_TYPE" => "aspro_max_content",
		"IBLOCK_ID" => "23",
		"BRAND_NAME" => $arResult["NAME"],
		"BRAND_ID" => $arResult["ID"],
		"BRAND_ELEM_COUNT" => $resultBrandCount,
		"SORT_BY1" => "ACTIVE_FROM",
		"SORT_ORDER1" => "DESC",
		"SORT_BY2" => "SORT",
		"SORT_ORDER2" => "ASC",
		"USE_FILTER" => "Y",
		"FILTER_NAME" => "filterBrandsDetail",
		"NEWS_COUNT" => "8",
		"FIELD_CODE" => [
			0 => "NAME",
			1 => "PREVIEW_TEXT",
		...
		"PAGER_TITLE" => "Новости",
		"PAGER_SHOW_ALWAYS" => "Y",
		"PAGER_TEMPLATE" => "brands-news-detail",
		"PAGER_DESC_NUMBERING" => "N",
		...
		"AJAX_OPTION_ADDITIONAL" => "",
		"COMPONENT_TEMPLATE" => "amikom-news-list-brands",
		"INCLUDE_SUBSECTIONS" => "Y",
		"STRICT_SECTION_CHECK" => "N",
		"TITLE_BLOCK" => "",
		"TITLE_BLOCK_ALL" => "",
	...
	false
				...
				<?
				$GLOBALS['filterSmartBrandsDetail'] = [
					'PROPERTY_BRAND_DOCUMENT' => $arResult['ID'],
				];
				$APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", 
					"smart-docs-filter-brands", 
					[
						"CACHE_GROUPS" => "N",
						"CACHE_TIME" => "36000000",
						"CACHE_TYPE" => "A",
						...
						"USE_FILTER" => "Y",
						"FILTER_NAME" => "filterSmartBrandsDetail",
						...
						"TEMPLATE_THEME" => "blue",
						"XML_EXPORT" => "N",
						"COMPONENT_TEMPLATE" => "smart-docs-filter-brands",
						"SECTION_CODE_PATH" => "",
						"PREFILTER_NAME" => "filterSmartBrandsDetail",
						"SEF_RULE" => "",
						"BRAND_NAME"=>$arResult['NAME']
				],
				false,
	...
	<?elseif($code == 'sale' && $templateData['LINK_SALE']):?>
		<?ob_start();?>
		<?$GLOBALS['arrSaleFilter'] = ['ID' => $templateData['LINK_SALE']];
