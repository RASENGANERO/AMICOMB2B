...
<?
$bActiveDate = strlen((string) $arResult['DISPLAY_PROPERTIES']['PERIOD']['VALUE']) || ($arResult['DISPLAY_ACTIVE_FROM'] && in_array('DATE_ACTIVE_FROM', $arParams['FIELD_CODE']));
...
if($arResult['FIELDS']['DETAIL_PICTURE']){
	$atrTitle = (strlen((string) $arResult['DETAIL_PICTURE']['DESCRIPTION']) ? $arResult['DETAIL_PICTURE']['DESCRIPTION'] : (strlen((string) $arResult['DETAIL_PICTURE']['TITLE']) ? $arResult['DETAIL_PICTURE']['TITLE'] : $arResult['NAME']));
	$atrAlt = (strlen((string) $arResult['DETAIL_PICTURE']['DESCRIPTION']) ? $arResult['DETAIL_PICTURE']['DESCRIPTION'] : (strlen((string) $arResult['DETAIL_PICTURE']['ALT']) ? $arResult['DETAIL_PICTURE']['ALT'] : $arResult['NAME']));
}
...
<div class="detail_wrapper detail-news1">
	<?if($bBannerChar): // banner char?>
		...
		$sectionPath = '';
		$res = CIBlockSection::GetNavChain($arParams['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID'], ['ID', 'NAME']);
		...
		if( isset($arResult['PROPERTIES']['BANNER_CHAR_BTN_TEXT']) && $arResult['PROPERTIES']['BANNER_CHAR_BTN_TEXT']['VALUE'] ) {
			$arBannerChar['BTN'] = [
				...
				'ANCHOR' => isset($arResult['PROPERTIES']['BANNER_CHAR_BTN_ANCHOR']) ? ($arAnchors[ $arResult['PROPERTIES']['BANNER_CHAR_BTN_ANCHOR']['VALUE_XML_ID'] ] ?? false) : false,
			];
		}
		if($arBannerChar):?>
									...
									<div class="banner-char__info-buttons flexbox flexbox--row flex-wrap">
										<?if($arBannerChar['BTN']):?>
											<div class="banner-char__info-buttons-btn">
												<span class="btn <?=($arBannerChar['BTN']['CLASS'] ?: "btn-default");?>" data-scroll-block="<?=($arBannerChar['BTN']['ANCHOR'] ?: $arAnchors['GOODS']);?>"><?=$arBannerChar['BTN']['TEXT'];?></span>
											</div>
										<?endif;?>
										...
										<?if($bShowFormQuestion):?>
											<div class="banner-char__info-buttons-question">
												<span class="btn btn-transparent-border-color  animate-load" data-event="jqm" data-param-form_id="ASK" data-name="ASK" data-autoload-product_name="<?=CMax::formatJsName($arResult['NAME']);?>">
													<?=(strlen((string) $arParams['S_ASK_QUESTION']) ? $arParams['S_ASK_QUESTION'] : Loc::getMessage('S_ASK_QUESTION'))?>
												</span>
											</div>
										<?endif;?>
									</div>
...
<?//Preview text block?>
<?ob_start()?>
	<?if(strlen((string) $arResult['FIELDS']['PREVIEW_TEXT']) && (!$bShowTopBanner || $arParams["IS_LANDING"] == "Y")):?>
...
<?//Detail text block?>
<?ob_start()?>
	<?if(strlen((string) $arResult['FIELDS']['DETAIL_TEXT'])):?>
						...
						<?foreach($arResult['DISPLAY_PROPERTIES_FORMATTED'] as $code => $arProp):?>
							...
							<div class="property">
								...
								<div class="value darken">
									...
									<?elseif($code == 'SITE'):?>
										<!--noindex-->
										<a class="dark-color" href="<?=(!str_contains((string) $arProp['VALUE'], 'http') ? 'http://' : '').$arProp['VALUE'];?>" rel="nofollow" target="_blank">
											<?=!str_contains((string) $arProp['VALUE'], '?') ? $arProp['VALUE'] : explode('?', (string) $arProp['VALUE'])[0]?>
										</a>
										<!--/noindex-->
									<?elseif($code == 'EMAIL'):?>
										<a href="mailto:<?=$arProp['VALUE']?>" class="dark-color"><?=$arProp['VALUE']?></a>
									<?elseif($code == 'PHONE'):?>
										<a href="tel:<?=str_replace([' ', ',', '-', '(', ')'], '', $arProp['VALUE']);?>" class="dark-color"><?=$arProp['VALUE']?></a>
									<?else:?>
										<?=$arProp['DISPLAY_VALUE']?>
									<?endif;?>
								...
								<div class="right_project_block col-md-3 col-sm-4 col-xs-5">
									<div class="buttons-block">
										<div class="wrap">
											<?if($bShowFormOrder):?>
												<div class="form_btn">
													<span class="btn btn-default btn-lg animate-load" data-event="jqm" data-param-form_id="<?=($arParams["FORM_ID_ORDER_SERVISE"] ?: 'SERVICES');?>" data-name="order_services" data-autoload-service="<?=CMax::formatJsName($arResult['NAME']);?>" data-autoload-project="<?=CMax::formatJsName($arResult['NAME']);?>" data-autoload-product="<?=CMax::formatJsName($arResult['NAME']);?>"><span><?=(strlen((string) $arParams['S_ORDER_SERVISE']) ? $arParams['S_ORDER_SERVISE'] : Loc::getMessage('S_ORDER_SERVISE'))?></span></span>
												</div>
											<?endif;?>
											<?if($bShowFormQuestion):?>
												<div class="form_btn">
													<span class="btn btn-lg <?=($bShowFormOrder ? 'btn-transparent-border-color' : 'btn-default')?> <?=($bShowFormOrder ? 'white' : '')?>  animate-load" data-event="jqm" data-param-form_id="ASK" data-autoload-need_product="<?=CMax::formatJsName($arResult['NAME']);?>" data-name="question"><span><?=(strlen((string) $arParams['S_ASK_QUESTION']) ? $arParams['S_ASK_QUESTION'] : Loc::getMessage('S_ASK_QUESTION'))?></span></span>
												</div>
											<?endif;?>
										</div>
									</div>
	...
	<div class="period_wrapper_inner">
		<?if($bShowDopBlock):?>
			<div class="info-sticker-block inline">
				...
				<?endif;?>
				<?if($bDiscountCounter):?>
					<?\Aspro\Functions\CAsproMax::showDiscountCounter(0, $arResult, [], [], '', 'compact');?>
				<?endif;?>
			</div>
		<?endif;?>
		...
		<?if($bActiveDate):?>
			<div class="period-block darken ncolor font_xs <?=($arResult['ACTIVE_TO'] ? 'red' : '');?>">
				...
				<?endif;?>
				<?if(strlen((string) $arResult['DISPLAY_PROPERTIES']['PERIOD']['VALUE'])):?>
...
<?if($arParams["STAFF_MODE"] == "Y"):?>
	<div class="ordered-block staff_info_block bordered">
		<?$staffImage = $arResult['FIELDS']['DETAIL_PICTURE'] ?: $arResult['FIELDS']['PREVIEW_PICTURE'] ;
		$staffImageTitle = $atrTitle ?: $arResult['NAME'];
		$staffImageAlt = $atrAlt ?: $arResult['NAME'];
		?>
		<div class="staff_top_wrapper clearfix">
			...
			<div class="properties_block">
				<?if($arResult['DISPLAY_PROPERTIES_FORMATTED'] && !$bBannerChar):?>
						...
						<?if(isset($arResult["DISPLAY_PROPERTIES"]['POST']) && strlen((string) $arResult["DISPLAY_PROPERTIES"]['POST']['DISPLAY_VALUE'])):?>
							<div class="post">
						...
						<div class="bottom-soc-props social_props">
							<!-- noindex -->
								<?foreach($arResult['SOCIAL_PROPS'] as $arProp):?>
									<a href="<?=$arProp['VALUE'];?>" target="_blank" rel="nofollow" class="value <?=strtolower((string) $arProp['CODE']);?>"><?//=$arProp['VALUE'];?>
										<?=(isset($arProp['FILE']) && $arProp['FILE'] ? CMax::showIconSvg(strtolower((string) $arProp['CODE']), $arProp['FILE']) : '');?>
...
<?if($arResult['FIELDS']['DETAIL_PICTURE'] && $arResult['PROPERTIES']['PHOTOPOS']['VALUE_XML_ID'] == "WIDE_TOP"):?>
	<?$this->SetViewTarget('DETAIL_IMG');?>
		<div class="detailimage image-wide ">
			<a href="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="fancy" title="<?=$atrTitle?>">
...
if($arParams["IS_LANDING"] != "Y" && strlen((string) $arResult['FIELDS']['PREVIEW_TEXT'])) {
	$bShowDetailContent = true;
}

if(strlen((string) $arResult['FIELDS']['DETAIL_TEXT']) ||  $arResult['FIELDS']['DETAIL_PICTURE']) {
	$bShowDetailContent = true;
}
...
<?if($bShowDetailContent):?>
	<?
	$showSideImg =$arResult['DISPLAY_PROPERTIES']['SIDE_IMAGE'] && strlen((string) $arResult['DISPLAY_PROPERTIES']['SIDE_IMAGE']["FILE_VALUE"]["SRC"]);
	...
	?>
	<div id="price-block" class="ordered-block <?=($bPartnersMode ? 'partners_mode' : '')?> detail_content_wrapper side_image_<?=$sideImageType ?: 'right';?> clearfix">
	    
	    <?if($arResult['FIELDS']['DETAIL_PICTURE'] 
	    	&& (!in_array($arResult['PROPERTIES']['PHOTOPOS']['VALUE_XML_ID'],['TOP', 'LEFT', 'RIGHT', 'WIDE_TOP']) || $imageInPartnersMode) 
			...
			<div class="detail-price">
				...
				<?if(!empty($arResult['PROPERTIES']['BRAND_PRICE'])):?>
					<a id="downloadprice" data-element="<?=$arResult['CODE']?>" class="price-download-btn">Скачать прайс</a>
				<?endif;?>
			</div>
		...
		<div class="inner_wrapper_text">
			<div class="content-text <?=($bWideText ? 'wide-text' : '');?> muted777">
				...
				<?// element name?>
				<?if($arParams['DISPLAY_NAME'] != 'N' && strlen((string) $arResult['NAME'])):?>
					<h2><?=$arResult['NAME']?></h2>
				<?endif;?>
		...
		<tr>
			...
			<td class="col-md-3 col-sm-4 col-xs-5 valign btns-col">
				...
				<?else:?>
					...
					<?endif;?>
					<div class="btns">
						<?if($bShowFormOrder):?><span><span class="btn btn-default  animate-load" data-event="jqm" data-param-form_id="<?=($arParams["FORM_ID_ORDER_SERVISE"] ?: 'SERVICES');?>" data-name="order_services" data-autoload-service="<?=CMax::formatJsName($arResult['NAME']);?>" data-autoload-project="<?=CMax::formatJsName($arResult['NAME']);?>"><span><?=(strlen((string) $arParams['S_ORDER_SERVISE']) ? $arParams['S_ORDER_SERVISE'] : Loc::getMessage('S_ORDER_SERVISE'))?></span></span></span><?endif;?>
						<?if($bShowFormQuestion && $bShowFormOrder):?><span><span class="btn  btn-transparent-border-color animate-load question" data-event="jqm" data-param-form_id="ASK" data-name="ASK" data-autoload-product_name="<?=CMax::formatJsName($arResult['NAME']);?>"><?=CMax::showIconSvg("question", SITE_TEMPLATE_PATH.'/images/svg/qmark.svg', "", "colored_theme_svg", true, false);?></span></span><?endif;?>
						<?if($bShowFormQuestion && !$bShowFormOrder):?><span><span class="btn btn-default  animate-load" data-event="jqm" data-param-form_id="ASK" data-name="ASK" data-autoload-product_name="<?=CMax::formatJsName($arResult['NAME']);?>"><span><?=(strlen((string) $arParams['S_ASK_QUESTION']) ? $arParams['S_ASK_QUESTION'] : Loc::getMessage('S_ASK_QUESTION'))?></span></span></span><?endif;?>
					</div>
				<?endif;?>
			</td>
		</tr>
...
<?if($arResult['DISPLAY_PROPERTIES_FORMATTED'] && !$bBannerChar && $arParams["SHOW_TOP_PROJECT_BLOCK"] != "Y" && $arParams["STAFF_MODE"] != "Y" && $arParams["SHOW_TOP_PARTNERS_BLOCK"] != "Y"):?>	
	...
	<div class="ordered-block wraps properties-block catalog_detail with-title">			
		<div class="ordered-block__title option-font-bold font_lg"> 
		    <?=($arParams["T_CHARACTERISTICS"] ?: Loc::getMessage("T_CHARACTERISTICS"));?> 
		</div>
...
<?if($arResult['TAGS']):?>
    <?$this->SetViewTarget('tags_content');?>
        <div class="search-tags-cloud">
            ...
            <div class="tags">
                <?$arTags = explode(",", (string) $arResult['TAGS']);?>
