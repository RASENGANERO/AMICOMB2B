...
<?$bImage = strlen((string) $arResult['FIELDS']['PREVIEW_PICTURE']['SRC']);
$arImage = ($bImage ? CFile::ResizeImageGet($arResult['FIELDS']['PREVIEW_PICTURE']['ID'], ['width' => 70, 'height' => 10000], BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : []);
$imageSrc = ($bImage ? $arImage['src'] : '');
if(!$imageSrc && strlen((string) $arResult['FIELDS']['DETAIL_PICTURE']['SRC'])){
	$bImage = strlen((string) $arResult['FIELDS']['DETAIL_PICTURE']['SRC']);
	$arImage = ($bImage ? CFile::ResizeImageGet($arResult['FIELDS']['DETAIL_PICTURE']['ID'], ['width' => 90, 'height' => 10000], BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : []);
	$imageSrc = ($bImage ? $arImage['src'] : '');
		...
		<div class="item <?=($bImage ? '' : 'wti')?><?=($bLogo ? ' wlogo' : '')?>">
				...
				<div class="top_wrapper clearfix">
					...
					<div class="top-info">
						<div class="wrap muted">
							<?if(isset($arResult['DISPLAY_PROPERTIES']['POST']) && strlen((string) $arResult['DISPLAY_PROPERTIES']['POST']['VALUE'])):?>
								<span class="font_upper"><?=$arResult['DISPLAY_PROPERTIES']['POST']['VALUE']?></span>
							<?endif?>
							<?if(isset($arResult['DISPLAY_ACTIVE_FROM']) && $arResult['DISPLAY_ACTIVE_FROM'] && isset($arResult['DISPLAY_PROPERTIES']['POST']) && strlen((string) $arResult['DISPLAY_PROPERTIES']['POST']['VALUE'])):?>
			...
			<div class="bottom-block">
				<?if(in_array('RATING', $arParams['PROPERTY_CODE'])):?>
					<?$ratingValue = ($arResult['DISPLAY_PROPERTIES']['RATING']['VALUE'] ?: 0);?>
				...
				<?endif;?>
				<?if($arResult['PREVIEW_TEXT'] && strlen((string) $arResult['PREVIEW_TEXT'])):?>
