...
Asset::getInstance()->addCss('/local/templates/aspro_max_custom/components/bitrix/news.detail/shops_custom/style.css');
	...
	if(is_array($arShop)){
		if(isset($arShop['IBLOCK_ID'])){
			...
			$arShop['TITLE'] = (in_array('NAME', $arParams['FIELD_CODE']) ? strip_tags((string) $arShop['~NAME']) : '');
			$arShop['ADDRESS'] = $arShop['DISPLAY_PROPERTIES']['ADDRESS']['VALUE'];
			$arShop['ADDRESS'] = $arShop['TITLE'].((strlen($arShop['TITLE']) && strlen((string) $arShop['ADDRESS'])) ? ', ' : '').$arShop['ADDRESS'];
			...
			if(
				...
				$arShop['SCHEDULE'] = htmlspecialchars_decode((string) $arShop['DISPLAY_PROPERTIES']['SCHEDULE']['~VALUE']['TEXT']);
			}
			else{
			...
			if($imageID){
				$arShop['IMAGE'] = CFile::ResizeImageGet($imageID, ['width' => 600, 'height' => 600], BX_RESIZE_IMAGE_PROPORTIONAL);
				$arPhotos[] = [
					'ID' => $arShop["DETAIL_PICTURE"]['ID'],
					'ORIGINAL' => ($arShop["DETAIL_PICTURE"]['SRC'] ?: $arShop['IMAGE']),
					'PREVIEW' => $arShop['IMAGE'],
					'DESCRIPTION' => (strlen((string) $arShop["DETAIL_PICTURE"]['DESCRIPTION']) ? $arShop["DETAIL_PICTURE"]['DESCRIPTION'] : $arShop['ADDRESS']),
				];
			}
			if(is_array($arShop['DISPLAY_PROPERTIES']['MORE_PHOTOS']['VALUE'])) {
				foreach($arShop['DISPLAY_PROPERTIES']['MORE_PHOTOS']['VALUE'] as $i => $photoID){
					$arPhotos[] = [
						...
						'PREVIEW' => CFile::ResizeImageGet($photoID, ['width' => 600, 'height' => 600], BX_RESIZE_IMAGE_PROPORTIONAL),
			...
			if($arShop['GPS_S'] && $arShop['GPS_N']){
				...
				if($arShop['PHONE'])
				{
					...
					{
						$str_phones .= '<div class="phone"><a rel="nofollow" href="tel:'.str_replace([' ', ',', '-', '(', ')'], '', $phone).'">'.$phone.'</a></div>';
					}
				}
			...
			$arShop["TITLE"] = strip_tags(htmlspecialchars_decode((string) $arShop["TITLE"]));
			$arShop["ADDRESS"] = htmlspecialchars_decode((string) $arShop["ADDRESS"]);
			$arShop["ADDRESS"] = (strlen($arShop["TITLE"]) ? $arShop["TITLE"].', ' : '').$arShop["ADDRESS"];
			$arShop["DESCRIPTION"] = htmlspecialchars_decode((string) $arShop['DESCRIPTION']);
			$arShop['SCHEDULE'] = htmlspecialchars_decode((string) $arShop['SCHEDULE']);
			if($arShop["IMAGE_ID"]  && $arShop["IMAGE_ID"] != "null"){
				$arShop['IMAGE'] = CFile::ResizeImageGet($arShop["IMAGE_ID"], ['width' => 600, 'height' => 600], BX_RESIZE_IMAGE_PROPORTIONAL );
				$arPhotos[] = [
					'ID' => $arShop["PREVIEW_PICTURE"]['ID'],
					'ORIGINAL' => CFile::GetPath($arShop["IMAGE_ID"]),
					'PREVIEW' => $arShop['IMAGE'],
					'DESCRIPTION' => (strlen((string) $arShop["PREVIEW_PICTURE"]['DESCRIPTION']) ? $arShop["PREVIEW_PICTURE"]['DESCRIPTION'] : $arShop["ADDRESS"]),
				];
			}
				...
				<div class="left_block_store <?=($showMap ? '' : 'margin0')?>">
					...
					<div class="bottom_block">
									...
									<div class="property phone">
										...
										<?foreach($arShop["PHONE"] as $phone):?>
											<div class="value phone darken">
												<a href="tel:<?=str_replace([' ', ',', '-', '(', ')'], '', $phone);?>" rel="nofollow" class="black"><?=$phone;?></a>
											</div>
										<?endforeach;?>
									</div>
								<?endif?>
								<?if(strlen((string) $arShop["EMAIL"])):?>
						...
						<div class="feedbacks-cust feedback item">
							<div class="wrap">
								<?$GLOBALS['filterDetailContact'] = ['!ID'=>$arShop['ID']];?>
								<?
								...
								<?if($arParams["MAP_TYPE"] != "0"):?>
									<?$APPLICATION->IncludeComponent(
										...
										[
											...
											"MAP_DATA" => serialize(["google_lat" => $mapLAT, "google_lon" => $mapLON, "google_scale" => 16, "PLACEMARKS" => $arPlacemarks]),
											"MAP_WIDTH" => "100%",
											"MAP_HEIGHT" => "100%",
											"CONTROLS" => [
											],
