...
<?foreach($arBlockOrder as $code):?>
	...
	<?elseif($code == 'goods'):?>
		<?if((in_array('LINK_GOODS', $arParams['PROPERTY_CODE']) || ($arParams['SHOW_LINKED_PRODUCTS'] == 'Y' && strlen((string) $arParams['LINKED_PRODUCTS_PROPERTY'])))
			...
			$arItems = CMaxCache::CIBLockElement_GetList(['CACHE' => ["MULTI" =>"Y", "TAG" => CMaxCache::GetIBlockCacheTag($arTheme["CATALOG_IBLOCK_ID"]["VALUE"])]], $arItemsFilter, false, $nTopCount, ["ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"]);
			?>
			<?if($arItems):?>
