...
$arSection = CMaxCache::CIblockSection_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "CACHE_GROUP" => [$arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()], "MULTI" => "N"]], $arSectionFilter, false, ['ID', 'DESCRIPTION', 'PICTURE', 'DETAIL_PICTURE', 'IBLOCK_ID', 'UF_TOP_SEO']);
...
$arGroup =  ["iNumPage" => $numPage, "nPageSize" => $arParams['NEWS_COUNT']];
$arSelect = ['ID', 'NAME','PREVIEW_TEXT'];
$arItemFilter['INCLUDE_SUBSECTIONS'] ='Y';
$arElement = CMaxCache::CIblockElement_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "MULTI" => "Y"], $arParams['SORT_BY1'] => $arParams['SORT_ORDER1'], $arParams['SORT_BY2'] => $arParams['SORT_ORDER2']], $arItemFilter, false, $arGroup, $arSelect);
if($pagen != $NavNum){
	$NavNum = $pagen - 1;
}
?>
...
$arSubSections = CMaxCache::CIblockSection_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "MULTI" => "Y"]], $arSubSectionFilter, false, ["ID", "DEPTH_LEVEL"]);
...
<?else:?>
	...
	$arSeoFilter = [
		...
		"ACTIVE"=>"Y",
		"PROPERTY_FILTER_URL" => [$real_url, $current_url, $gaps_encode_current_url, $urldecodedCP_slash, $encode_current_url_slash, $replacedSpecChar],
	];
	if($arRegion) {
		$arSeoFilter[] = [
			'LOGIC' => 'OR',
			['PROPERTY_LINK_REGION_VALUE' => $arRegion['ID']],
			['PROPERTY_LINK_REGION_VALUE' => false],
		];	
	}
	$arSeoItems = CMaxCache::CIBLockElement_GetList(['CACHE' => ["MULTI" =>"Y", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["LANDING_IBLOCK_ID"])]], $arSeoFilter, false, false, ["ID", "IBLOCK_ID", "PROPERTY_FILTER_URL","PROPERTY_LINK_REGION"]);
	$arSeoItem = [];
	if($arSeoItems)
	{
		$iLandingItemID = 0;
		...
		foreach($arSeoItems as $seoKey => $arItem)
		{
			if(!$arSeoItem)
			{
				$urldecoded = urldecode((string) $arItem["PROPERTY_FILTER_URL_VALUE"]);
				...
				if($urldecoded == $url || $urldecoded == $current_url || $urldecodedCP == $current_url)
				{
					...
					$arSeoItem = CMaxCache::CIBLockElement_GetList(['SORT' => 'ASC', 'CACHE' => ["MULTI" => "N", "TAG" => CMaxCache::GetIBlockCacheTag($catalogInfoIblockId)]], ["IBLOCK_ID" => $arParams["LANDING_IBLOCK_ID"], "ID" => $iLandingItemID], false, false, ["ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "DETAIL_PICTURE", "PROPERTY_FILTER_URL", "PROPERTY_LINK_REGION", "PROPERTY_FORM_QUESTION", "PROPERTY_SECTION_SERVICES", "PROPERTY_TIZERS", "PROPERTY_SECTION", "DETAIL_TEXT", "PROPERTY_I_ELEMENT_PAGE_TITLE", "PROPERTY_I_ELEMENT_PREVIEW_PICTURE_FILE_ALT", "PROPERTY_I_ELEMENT_PREVIEW_PICTURE_FILE_TITLE", "PROPERTY_I_SKU_PAGE_TITLE", "PROPERTY_I_SKU_PREVIEW_PICTURE_FILE_ALT", "PROPERTY_I_SKU_PREVIEW_PICTURE_FILE_TITLE", "ElementValues"]);
	...
	<div class="main-section-wrapper">
		<?if($arSection['UF_TOP_SEO'] && !str_contains((string) $_SERVER['REQUEST_URI'], 'PAGEN')):?>
		...
		<?if($arSection['DESCRIPTION'] && !str_contains((string) $_SERVER['REQUEST_URI'], 'PAGEN')):?>
