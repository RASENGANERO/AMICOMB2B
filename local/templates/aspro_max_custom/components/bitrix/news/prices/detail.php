...
$arElement = CMaxCache::CIblockElement_GetList(['CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N']], $arItemFilter, false, false, ['ID', 'NAME', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'ACTIVE_FROM', 'ACTIVE_TO', 'DETAIL_PICTURE', 'DETAIL_PAGE_URL', 'LIST_PAGE_URL', 'PROPERTY_LINK_PROJECTS', 'PROPERTY_LINK_GOODS', 'PROPERTY_LINK_REVIEWS', 'PROPERTY_LINK_STAFF', 'PROPERTY_LINK_SERVICES', 'PROPERTY_FORM_QUESTION', 'PROPERTY_LINK_REGION', 'PROPERTY_LINK_GOODS_FILTER', 'PERIOD', 'SALE_NUMBER', 'DATE_CREATE']);
...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?> fixed_wrapper">
		...
		<?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE'])):?>
		    ...
		    $dbProperty = CIBlockProperty::GetList([], ["IBLOCK_ID" => $catalogID, "CODE" => "LINK_NEWS"]);
		    if($dbProperty->SelectedRowsCount() && $arElement['ID'])
		    {
			    $arTmpElement = CMaxCache::CIblockElement_GetList(['CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($catalogID), 'MULTI' => 'Y']], ['IBLOCK_ID' => $catalogID, 'PROPERTY_LINK_NEWS' => $arElement['ID']], false, false, ['ID']);
			    if($arTmpElement)
			    {
