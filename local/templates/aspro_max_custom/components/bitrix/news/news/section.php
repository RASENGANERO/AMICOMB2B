...
<?
use Amikomnew;
$sectionsNews = Amikomnew\GetSections::getAllSectionsIblock(23,'news');


//print_r($sectionsBrands);
?>
...
<?if(!$arSection && $arParams['SET_STATUS_404'] !== 'Y'):?>
	...
	$arElement = CMaxCache::CIblockElement_GetList(['CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N']], $arItemElementFilter, false, false, ['ID', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE']);
	if ($arElement) {
...
// get element end?>
	<?\Bitrix\Iblock\Component\Tools::process404(
		trim((string) $arParams["MESSAGE_404"]) ?: GetMessage("ELEMENT_NOTFOUND")
		,true
		,true
		,$arParams["SHOW_404"] === "Y"
		,$arParams["FILE_404"]
...
<?else:?>
	...
	<div class="news-section-elements">
	<?foreach($sectionsNews['SECTIONS'] as $SID => $arSection):?>
		<?if (!$bIsAjax):?>
		<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
	
			<a class="<?=$arSection['ACTIVE_PAGE_CLASS']?> title news-font pull-left section-btn btn-lg has-ripple" href="<?=$arSection['SECTION_PAGE_URL']?>"><?=$arSection['NAME']?></a>
	...
	$arSections = CMaxCache::CIBLockSection_GetList(['SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => ['ID'], 'MULTI' => 'N', 'URL_TEMPLATE' => $arResult['FOLDER'].$arResult['URL_TEMPLATES']['section']]], ['IBLOCK_ID' => $arParams['IBLOCK_ID'], /*'<=DEPTH_LEVEL' => 2,*/ 'ACTIVE' => 'Y', 'CNT_ACTIVE' => "Y"], false, ['ID', 'SECTION_PAGE_URL', 'DEPTH_LEVEL', 'IBLOCK_SECTION_ID']);
	...
	{
		$arElements = CMaxCache::CIblockElement_GetList(['CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y']], array_merge($arItemFilter, ["SECTION_ID" => $arItem["PARAMS"]["ID"], "INCLUDE_SUBSECTIONS" => "Y"]), false, false, ['ID', 'TAGS', 'IBLOCK_SECTION_ID']);
		if(!$arElements)
	...
	$arSelect = ['ID', 'NAME','PREVIEW_TEXT', 'DETAIL_PAGE_URL', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'DATE_CREATE'];
					...
					<ul class="categories left_menu dropdown">
						<?foreach($arAllSections as $arSection):
							if(isset($arSection['TEXT']) && $arSection['TEXT']):?>
								<?
								if (empty($arSection['PARAMS'])) {
									$arSection['CURRENT'] = false;
								}
								?>
