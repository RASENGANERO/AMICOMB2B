...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?> fixed_wrapper">
	    ...
	    <?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE'])):?>
		    ...
		    <?
			$dbProperty = CIBlockProperty::GetList([], ["IBLOCK_ID" => $catalogID, "CODE" => "LINK_SALE"]);
			if($dbProperty->SelectedRowsCount() && $arElement['ID'])
			{
				$arTmpElement = CMaxCache::CIblockElement_GetList(['CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($catalogID), 'MULTI' => 'Y']], ['IBLOCK_ID' => $catalogID, 'PROPERTY_LINK_SALE' => $arElement['ID']], false, false, ['ID']);
				if($arTmpElement)
				{
