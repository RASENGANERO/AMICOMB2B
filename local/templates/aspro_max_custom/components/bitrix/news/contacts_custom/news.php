...
$arItemSelect = ['ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL', 'IBLOCK_SECTION_ID', 'PROPERTY_MAP', 'PROPERTY_PHONE', 'PROPERTY_SCHEDULE', 'PROPERTY_METRO', 'PROPERTY_EMAIL', 'PROPERTY_ADDRESS'];
$arItems = CMaxCache::CIblockElement_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], $arItemFilter, false, false, $arItemSelect);
		...
		<div class="contacts maxwidth-theme" itemscope itemtype="http://schema.org/Organization">
			<div class="row">
				...
				<div class="col-lg-<?=($bHasSections ? 6 : 12);?> col-md-12 info-row">
					<div class="contact-news-custom-row row">
						<div class="contact-news-custom-phone col-md-4 col-sm-6 print-6">
							<div class="property phone">
								<div class="title font_upper muted">Справочная служба</div>
								<div class="value darken" itemprop="telephone">
									<a href="tel:+74951200686">+7 (495) 120-06-86</a>
								</div>
							</div>
						</div>
						<div class="contact-news-custom-email col-md-4 col-sm-6 print-6">
							<?CMax::showContactEmail('E-mail', true, '', 'Email.svg', 'grey');?>
						</div>
						<div class="contact-news-custom-form col-md-4 col-sm-6 ask_button text-right">
							<span>
					...
					<div class="hidden">
						<?if($arRegion):?>
							...
							<?endif;?>
						<?else:?>
							<span itemprop="address"><?$APPLICATION->IncludeFile(SITE_DIR."include/contacts-site-address.php", [], ["MODE" => "html", "NAME" => "Address"]);?></span>
						<?endif;?>
						<?global $arSite;?>
...
<div class="ajax_items">
	...
	<?if($arItems):?>
		...
		if($bPostSection)
		{
			$arItems = CMaxCache::CIblockElement_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], array_merge($arItemFilter, ['SECTION_ID' => $_POST['ID']]), false, false, $arItemSelect);
		...
		foreach($arItems as $arItem)
		{
			if($arItem['PROPERTY_MAP_VALUE']){
				$arCoords = explode(',', (string) $arItem['PROPERTY_MAP_VALUE']);
				$mapLAT += $arCoords[0];
				$mapLON += $arCoords[1];
				...
				if(strlen($arItem['PROPERTY_SCHEDULE_VALUE']['TEXT'] ?? '') || $arItem['PROPERTY_PHONE_VALUE'] || $arItem['PROPERTY_METRO_VALUE'] || $arItem['PROPERTY_EMAIL_VALUE']){
					$html .= '<div class="properties">';
						...
						if($arItem['PROPERTY_PHONE_VALUE']){
							...
							if(is_array($arItem['PROPERTY_PHONE_VALUE'])){
								foreach($arItem['PROPERTY_PHONE_VALUE'] as $value){
									$phone .= '<div class="value"><a class="dark_link" rel= "nofollow" href="tel:'.str_replace([' ', ',', '-', '(', ')'], '', $value).'">'.$value.'</a></div>';
								}
							}
							else{
								$phone = '<div class="value font_sm"><a class="dark_link" rel= "nofollow" href="tel:'.str_replace([' ', ',', '-', '(', ')'], '', $arItem['PROPERTY_PHONE_VALUE']).'">'.$arItem['PROPERTY_PHONE_VALUE'].'</a></div>';
						...
						$html .= (strlen((string) $arItem['PROPERTY_EMAIL_VALUE']) ? '<div class="property email"><div class="title-prop font_upper">'.$arProperties['EMAIL']['NAME'].'</div><div class="value font_sm"><a class="dark_link" href="mailto:'.$arItem['PROPERTY_EMAIL_VALUE'].'">'.$arItem['PROPERTY_EMAIL_VALUE'].'</a></div></div>' : '');
					$html .= '</div></div>';
				}
		...
		if($iCountShops && $bUseMap)
		{
			...
			<div class="contacts-page-map">
				<?$APPLICATION->IncludeComponent(
					...
					[
						"INIT_MAP_TYPE" => "MAP",
						"MAP_DATA" => serialize(["yandex_lat" => $mapLAT, "yandex_lon" => $mapLON, "yandex_scale" => 19, "PLACEMARKS" => $arPlacemarks]),
						"MAP_WIDTH" => "100%",
						"MAP_HEIGHT" => "550",
						"CONTROLS" => [
							0 => "ZOOM",
