				...
				<?if(0):?><div class="select_wrapper visible-xs">
					<select class=" form-control" > 
						...
						<?foreach($arYears as $value):?> 
							...
							$bSelected = ($bHasYear && $value == $year); 
							?> 
						<option data-mix_filter=".d-<?=$value?>" value="<?=$APPLICATION->GetCurPageParam('year='.$value, ['year']);?>"<?=($bSelected ? ' selected' : '')?>><?=$value?></option> 
						<?endforeach?> 
					</select>
				</div><?endif;?>
				<?
...
<?elseif($arParams['TYPE_HEAD_BLOCK']=='sections_mix' || $arParams['TYPE_HEAD_BLOCK']=='sections_links'):?>
		<?$arFilter = ['IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'ACTIVE' => 'Y', 'DEPTH_LEVEL' => 1];
		$arSelect = ['ID', 'SORT', 'IBLOCK_ID', 'NAME', 'SECTION_PAGE_URL'];
		$arParentSections = CMaxCache::CIBLockSection_GetList(['SORT' => 'ASC', 'ID' => 'ASC', 'CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y']], $arFilter, false, $arSelect);
		if($arParentSections)
		{
			$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/mixitup.min.js');
