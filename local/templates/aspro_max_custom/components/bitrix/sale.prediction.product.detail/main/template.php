...
{
	$arParams['RCM_TEMPLATE'] ??= '';
	$arResult['RCM_TEMPLATE'] ??= '';
	...
	$signedTemplate = $signer->sign($arResult['RCM_TEMPLATE'] ?: $arParams['RCM_TEMPLATE'], 'bx.sale.prediction.product.detail');
...
{
	// fix &#8381; currency
	if(preg_match_all('/\&((\d+\.\d+)|(\d+))(8381);/', (string) $arResult['PREDICTION_TEXT'], $arMatches)){
