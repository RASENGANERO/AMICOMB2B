				...
				<?if ($arParams['VISIBLE_PROP_COUNT'] > 0):?>
					...
					<div class="char-side char-toggle-visible <?=($isShowProps ? '' : 'hidden')?>">
						<?TSolution\Functions::showBlockHtml([
							...
							'TITLE_TOP' => '<div class="char-side__title font_sm darken">'.($arParams["T_CHARACTERISTICS"] ?: Loc::getMessage("T_CHARACTERISTICS")).'</div>',
							'ITEM' => $arResult,
							'PARAMS' => [
								...
								<?if($arParams["SHOW_DISCOUNT_TIME"]=="Y"){?>
									...
									<?else:?>
										<?if($arResult['JS_OFFERS'])
										{
											...
											{
												$active_to = '';
												$arDiscounts = CCatalogDiscount::GetDiscountByProduct( $arTmpOffer2['ID'], $arUserGroups, "N", [], SITE_ID );
												if($arDiscounts)
												{
