...
$arParams['ADD_PICT_PROP'] = trim((string) $arParams['ADD_PICT_PROP']);
if ('-' == $arParams['ADD_PICT_PROP'])
	$arParams['ADD_PICT_PROP'] = '';
$arParams['LABEL_PROP'] = trim((string) $arParams['LABEL_PROP']);
if ('-' == $arParams['LABEL_PROP'])
	$arParams['LABEL_PROP'] = '';
$arParams['OFFER_ADD_PICT_PROP'] = trim((string) $arParams['OFFER_ADD_PICT_PROP']);
...
$arParams['MESS_BTN_BUY'] = trim((string) $arParams['MESS_BTN_BUY']);
$arParams['MESS_BTN_ADD_TO_BASKET'] = trim((string) $arParams['MESS_BTN_ADD_TO_BASKET']);
$arParams['MESS_BTN_SUBSCRIBE'] = trim((string) $arParams['MESS_BTN_SUBSCRIBE']);
$arParams['MESS_BTN_COMPARE'] = trim((string) $arParams['MESS_BTN_COMPARE']);
$arParams['MESS_NOT_AVAILABLE'] = trim((string) $arParams['MESS_NOT_AVAILABLE']);
...
if ($arResult['MODULES']['catalog'])
{
	...
	if ( $boolSKU && $featureProps = \Bitrix\Iblock\Model\PropertyFeature::getDetailPageShowPropertyCodes( $arSKU["IBLOCK_ID"], ['CODE' => 'Y'] ) ) {
        ...
        foreach ($arResult['OFFERS'] as $offer) {
            ...
            if (!empty($imageIDs)) {
                foreach ($imageIDs as $imgID) {
					...
					$alt = trim((string) $arFileImage['DESCRIPTION']) ?: trim((string) $arResult['DETAIL_PICTURE']['ALT']) ?: trim((string) $arFileImage['ALT']) ?: trim((string) $arResult['NAME']);
					$title = trim((string) $arFileImage['DESCRIPTION']) ?: trim((string) $arResult['DETAIL_PICTURE']['TITLE']) ?: trim((string) $arFileImage['TITLE']) ?: trim((string) $arResult['NAME']);

					if ($arParams['ALT_TITLE_GET'] == 'SEO') {
						$alt =  trim((string) $arResult['DETAIL_PICTURE']['ALT']) ?: trim((string) $arFileImage['ALT']) ?: trim((string) $arResult['NAME']);
						$title = trim((string) $arResult['DETAIL_PICTURE']['TITLE']) ?: trim((string) $arFileImage['TITLE']) ?: trim((string) $arResult['NAME']);
					}
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	if ('TYPE_1' != $arParams['TYPE_SKU'] ) {
		...
		$arSortFields[strtoupper((string) $arParams["OFFERS_SORT_FIELD"])] = ($arParams["OFFERS_SORT_ORDER"] == "asc" ? SORT_ASC : SORT_DESC);
		$arSortFields[strtoupper((string) $arParams["OFFERS_SORT_FIELD2"])] = ($arParams["OFFERS_SORT_ORDER2"] == "asc" ? SORT_ASC : SORT_DESC);
	}
	...
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		...
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer){
			...
			$imgID=($arOffer['PREVIEW_PICTURE'] ?: (($arOffer['DETAIL_PICTURE'] ?: false)));
			if(!$imgID){
				$imgOffers=false;
			}
...
if(($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"] ?? false) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	...
	if($arBrand){
		if($arParams["SHOW_BRAND_PICTURE"] == "Y" && ($arBrand["PREVIEW_PICTURE"] || $arBrand["DETAIL_PICTURE"])){
			$picture = ($arBrand["PREVIEW_PICTURE"] ?: $arBrand["DETAIL_PICTURE"]);
			$arBrand["IMAGE"] = CFile::ResizeImageGet($picture, ["width" => 200, "height" => 40], BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);
...
if($arResult["SECTION"])
{
	...
	if ($arInherite['UF_TABLE_SIZES']) {
		$arResult["SIZE_PATH"] = SITE_DIR."/include/table_sizes/detail_".strtolower((string) $arInherite['UF_TABLE_SIZES']).".php";
...
if(in_array('HELP_TEXT', $arParams['PROPERTY_CODE']))
{
	...
	elseif($arParams['HELP_TEXT']):
		...
		if( strlen( trim($help_text) ) < 1){
			...
			$bIsBitrixDiv = ( str_contains($help_text, 'bx_incl_area') );
...
if($arInherite['UF_VIDEO']){
	$arResult["VIDEO"]['VIDEO_FILE'] = array_merge((array)$arResult["VIDEO"]['VIDEO_FILE'], array_map(unserialize(...), $arInherite['UF_VIDEO']));
}
