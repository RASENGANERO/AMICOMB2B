...
$allPricesRetailOpt = \Amikomnew\FunctionsProducts::getPriceOpt($arResult['ID']);
...
$strObName = 'ob'.preg_replace("/[^a-zA-Z0-9_]/", "x", (string) $strMainID);
...
$arParams["BASKET_ITEMS"]=($arParams["BASKET_ITEMS"] ?: []);
...
if( $showCustomOffer && isset($arResult['OFFERS'][$arResult['OFFERS_SELECTED']]) ){
	...
	if(strlen((string) $arParams["SKU_DETAIL_ID"]))
...
$actualItem = $arResult["OFFERS"] ? ($arResult['OFFERS'][$arResult['OFFERS_SELECTED']] ?? reset($arResult['OFFERS'])) : $arResult;
					...
					<?foreach($arResult["OFFERS"] as $key => $arSKU):?>
						...
						<div class="table-view__item item bordered box-shadow main_item_wrapper <?=($useStores ? "table-view__item--has-stores" : "");?> js-notice-block">
							...
							if(isset($arFirstSkuPicture["ID"])){
								$arFirstSkuPicture = \CFile::ResizeImageGet($arFirstSkuPicture["ID"], [ "width" => 60, "height" => 60 ], BX_RESIZE_IMAGE_PROPORTIONAL,true );
							}
							?>
...
<div class="product-info <?=(!$showCustomOffer ? "noffer" : "");?> bordered rounded3" id="<?=$arItemIDs["strMainID"];?>">
	...
	<meta itemprop="description" content="<?=(strlen(strip_tags((string) $arResult['PREVIEW_TEXT'])) ? strip_tags((string) $arResult['PREVIEW_TEXT']) : (strlen(strip_tags((string) $arResult['DETAIL_TEXT'])) ? strip_tags((string) $arResult['DETAIL_TEXT']) : $name))?>" />
					...
					if($arParams['REVIEWS_VIEW'] == 'EXTENDED'):?>
						<div class="blog-info__rating--top-info pointer">
							<div class="votes_block nstar with-text" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
								<meta itemprop="ratingValue" content="<?=($arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']['VALUE'] ?: 5)?>" />
								<meta itemprop="reviewCount" content="<?=(intval($arResult['PROPERTIES']['EXTENDED_REVIEWS_COUNT']['VALUE']) ?: 1)?>" />
	...
	<?//article block?>
	<?$isArticle=(strlen((string) $article["VALUE"]) || ($arResult['SHOW_OFFERS_PROPS'] && $showCustomOffer));?>
	<?ob_start()?>
	<?if($isArticle):?>
		<div class="product-info-headnote__article">
			<div class="article muted font_xs" itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue" <?if(!strlen((string) $article["VALUE"])){?>id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_ARTICLE_DIV'] ?>" style="display: none;"<?}?>>
	...
	<?if($arResult["BRAND_ITEM"]){?>
		<div class="product-info-headnote__brand">
			<div class="brand" itemprop="brand" itemtype="https://schema.org/Brand" itemscope>
				...
				<?
				$imageBrand = !empty($arResult['BRAND_ITEM']['DETAIL_PICTURE']) ? $arResult['BRAND_ITEM']['DETAIL_PICTURE'] : null;
				$imageBrand = CFile::GetByID($imageBrand)->Fetch()['SRC'];
				?>
				<?if(!$imageBrand):?>
					<a href="<?=$arResult["BRAND_ITEM"]["LIST_PAGE_URL"].$arResult["BRAND_ITEM"]["CODE"].'/'?>" class="brand__link dark_link"><?=$arResult["BRAND_ITEM"]["NAME"]?></a>
				<?else:?>
					<a class="brand__picture" href="<?=$arResult["BRAND_ITEM"]["LIST_PAGE_URL"].$arResult["BRAND_ITEM"]["CODE"].'/'?>">
						<img src="<?=$imageBrand?>" alt="<?=$arResult["BRAND_ITEM"]["IMAGE"]["ALT"]?>" title="<?=$arResult["BRAND_ITEM"]["IMAGE"]["TITLE"]?>" />
					</a>
				<?endif;?>
			</div>
		</div>
					...
					<div class="block-deliveries-cust">
						<div class="text-form">
							<div class="text-form-cust price_txt muted777 font_sxs muted ncolor">
								...
								<div class="text-form-info-cust text-form-info"><?=$arResult['HELP_TEXT']?></div>
							</div>
						</div>
						<?//$arParams['EXPRESSION_FOR_CALCULATE_DELIVERY']?>
						<div class="text-form">
							<a href="/delivery/" class="text-delivery-cust"><?=GetMessage('DELIVERY_TEXT_CUSTOM')?></a>
						</div>
					</div>
					...
					<div class="text-form muted ncolor">
						<div class="price_txt muted777 font_sxs ">
							...
							<div class="text-form-info">
								<span><span class="animate-load dotted" data-event="jqm" data-param-form_id="SEND_GIFT" data-name="send_gift" data-autoload-product_name="<?=CMax::formatJsName($arResult["NAME"]);?>" data-autoload-product_link="<?=$sCurrentPage;?>" data-autoload-product_id="<?=$arResult["ID"];?>"><?=($arParams["SEND_GIFT_FORM_NAME"] ?: GetMessage("SEND_GIFT_FORM"));?></span></span>
							</div>
						</div>
					</div>
				<?endif;?>
							...
							<?ob_start()?>
								...
								<?else:?>
									<div class="cost prices detail prices_block"> 
										...
										<?if ((intval($arResult['PRICES']['BASE']['VALUE']) !== 0) || (intval($allPricesRetailOpt) !== 0)):?>
											<div class="prices-all-container">
												<div class="price-bs prices-all-item">
													...
													<?elseif(isset($arResult["PRICES"])):?>
														...
														<?if (intval($arResult['PRICES']['BASE']['VALUE']) === 0):?>
															<?//print_r($arResult['PRICES']);?>
												...
												<div class="prices-all-item">
													...
													<div class="price_matrix_wrapper">
														<?if (!empty($allPricesRetailOpt) && intval($allPricesRetailOpt)>0):?>
															<div class="price font-bold font_mxs" data-currency="RUB" data-value="<?=$allPricesRetailOpt?>">
																<span class="opt-values-prices values_wrapper">
																	<span class="price_value"><?=number_format((int)$allPricesRetailOpt, 0, '', ' ')?></span>
								...
								<?ob_start()?>
									<div class="text-additional">
										...
										if (intval($arResult['PRICES']['BASE']['VALUE'])!== 0) {
											...
											$path = SITE_DIR."include/element_detail_text_request.php";
										}
										?>	
										
										<div class="price_txt muted777 font_sxs<?=((CMax::checkContentFile($path) ? ' filed' : ''));?>">
											<?$APPLICATION->IncludeFile($path, [], ["MODE" => "html",  "NAME" => GetMessage('CT_BCE_CATALOG_DOP_DESCR')]);?>
										</div>
									</div>
												...
												<?if($arAddToBasketData["ACTION"] !== "NOTHING"):?>
													<?if (intval($arResult['PRICES']['BASE']['VALUE'])!== 0):?>
														<div class="wrapp-one-click">
															<span class="wrapp-url btn type_block transition_bg one_click animate-load has-ripple" data-item="<?=$arResult['ID']?>" data-iblockid="<?=$arParams['IBLOCK_ID']?>" data-quantity="1" onclick="oneClickBuy('<?=$arResult['ID']?>', '<?=$arParams['IBLOCK_ID']?>', this)">
																<span class="wrapp-url-text"><?=GetMessage("ONE_CLICK_BUY")?></span>
															</span>
														</div>
													<?endif;?>
												<?endif;?>
							...
							<div class="brand-detail-info bordered rounded3">
								...
								<?if($imageBrand):?>
									<div class="brand-detail-info__image">
										<a href="<?=$arResult["BRAND_ITEM"]["LIST_PAGE_URL"].$arResult["BRAND_ITEM"]["CODE"].'/'?>">
											<img src="<?=$imageBrand;?>" alt="<?=$arResult['BRAND_ITEM']["NAME"];?>" title="<?=$arResult['BRAND_ITEM']["NAME"];?>" itemprop="image">
										</a>
									</div>
								<?endif;?>
								<div class="brand-detail-info__preview">
									...
									<?if($arResult['SECTION']):?>
										<div class="link font_xs">
											<a href="<?=$arResult['SECTION']['SECTION_PAGE_URL']?>" target="_blank"><?=GetMessage("ITEMS_BY_SECTION")?></a>
										</div>
									<?endif;?>
									<div class="link font_xs">
										<a href="<?=$arResult['BRAND_ITEM']['CATALOG_PAGE_URL']?>" target="_blank"><?=GetMessage("ITEMS_BY_BRAND", ["#BRAND#" => $arResult['BRAND_ITEM']["NAME"]])?></a>
									</div>
								</div>
							</div>
						</div>
	...
	<?if($arResult['OFFERS']):?>
		<span itemprop="offers" itemscope itemtype="http://schema.org/AggregateOffer" style="display:none;">
			...
			<meta itemprop="lowPrice" content="<?=($arResult['MIN_PRICE']['DISCOUNT_VALUE'] ?: $arResult['MIN_PRICE']['VALUE'] )?>" />
			<meta itemprop="highPrice" content="<?=($arResult['MAX_PRICE']['DISCOUNT_VALUE'] ?: $arResult['MAX_PRICE']['VALUE'] )?>" />
			<meta itemprop="priceCurrency" content="<?=$arResult['MIN_PRICE']['CURRENCY']?>" />
			<?foreach($arResult['OFFERS'] as $arOffer):?>
				<?$currentOffersList = [];?>
				<?foreach($arOffer['TREE'] as $propName => $skuId):?>
					<?$propId = (int)substr((string) $propName, 5);?>
				...
				<span itemprop="offers" itemscope itemtype="http://schema.org/Offer">
					...
					<meta itemprop="price" content="<?=$arOffer['MIN_PRICE']['DISCOUNT_VALUE'] ?: $arOffer['MIN_PRICE']['VALUE']?>" />
			...
			<div class="additional-gallery <?=($arResult['OFFERS'] && 'TYPE_1' === $arParams['TYPE_SKU'] && !$arResult['ADDITIONAL_GALLERY'][$arCurrentSKU['ID']] ? ' hidden' : '')?>">
				...
				<div class="big-gallery-block"<?=($bShowSmallGallery ? ' style="display:none;"' : '');?> >
					<div class="owl-carousel owl-theme owl-bg-nav short-nav" data-plugin-options='{"items": "1", "autoplay" : false, "autoplayTimeout" : "3000", "smartSpeed":1000, "dots": true, "nav": true, "loop": false, "index": true, "margin": 5}'>
						<?foreach($additionalGallery as $arPhoto):?>
							<div class="item">
...
<?//files?>
<?$instr_prop = ($arParams["DETAIL_DOCS_PROP"] ?: "INSTRUCTIONS");?>
<?if(
...
<?
$arParams["SMART_SEO_PARAMETERS"]["BRAND_NAME"] = $arResult['BRAND_ITEM']['NAME'];
$arParams["SMART_SEO_PARAMETERS"]["IBLOCK_ID"] = $arResult['IBLOCK_ID'];
$arParams["SMART_SEO_PARAMETERS"]["ID"] = $arResult['ID'];

$APPLICATION->IncludeComponent(
	"aspro:smartseo.tags",
	"custom_tags",
	$arParams["SMART_SEO_PARAMETERS"],
	false, 
	["HIDE_ICONS" => "Y"]
);


$similarProducts = \Amikomnew\FunctionsProducts::getElementsRelation($arResult['IBLOCK_SECTION_ID'],$arResult['ID']);
?>
<h2 class="related-products">Похожие товары</h2>
<?
$GLOBALS['arrProductsFilterElement'] = ['ID'=>$similarProducts];
$APPLICATION->IncludeComponent(
	"bitrix:catalog.section",
	"catalog_block_brands",
	[
		"COMPATIBLE_MODE" => "Y",
		"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
		"IBLOCK_ID" => CMax::GetFrontParametrValue('CATALOG_IBLOCK_ID'),
		"PAGE_ELEMENT_COUNT" =>($arParams["DISPLAY_ELEMENT_SLIDER"] ?? 10),
		"SORT_BY1" => "SORT",
		"SORT_ORDER1" => "ASC",
		"SORT_BY2" => "ID",
		"SORT_ORDER2" => "DESC",
		"FILTER_NAME" => "arrProductsFilterElement",
		...
		"PROPERTY_CODE" => $arParams['LINKED_PROPERTY_CODE'] ?? $arParams['PROPERTY_CODE'],
		"OFFERS_LIMIT" => count($similarProducts),
		"OFFER_ADD_PICT_PROP" => $arParams["OFFER_ADD_PICT_PROP"],
		"SECTION_URL" => "",
		"DETAIL_URL" => "",
		...
		"SHOW_BIG_BLOCK" => "N",
		"DISPLAY_TOP_PAGER" => "N",
		"DISPLAY_BOTTOM_PAGER" => ($arParams["DISPLAY_LINKED_PAGER"] ?: "N"),
		"CACHE_TYPE" => 'A',//$arParams['CACHE_TYPE'],
		"CACHE_TIME" => "3600000",
		"CACHE_GROUPS" => 'N',
		"CACHE_FILTER" => "N",
		...
		"CONVERT_CURRENCY" => $arParams["CONVERT_CURRENCY"] ?: 'N',
		"CURRENCY_ID" => $arParams["CURRENCY_ID"] ?: 'RUB',
		"TYPE_VIEW_BASKET_BTN" => "TYPE_2",
		"USE_PRODUCT_QUANTITY" => "N",
		"COMPONENT_TEMPLATE" => "catalog_block_brands",
		"HIDE_NOT_AVAILABLE" => 'N',
		'HIDE_NOT_AVAILABLE_OFFERS' => $arParams["HIDE_NOT_AVAILABLE_OFFERS"],
		"ADD_DETAIL_TO_SLIDER" => ($arParams["ADD_DETAIL_TO_SLIDER"] ?: "Y"),
		"OFFERS_FIELD_CODE" => [
			0 => "ID",
			1 => "NAME",
			2 => "DETAIL_PAGE_URL",
		...
		"ELEMENT_SORT_FIELD" => ($arParams["LINKED_ELEMENT_TAB_SORT_FIELD"] ?: "SORT"),
		"ELEMENT_SORT_ORDER" => ($arParams["LINKED_ELEMENT_TAB_SORT_ORDER"] ?: "ASC"),
		"ELEMENT_SORT_FIELD2" => ($arParams["LINKED_ELEMENT_TAB_SORT_FIELD2"] ?: "ID"),
		"ELEMENT_SORT_ORDER2" => ($arParams["LINKED_ELEMENT_TAB_SORT_ORDER2"] ?: "DESC"),
		"SEF_MODE" => "N",
