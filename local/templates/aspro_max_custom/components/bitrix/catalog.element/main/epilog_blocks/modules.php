...
<?if($templateData['PRODUCT_SET_OPTIONS']['PRODUCT_SET'] && $templateData['PRODUCT_SET_OPTIONS']['PRODUCT_SET_FILTER']):?>
	...
	if($bUseSetGroup){
		...
		$arItems = CMaxCache::CIBLockElement_GetList(['CACHE' => ["MULTI" =>"Y", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], $arItemsFilter, false, ["nTopCount" => 10000], ["ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"]);

		if($arItems)
		{
			$arSectionsID = $arProductSection = [];
		...
		//get sections title
		$arSectionsName = CMaxCache::CIBlockSection_GetList(['CACHE' => ["MULTI" =>"Y", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], ['ACTIVE' => 'Y', "ID" => $arSectionsID, "IBLOCK_ID" => $arParams["IBLOCK_ID"]], false, ["ID", "IBLOCK_ID", "NAME"], ["nTopCount" => 5000]);
