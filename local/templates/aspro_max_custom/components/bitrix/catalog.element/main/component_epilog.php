...
$charactersProduct = Amikomnew\FunctionsProducts::getCharactersProduct($arResult['IBLOCK_ID'],$arResult['ID']);


global $arTheme, $arRegion;

require_once('classes/ShowHelper.php');

$arBlockOrder = explode(",", (string) $arParams["DETAIL_BLOCKS_ORDER"]);
$arTabOrder = explode(",", (string) $arParams["DETAIL_BLOCKS_TAB_ORDER"]);

$productHelper = new ShowHelper($arResult['ID']);
$countReviews = $productHelper->getCountReviews();
$GLOBALS['filterProductsReviews'] = ['PROPERTY_PRODUCT' => [$arResult['ID']]];
$GLOBALS['filterDocsProducts'] = ['PROPERTY_PRODUCT_DOCUMENT' => [$arResult['ID']]];
...
if($arTheme['USE_DETAIL_TABS']['VALUE'] != 'Y'){
	$arBlockOrder = explode(",", (string) $arParams["DETAIL_BLOCKS_ALL_ORDER"]);
...
<?if($arResult["ID"]):?>
	...
	<?if($templateData['LINK_TIZERS'] && $arParams['IBLOCK_TIZERS_ID']):?>
		<?$GLOBALS['arrTizersFilter'] = ['ID' => $templateData['LINK_TIZERS']];
	...
	<?if($templateData['LINK_SALES'] && $arParams['IBLOCK_LINK_SALE_ID']):?>
		<?ob_start()?>
			<?$GLOBALS['arrSalesFilter'] = ['ID' => $templateData['LINK_SALES']];
	...
	<?if($templateData['LINK_SERVICES'] && !$templateData['OUT_OF_PRODUCTION']):?>
		...
		ob_start();		
			...
			if( $bServicesRegionality && isset($arRegion['ID']) ){
				$GLOBALS['arBuyServicesFilter'][] = [ "PROPERTY_LINK_REGION" => $arRegion['ID'] ];
			}
			?>									
			<?$APPLICATION->IncludeComponent(
		...
		if($htmlBuyServicesAnounce && trim($htmlBuyServicesAnounce) && !str_contains($htmlBuyServicesAnounce, 'error')){?>
	...
	if(!$bShowAllServicesInAnounce):
		...
		if($htmlBuyServices && trim($htmlBuyServices) && !str_contains($htmlBuyServices, 'error')){
			$buyServices = true;
		}?>
	<?endif;?>
	...
	$bShowCharactersTab = (($arParams["SHOW_CHARACTERS"] == "Y") ? ++$i : '');
...
?>
	<?foreach($arBlockOrder as $code):?>
		...
		<?if($code == 'complect' && $templateData['CATALOG_SETS']['SET_ITEMS'] && $arParams['SHOW_KIT_PARTS'] == "Y"):?>
			<div class="ordered-block">
				<div class="ordered-block__title option-font-bold font_lg"><?=($arParams["T_KOMPLECT"] ?: Loc::getMessage('KOMPLECT_TITLE'));?></div>
										...
										<div class="item_info">
											...
											<?if($arParams["SHOW_KIT_PARTS_PRICES"] == "Y"):?>
												<div class="cost prices clearfix">
													...
													foreach($arSetItem["PRICES"] as $arPrice){
		...
		<?elseif($code == 'tabs'):?>
			...
			<?if($bShowDetailTextTab || $bShowPropsTab || $bShowVideoTab || $bShowHowBuyTab || $bShowStoresTab || $bShowCustomTab || $bShowReviewsTab || $bShowBuyServicesTab || $bShowCharactersTab):?>
				<div class="ordered-block js-store-scroll tabs-block" data-hash="Y">
								...
								<?foreach($arTabOrder as $value):?>
									...
									<?if($value == "desc"):?>
										<?if($bShowDetailTextTab || ($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB" && $bShowPropsTab)):?>
											<li class="bordered rounded3 <?=((!($iTab++) && (!$templateData['HIDE_ADDITIONAL_GALLERY'] || $bShowDetailText || $bShowDocs)) ? 'active' : '')?> <?=$templateData['HIDE_ADDITIONAL_GALLERY'] && !$bShowDocs && !$bShowDetailText ? "hidden" : "";?>"><a href="#desc" data-toggle="tab"><?=($arParams["T_DESC"] ?: Loc::getMessage("T_DESC"));?></a></li>
										<?endif;?>
									<?endif;?>
									<?//show char block?>
									<?if($value == "char"):?>
										<?if($bShowPropsTab && $arParams["PROPERTIES_DISPLAY_LOCATION"] == "TAB"):?>
											<li class="bordered rounded3 <?=(!($iTab++) || ($templateData['HIDE_ADDITIONAL_GALLERY'] && !$bShowDetailText && !$bShowDocs) ? 'active' : '')?>"><a href="#props" data-toggle="tab"><?=($arParams["T_CHARACTERISTICS"] ?: Loc::getMessage("T_CHARACTERISTICS"));?></a></li>
										<?endif;?>
									<?endif;?>
									<?//show howbuy block?>
									<?if($value == "buy"):?>
										...
										<?endif;?>
									<?endif;?>
									<?//show characteristics block?>
									<?if($value == "characteristics"):?>
										<?if($bShowCharactersTab):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#characteristics" data-toggle="tab"><?=$arParams["TITLE_CHARACTERS"];?></a></li>
										<?endif;?>
									<?endif;?>
					...
					<div class="tab-content<?=($i <= 1 ? ' not_tabs' : '')?>">
						...
						<?foreach($arTabOrder as $value):?>
										...
										<?if($bShowDetailText):?>
											<?if($i == 1):?>
												<div class="ordered-block__title option-font-bold font_lg">
													<?=($arParams["T_DESC"] ?: Loc::getMessage("T_DESC"));?>
												</div>
											<?endif;?>
							...
							<?if($value == "characteristics"):?>
								<?if($bShowCharactersTab):?>
									<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="characteristics">
										<?if($i == 1):?>
											<div class="ordered-block__title option-font-bold font_lg">
												<?=$arParams["TITLE_CHARACTERS"];?>
											</div>
										<?endif;?>
										<div class="product-attributes">
											<h2>Характеристики товара</h2>
											<?foreach($charactersProduct as $charItem):?>
											<div class="attribute">
												<span class="attribute-name"><?=$charItem['NAME']?></span>
												<span class="attribute-value"><?=$charItem['VALUE']?></span>
											</div>
											<?endforeach;?>
										</div>
									</div>
								...
								<?if($bShowCustomTab):?>
									<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="custom_tab">
										...
										$APPLICATION->IncludeComponent(
											...
											'documents-products', 
											[
												'COMPONENT_TEMPLATE' => 'documents-products',
												'IBLOCK_TYPE' => 'aspro_max_content',
												'IBLOCK_ID' => '10',
												'NEWS_COUNT' => '40',
												'USE_SEARCH' => 'N',
												...
												'SHOW_404' => 'N',
												'MESSAGE_404' => '',
												'FILTER_NAME' => 'filterDocsProducts',
												'FILTER_FIELD_CODE' => [
													0 => '',
													1 => '',
												],
							...
							<?if($value == "reviews"):?>
								...
								<?if($bShowReviewsTab):?>
									...
									<?$APPLICATION->IncludeComponent(
										...
										"reviews-products", 
										[
											"COMPONENT_TEMPLATE" => "reviews-products",
											"IBLOCK_TYPE" => "aspro_max_content",
											"IBLOCK_ID" => "22",
											"NEWS_COUNT" => "20",
											"USE_SEARCH" => "N",
											...
											"USE_REVIEW" => "N",
											"USE_FILTER" => "Y",
											"FILTER_NAME" => "filterProductsReviews",
											"SORT_BY1" => "ACTIVE_FROM",
											"SORT_ORDER1" => "DESC",
											"SORT_BY2" => "SORT",
											"SORT_ORDER2" => "ASC",
	...
	<!--<?if($i == 1 && $arParams['REVIEWS_VIEW'] == 'STANDART'):?>
		...
		<?//characters?>
		<?elseif($code == 'characteristics' && $bShowCharactersTab):?>
			<div class="ordered-block <?=$code?>">
				<div class="ordered-block__title option-font-bold font_lg">
					<?=$arParams["TITLE_CHARACTERS"];?>
				</div>
						...
						<?ob_start()?>
							<?$APPLICATION->IncludeComponent(
								...
								[
									...
									'CACHE_TIME' => $arParams['CACHE_TIME'],
									'CACHE_GROUPS' => $arParams['CACHE_GROUPS'],
									"COMMENTS_COUNT" => ($arParams["MESSAGES_PER_PAGE"] ?? $arParams['COMMENTS_COUNT']),
									"ELEMENT_CODE" => "",
									"ELEMENT_ID" => $arResult["ID"],
									"XML_ID" => $templateData['XML_ID'].(isset($templateData['OFFERS_INFO']) && isset($templateData['OFFERS_INFO']['OFFERS']) && count($templateData['OFFERS_INFO']['OFFERS']) ? '%' : ''),
									"IBLOCK_ID" => $arParams["IBLOCK_ID"],
									...
									"SHOW_RATING" => "Y",
									"RATING_TYPE" => "like_graphic_catalog_reviews",
									'SORT_PROP' => $_COOKIE['REVIEW_SORT_PROP'] ?: 'UF_ASPRO_COM_RATING',
									'SORT_ORDER' => $_COOKIE['REVIEW_SORT_ORDER'] ?: 'SORT_DESC',
									"BLOG_URL" => $arParams["BLOG_URL"],
									"REVIEW_COMMENT_REQUIRED" => $arParams["REVIEW_COMMENT_REQUIRED"],
									"REVIEW_FILTER_BUTTONS" => $arParams["REVIEW_FILTER_BUTTONS"],
									"REAL_CUSTOMER_TEXT" => $arParams["REAL_CUSTOMER_TEXT"],
		...
		<?elseif($code == 'services' && $templateData['LINK_SERVICES']):?>
			<?ob_start();?>
			<?$GLOBALS['arrServicesFilter'] = ['ID' => $templateData['LINK_SERVICES']];
			...
			<?$html=ob_get_clean();?>
			<?if($html && trim($html) && !str_contains($html, 'error')):?>
				<div class="ordered-block <?=$code?>">
					<div class="ordered-block__title option-font-bold font_lg">
		...
		<?elseif($code == 'news' && $templateData['LINK_NEWS']):?>
			<?ob_start();?>
				<?$GLOBALS['arrNewsFilter'] = ['ID' => $templateData['LINK_NEWS']];
		...
		<?elseif($code == 'blog' && $templateData['LINK_BLOG']):?>
			<?ob_start();?>
				<?$GLOBALS['arrBlogFilter'] = ['ID' => $templateData['LINK_BLOG']];
		...
		<?elseif($code == 'staff' && $templateData['LINK_STAFF']):?>
			<?ob_start();?>
				<?$GLOBALS['arrStaffFilter'] = ['ID' => $templateData['LINK_STAFF']];
		...
		<?elseif($code == 'vacancy' && $templateData['LINK_VACANCY']):?>
			<?ob_start();?>
				<?$GLOBALS['arrVacancyFilter'] = ['ID' => $templateData['LINK_VACANCY']];
			...
			<?if($arParams['DETAIL_LINKED_GOODS_TABS'] != 'N'):?>
				...
				<div class="ordered-block <?=$code?>">
					...
					<?if($templateData['ASSOCIATED'] && $templateData['EXPANDABLES']):?>
						<?
						$bShowAssociatedTab = \Aspro\Functions\CAsproMax::checkAvailable($templateData['ASSOCIATED'], ['REGION'], $arParams);
						$bShowExpandablesTab = \Aspro\Functions\CAsproMax::checkAvailable($templateData['EXPANDABLES'], ['REGION'], $arParams);
						?>
