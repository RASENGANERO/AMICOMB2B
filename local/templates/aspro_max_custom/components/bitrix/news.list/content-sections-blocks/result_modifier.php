	...
	if($arParams['SHOW_CHILD_SECTIONS'] == 'Y')
	{
		...
		foreach($arSections as $key => &$arSection){
			...
			if( $iMaxChilds === false || $iMaxChilds > 0 ){
				...
				$arItems = CMaxCache::CIBlockElement_GetList(['SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), "CACHE_GROUP" => [$arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()]]], array_merge($arSectionsFilter, ['SECTION_ID' => $arSection['ID']], $arItemsRegionFilter), false, $arNavStartParams, ['ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL']);
