	...
	foreach($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if($arItem['DISPLAY_PROPERTIES']['METRO']['VALUE'])
		{
			if(!is_array($arItem['DISPLAY_PROPERTIES']['METRO']['VALUE']))
				$arItem['DISPLAY_PROPERTIES']['METRO']['VALUE'] = [$arItem['DISPLAY_PROPERTIES']['METRO']['VALUE']];
		...
		if(
			...
			$arResult['ITEMS'][$key]['SCHEDULE'] = htmlspecialchars_decode((string) $arItem['DISPLAY_PROPERTIES']['SCHEDULE']['~VALUE']['TEXT']);
		else
