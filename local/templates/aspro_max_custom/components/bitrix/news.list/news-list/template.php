...
<?if($arResult['ITEMS']):?>
	...
	<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $arItem):?>		    
				...
				$bImageDetail = isset($arItem['FIELDS']['DETAIL_PICTURE']) && strlen((string) $arItem['DETAIL_PICTURE']['SRC']);
				$imageSrc = ($bImage ? $arItem['PREVIEW_PICTURE']['SRC'] : false);
				$imageDetailSrc = ($bImageDetail ? $arItem['DETAIL_PICTURE']['SRC'] : false);
				// show active date period
				$bActiveDate = ( isset($arItem['DISPLAY_PROPERTIES']['PERIOD']) && strlen((string) $arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE']) ) || ($arItem['DISPLAY_ACTIVE_FROM'] && in_array('DATE_ACTIVE_FROM', $arParams['FIELD_CODE']));
				...
				$bShowSectionName = isset($arResult['SECTIONS'][$arItem['IBLOCK_SECTION_ID']]) && strlen((string) $arResult['SECTIONS'][$arItem['IBLOCK_SECTION_ID']]['NAME']) && !$bHideSectionName;
				$noProps = true;
				?>
									...
									<div class="top_title_wrap">
										...
										<?if($bActiveDate):?>
											<div class="<?=(strlen($bShowSectionName) && $arParams['LINKED_MODE'] != 'Y') ? ' period-block-bottom darken ' : ' period-block muted ';?> <?=($arParams['LINKED_MODE'] == 'Y' ? 'font_upper' : 'font_xs')?> ncolor <?=($arItem['ACTIVE_TO'] ? 'red' : '');?> ">
												...
												<?endif;?>
												<?if(array_key_exists('PERIOD', $arItem['DISPLAY_PROPERTIES']) && strlen((string) $arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE'])):?>
									...
									<div class="properties">
										<?foreach($arItem['FILTERED_PROPERTIES'] as $PCODE => $arProperty):?>
											...
											<div class="inner-wrapper">
												<div class="property <?= strtolower((string) $PCODE); ?>">													
									...
									<?else:?>
										<?if(isset($arItem["DISPLAY_PROPERTIES"]['FORM_ORDER']) && $arItem["DISPLAY_PROPERTIES"]['FORM_ORDER']['VALUE_XML_ID'] === 'YES'):?>
											<div class="order_service_in_list <?=($noProps ? 'no-props' : '1')?>"><span>
												<span class="btn btn-default animate-load" data-event="jqm" data-param-form_id="<?=($arParams["FORM_ID_ORDER_SERVISE"] ?: 'SERVICES');?>" data-name="order_services" data-autoload-service="<?=CMax::formatJsName($arItem['NAME']);?>" data-autoload-project="<?=CMax::formatJsName($arItem['NAME']);?>"><span><?=(strlen((string) $arParams['S_ORDER_SERVISE']) ? $arParams['S_ORDER_SERVISE'] : Loc::getMessage('S_ORDER_SERVISE'))?></span></span>
											</span></div>
											<?endif;?>
									<?endif;?>
								<?endif;?>								
