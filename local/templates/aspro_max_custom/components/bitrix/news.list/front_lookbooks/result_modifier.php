    ...
    if ($arSectionsID) {
        ...
        $arSectionsItem = CMaxCache::CIBLockSection_GetList(
         ...
         if ($arSectionsItem) {
             ...
             //sort root sections
             if ($arSectionsRoot) {
                Collection::sortByColumn($arSectionsRoot, ["SORT" => [SORT_NUMERIC, SORT_ASC], "ID" => [SORT_NUMERIC, SORT_ASC]]);

                foreach ($arSectionsRoot as $arSection) {
                    $arResult['SECTIONS'][$arSection['ID']] = $arSection;
                    if ($arSectionsByRoot[$arSection['ID']]) {
                        $arTmpItems = [];
                        foreach ($arSectionsByRoot[$arSection['ID']] as $id) {
...
function _GetElementSectionsArray($ID){
    $arSections = [];
    $resGroups = CIBlockElement::GetElementGroups($ID, true, ["ID"]);
...
$arParams['FILTER_NAME'] = ($arParams['FILTER_NAME'] ?: 'arGoodsFilter');
