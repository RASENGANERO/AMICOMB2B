											...
											<div class="wrap  rounded3 box-shadow clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
													...
													<div class="image<?=($bImage ? "" : " wti" );?>">
														<div class="wrap">
															<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
																<?$img = ($bImage ? CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], ['width' => 560, 'height' => 10000], BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : []);?>
																<?$img['src'] = (strlen((string) $img['src']) ? $img['src'] : SITE_TEMPLATE_PATH.'/images/svg/noimage_content.svg');?>
														...
														<div class="middle-props bottom-block">
															<div class="props">
																<?if(isset($arItem['MIDDLE_PROPS']) && $arItem['MIDDLE_PROPS']):?>
																	<?foreach($arItem['MIDDLE_PROPS'] as $arProp):?>
																		<div class="prop">
												...
												<?if($bShowMessButton):?>
													<div class="send_message_button">
														<span class="animate-load btn btn-default btn-xs" data-event="jqm" data-param-form_id="ASK_STAFF" data-autoload-staff="<?=CMax::formatJsName($arItem['NAME'])?>" data-name="ask_staff"><?=(strlen((string) $arParams['SEND_MESSAGE_BUTTON_TEXT']) ? $arParams['SEND_MESSAGE_BUTTON_TEXT'] : Loc::getMessage('SEND_MESSAGE_BUTTON_TEXT'))?></span>
													</div>
												<?endif?>
