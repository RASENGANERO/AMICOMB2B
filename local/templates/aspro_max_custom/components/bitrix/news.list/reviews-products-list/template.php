		...
		<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL'] || ($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW'])):?>
			...
			<?else:?>
				<div class="top_block">
					...
					<?endif;?>
					<?if($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW']):?>
						<span class="pull-right reviews"><span class="pull-right font_upper muted dark_link animate-load" data-event="jqm" data-param-form_id="REVIEWS_CUSTOM" data-name="send_review"><?=CMax::showIconSvg("resume", SITE_TEMPLATE_PATH."/images/svg/leaveareview.svg", "", "", true, false);?><span><?=$arParams['TITLE_ADD_REVIEW'] ;?></span></span></span>
					<?endif;?>
				</div>
			<?endif;?>
		<?endif;?>
				...
				<?foreach($arResult['ITEMS'] as $i => $arItem):?>
					...
					<div class="item-wrapper col-xs-12<?=(!$bOneItem ?  ' bg-fill-white bordered1 box-shadow1' : '');?>">
						<div class="item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<div class="top_wrapper clearfix">
								...
								<div class="top-info">
									<div class="wrap muted">
										<?if(isset($arItem['DISPLAY_PROPERTIES']['POST']) && strlen((string) $arItem['DISPLAY_PROPERTIES']['POST']['VALUE'])):?>
											<span class="font_upper"><?=$arItem['DISPLAY_PROPERTIES']['POST']['VALUE']?></span>
										<?endif?>
										<?if(isset($arItem['DISPLAY_ACTIVE_FROM']) && $arItem['DISPLAY_ACTIVE_FROM'] && isset($arItem['DISPLAY_PROPERTIES']['POST']) && strlen((string) $arItem['DISPLAY_PROPERTIES']['POST']['VALUE'])):?>
									...
									<?if(in_array('RATING', $arParams['PROPERTY_CODE'])):?>
										<?$ratingValue = floatval($arItem['DISPLAY_PROPERTIES']['RATING']['VALUE'] ?: 0);?>
							...
							<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && $arItem['FIELDS']['PREVIEW_TEXT']):?>
								<div class="body-info">
									...
									<?endif;?>
									<?if(in_array('RATING', $arParams['PROPERTY_CODE'])):?>
										<?$ratingValue = ($arItem['DISPLAY_PROPERTIES']['RATING']['VALUE'] ?: 0);?>
									...
									<?if(($arParams['PREVIEW_TRUNCATE_LEN'] > 0) && (strlen((string) $arItem['~PREVIEW_TEXT']) > $arParams['PREVIEW_TRUNCATE_LEN'])):?>
						...
						<?	$answer = (isset($arItem['FIELDS']['DETAIL_TEXT']) && strlen($arItem['FIELDS']['DETAIL_TEXT']) ? $arItem['FIELDS']['DETAIL_TEXT'] : '');
							...
							$bImageResp = strlen((string) $arStaff['PREVIEW_PICTURE']);
