...
Asset::getInstance()->addCss('/local/templates/aspro_max_custom/components/bitrix/news.list/docs-list/docsKeysFilter/docsfilterstyle.css');
Asset::getInstance()->addJs('/local/templates/aspro_max_custom/components/bitrix/news.list/docs-list/docsKeysFilter/docsfilter.js');

use Amikomnew;
$sectionsDocs = \Amikomnew\GetSections::getAllSectionsIblock(10,'docs');

?>
<?if($sectionsDocs['SECTIONS']):?>
	...
	<?endif;?>
			...
			<div class="tabs">
				<div class="arrow_scroll">
					<ul class="nav nav-tabs font_upper_md">
						<?$i = 0;?>
						<?foreach($sectionsDocs['SECTIONS'] as $arSection):?>
		...
		<?endif;?>
				...
				<div class="<?=($arParams['SHOW_TABS'] == 'Y' ? 'tab-content' : 'group-content')?>">
					...
					<div class="docs-main-format">
						<?// group elements by sections?>
						<div class="docs-section-elements">
						<?foreach($sectionsDocs['SECTIONS'] as $arSection):?>
							<?
							// edit/add/delete buttons for edit mode
							$arSectionButtons = CIBlock::GetPanelButtons($arSection['IBLOCK_ID'], 0, $arSection['ID'], ['SESSID' => false, 'CATALOG' => true]);
							...
							?>
							<?if (!$bIsAjax):?>
							<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="docs-tab-pane tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">

								<?if($arParams['SHOW_SECTION_PREVIEW_DESCRIPTION'] == 'Y'):?>

									<?if($arParams['SHOW_SECTION_NAME'] != 'N'):?>
										<?// section name?>
										<?if(strlen((string) $arSection['NAME'])):?>
											<a class="<?=$arSection['ACTIVE_PAGE_CLASS']?> title docs-font pull-left section-btn btn-lg has-ripple" href="<?=$arSection['SECTION_PAGE_URL']?>"><?=$arSection['NAME']?></a>
										<?endif;?>
									<?endif;?>
						...
						<div class="docs-main-filter-smart">
							<input class="filter-docses" placeholder="Найти документ"/>
							<div class="filter-docses-result-not filter-docses-result"></div>
						</div>
						...
						<?foreach($arResult['ITEMS'] as $arItem):?>
							...
							$bActiveDate = strlen((string) $arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE']) || ($arItem['DISPLAY_ACTIVE_FROM'] && in_array('DATE_ACTIVE_FROM', $arParams['FIELD_CODE']));
							...
							<?ob_start();?>
								<?// element name?>
								<?if(strlen((string) $arItem['FIELDS']['NAME'])):?>
								...
								<?if(!(($licensesMode || $documentsMode) && $arParams['VIEW_TYPE'] == 'table')):?>
									...
									<?if($bActiveDate):?>
										<div class="period">
											<?if(strlen((string) $arItem['DISPLAY_PROPERTIES']['PERIOD']['VALUE'])):?>
									...
									<?if(strlen((string) $arItem['FIELDS']['PREVIEW_TEXT']) || strlen((string) $arItem['FIELDS']['DETAIL_TEXT'])):?>
										<div class="previewtext <?=($arParams['VIEW_TYPE'] == 'list' ? 'font_sm line-h-165' : '')?> <?=($arParams['VIEW_TYPE'] == 'table' ? 'font_xs' : '')?> muted777 ">
											<div>
												<?if(strlen((string) $arItem['FIELDS']['PREVIEW_TEXT'])):?>
											...
											<?// element detail text?>
											<div>
												<?if(strlen((string) $arItem['FIELDS']['DETAIL_TEXT'])):?>
									...
									<?// button?>
									<?if(strlen((string) $arItem['DISPLAY_PROPERTIES']['TITLE_BUTTON']['VALUE']) && strlen((string) $arItem['DISPLAY_PROPERTIES']['LINK_BUTTON']['VALUE'])):?>
									...
									<?if($arItem['DISPLAY_PROPERTIES']):?>
										<div class="properties">
											<?foreach($arItem['DISPLAY_PROPERTIES'] as $PCODE => $arProperty):?>
												<?if(in_array($PCODE, ['PERIOD', 'TITLE_BUTTON', 'LINK_BUTTON'])) continue;?>
												<?if($documentsMode && $PCODE == 'DOCUMENT') continue;?>
												<?//$bIconBlock = ($PCODE == 'EMAIL' || $PCODE == 'PHONE' || $PCODE == 'SITE');?>
												<div class="inner-wrapper">
													<div class="property <?=($bIconBlock ? "icon-block" : "");?> <?=strtolower((string) $PCODE);?>">
														<?if(!$bIconBlock):?>
														...
														<div class="value darken">
															...
															<?if($PCODE == 'SITE'):?>
																<!--noindex-->
																<a href="<?=(!str_contains((string) $arProperty['VALUE'], 'http') ? 'http://' : '').$arProperty['VALUE'];?>" rel="nofollow" target="_blank" class="dark-color">
																	<?=!str_contains((string) $arProperty['VALUE'], '?') ? $arProperty['VALUE'] : explode('?', (string) $arProperty['VALUE'])[0]?>
																</a>
																<!--/noindex-->
															<?elseif($PCODE == 'EMAIL'):?>
								...
								<?if($bImage || $onlyImgPart):?>
									<div class="image <?=($bImage ? ' w-picture' : ' wo-picture wpi')?>">
										...
										<?elseif(isset($arItem['FIELDS']['DETAIL_PICTURE'])):?>
											<a href="<?=$imageDetailSrc ?? $imageSrc;?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" data-caption="<?=$arItem['NAME'];?>" class="img-inside fancy shine">
										<?endif;?>
							...
							<?elseif($arParams['VIEW_TYPE'] == 'table'):?>
								<?
								$documentPath = CFile::GetByID($arItem['PROPERTIES']['FILE_DOCUMENT']['VALUE'])->Fetch()['SRC'];
								$documentBrand = CIBlockElement::GetByID($arItem['PROPERTIES']['BRAND_DOCUMENT']['VALUE'])->Fetch();
								
								?>
								<div class="docs-item item-width-261 box-shadow bordered colored_theme_hover_bg-block item-wrap col-xs-12">
									<div class="docs-item-new item  <?=($bImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
										<div class="row">
											<div class="col-md-12">
												<div class="docs-item-view">
													<span class="docs-title text title"><?=$arItem['NAME']?></span>
														<div class="docs-props">
															<a class="docs-props-url" href="/brands/<?=$documentBrand['CODE']?>/"><?=$documentBrand['NAME']?></a>
															<a class="docs-props-url" target="_blank" href="<?=$documentPath?>">Загрузить</a>
														</div>
												</div>
											</div>
										</div>
