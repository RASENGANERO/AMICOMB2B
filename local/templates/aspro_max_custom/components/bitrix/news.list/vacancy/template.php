				...
				<div class="col-md-12 <?=$bCarousel ? "item-width-261" : ""?>">
					<div class="item_wrap box-shadow rounded3 <?=($arParams['BORDERED']=='Y' ? 'bordered-block ' : '')?> <?=$bCarousel ? "item" : ""?> <?=($arParams['ACCORDION']=='Y' ? 'item-accordion-wrapper ' : ' colored_theme_hover_bg-block ');?>" >
						...
						<?endif;?>
							...
							<?endif;?>
							<div class="body-info <?=strlen((string) $arResult['SECTIONS'][$arItem['IBLOCK_SECTION_ID']]['NAME'])? 'with-section': '';?> <?=($bDetailLink) ? 'has-link':'';?>">
										...
										<?if($arItem['DISPLAY_PROPERTIES']):?>
											<div class="properties muted font_upper">
												<?foreach($arItem['DISPLAY_PROPERTIES'] as $PCODE => $arProperty):?>
													<?if(in_array($PCODE, ['PERIOD', 'TITLE_BUTTON', 'LINK_BUTTON', 'TYPE_BLOCK', 'SALE_NUMBER'])) continue;?>
													<?if($PCODE != 'PAY'):?>
														<?$hasProps = true;?>
														<div class="inner-wrapper">
															<div class="property <?=($bIconBlock ? "icon-block" : "");?> <?=strtolower((string) $PCODE);?>">
																...
																<?if($PCODE == 'SITE'):?>
																	<!--noindex-->
																	<a href="<?=(!str_contains((string) $arProperty['VALUE'], 'http') ? 'http://' : '').$arProperty['VALUE'];?>" rel="nofollow" target="_blank">
																		<?=$arProperty['VALUE'];?>
																	</a>
																	<!--/noindex-->
																<?elseif($PCODE == 'EMAIL'):?>
								...
								<?if(strlen((string) $arItem['FIELDS']['PREVIEW_TEXT']) && $arParams['ACCORDION'] !='Y'):?>
