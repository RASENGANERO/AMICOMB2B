	...
	<?if(!$arParams['IS_AJAX']):?>
		<div class="content_wrapper_block <?=$templateName;?> <?=$arResult['NAV_STRING'] ? '' : 'without-border'?>">
			...
			<div class="item-views sales <?=$sTemplateMobile;?>">
				<div class="items">
					<div class="front-catalog row flexbox<?=($arParams['NO_MARGIN'] == 'Y' ? ' margin0' : '');?> <?=$sTemplateMobile;?><?=($bSlider ? ' swipeignore mobile-overflow mobile-margin-16 mobile-compact' : '');?><?=$bHasBottomPager ? ' has-bottom-nav' : ''?>">
	<?endif;?>
		<?$position = ($arParams['BG_POSITION'] ? ' set-position '.$arParams['BG_POSITION'] : '');?>
			<?foreach($arResult['ITEMS'] as $arItem):?>
				...
				$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem['IBLOCK_ID'], 'SECTION_DELETE'), ['CONFIRM' => Loc::getMessage('CT_BNL_SECTION_DELETE_CONFIRM')]);
				// use detail link?
				...
				if (!empty($arItem['PREVIEW_PICTURE'])){
					$imagePrev = CFile::GetByID($arItem['PREVIEW_PICTURE'])->Fetch()['SRC'];
				}
				else{
					$imagePrev = '/local/templates/aspro_max_custom/images/svg/noimage_product.svg';
				}
				?>
				<div class="front-catalog-item-wrapper item-wrapper col-md-6 col-sm-6 col-xs-6 col-xxs-12 <?=($bSlider ? ' item-width-261' : '');?>">
					<div class="front-catalog-item item<?=($arParams['FILLED'] == 'Y' ? ' bg-fill-grey' : ' bg-fill-white');?> bordered box-shadow rounded3 clearfix <?=($bShowDopBlock && $arParams['IMG_POSITION'] != 'left' ? 'wdate' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<div class="image pull-<?=($arParams['IMG_POSITION'] ?: 'right');?> shine">
								<a href="<?=$arItem['CODE']?>">
									<img class="img-responsive ls-is-cached lazyloaded" data-src="<?=$imagePrev?>" src="<?=$imagePrev?>" alt="<?=$arItem['NAME']?>" title="<?=$arItem['NAME']?>">
								</a>
							</div>
						<div class="front-catalog-inner-text inner-text">
							<?// date active period?>
							<?if($bActiveDate):?>
							...
							<div class="front-catalog-title title font_mlg">
								<a class="front-catalog-url" href="<?=$arItem['CODE']?>"><?=$arItem['NAME']?></a>
							</div>
						...
						<?if($bShowDopBlock):?>
							<div class="info-sticker-block <?=($arParams['IMG_POSITION'] == 'left' ? 'top' : 'bottom');?>">
								...
								<?endif;?>
								<?if($bDiscountCounter):?>
									<?\Aspro\Functions\CAsproMax::showDiscountCounter(0, $arItem, [], [], '', 'compact');?>
								<?endif;?>
							</div>
						<?endif;?>
					</div>
