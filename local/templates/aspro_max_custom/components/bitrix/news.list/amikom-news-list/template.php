					...
					<?endif;?>
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<div class="period-block<?=($bShowSection && !$bLineImg ? '' : ' muted');?> <?=($arParams['HALF_BLOCK'] == 'Y' && $indexItem ? 'font_xxs' : 'font_xs');?>">
								<?if(intval($arItem['IBLOCK_SECTION_ID']) === 853):?>
									<?if(!empty($arItem['PROPERTIES']['TIME_PROMO']['VALUE'])):?>
										<div class="period-bl-sale">
											<i class="svg svg-inline-sale" aria-hidden="true">
												<svg class="sale-svg" xmlns="http://www.w3.org/2000/svg" width="9" height="12" viewBox="0 0 9 12">
