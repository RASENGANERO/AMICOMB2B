	...
	<?if(isset($arResult["DB_SOCSERV_USER"]) && $arParams["SHOW_PROFILES"] != 'N'):?>
		...
		<div class="soc-serv-accounts">
			<table cellspacing="0" cellpadding="0">
				<?
				foreach($arResult["DB_SOCSERV_USER"] as $arUser)
				{
					...
					<tr class="soc-serv-personal">
						...
						<td class="soc-serv-name">
							...
							<div class="auth-soc-name">
								...
								<?$arTmpUser = CUser::GetList(($by="id"), ($order="asc"), ["ID" => $userID], ["FIELDS" => ["ID", "PERSONAL_PHONE", "PASSWORD", "PERSONAL_PHOTO"], "SELECT" => ["UF_".strtoupper((string) $arUser['EXTERNAL_AUTH_ID'])]])->Fetch();
								$arSocUser = CSocServAuthDB::GetList([], ['USER_ID' => $userID])->Fetch();?>
								...
								<?elseif(intval($arTmpUser["UF_".strtoupper((string) $arUser['EXTERNAL_AUTH_ID'])]) > 0):?>
									<i class="soc-serv-photo">
										<?echo CFile::ShowImage($arTmpUser["UF_".strtoupper((string) $arUser['EXTERNAL_AUTH_ID'])], 30, 30, "border=0", "", true);?>
									</i>
			...
			<div class="soc-serv-title-grey">
				...
				$APPLICATION->IncludeComponent(
					...
					$formParams = [
						...
						"SHOW_MORE" => "Y",
						"PARSER" => ["Bold", "Italic", "Underline", "Strike", "ForeColor",
