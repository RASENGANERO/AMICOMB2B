	...
	<?if($arParams["SHOW_TAGS_CLOUD"] == "Y")
	{
		...
		if(is_array($arCloudParams["arrFILTER"]))
		{
			foreach($arCloudParams["arrFILTER"] as $strFILTER)
			{
				...
				elseif(str_starts_with((string) $strFILTER, "iblock_"))
				{
			...
			<tbody><tr>
				<td style="width: 100%;">
					<?if($arParams["USE_SUGGEST"] === "Y"):
						if(strlen((string) $arResult["REQUEST"]["~QUERY"]) && is_object($arResult["NAV_RESULT"]))
						{
			...
			<div class="search-item">
				...
				<div class="search-item-meta">
					<?if (
						...
						&& strlen((string) $arItem["RATING_TYPE_ID"]) > 0
