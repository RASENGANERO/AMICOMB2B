...
{
	$bNull = null;
	__MPF_ImageResizeHandler($bNull);
}
...
$arOrder = [];
$arFilter = ["POST_ID" => $arParams["ID"], "BLOG_ID" => $arResult["Blog"]["ID"], 'PUBLISH_STATUS' => 'P'];
$arSelectedFields = ["ID", 'UF_ASPRO_COM_RATING', 'UF_ASPRO_COM_LIKE', 'UF_ASPRO_COM_DISLIKE', 'UF_ASPRO_COM_APPROVE'];
...
$arParams['SORT_ORDER'] = $_SESSION['REVIEW_SORT_ORDER'] ?: 'SORT_DESC';
...
if($arResult["PagesComment"]) {
	...
	foreach ($arResult["PagesComment"] as &$comments) {
		foreach ($comments as &$comment) {
			$inputText = $comment['~POST_TEXT'];

			$pattern = '/<virtues>(.*?)<\/virtues>/s';
			preg_match($pattern, (string) $inputText, $matches);
			$virtues = $matches[1];
			...
			$resImages = CBlogImage::GetList(["ID"=>"DESC"], ['COMMENT_ID' => $comment['ID']]);
		...
		if($arParams['SORT_PROP'] == 'DateFormated') {
			if($arParams['SORT_ORDER'] != 'SORT_DESC') {
				function sortByDate($a, $b) {
					return strtotime((string) $a['DateFormated']) - strtotime((string) $b['DateFormated']);
				}
			} else {
				function sortByDate($a, $b) {
					return strtotime((string) $b['DateFormated']) - strtotime((string) $a['DateFormated']);
				}
			}
			$result = usort($arCommentsSort, sortByDate(...));
		} else {
			$arParams['SORT_ORDER'] = $arParams['SORT_ORDER'] == 'SORT_DESC' ? SORT_DESC : SORT_ASC;
			Collection::sortByColumn($arCommentsSort, ['HAS_COMMENT' => [SORT_DESC], $arParams['SORT_PROP'] => [$arParams['SORT_ORDER']], "DATE_CREATE" => [SORT_DESC]]);
		}
