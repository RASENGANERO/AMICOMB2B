...
else
{
	...
	if(strlen((string) $arResult["COMMENT_ERROR"])>0)
	{
		?>
...
if(strlen((string) $arResult["MESSAGE"])>0)
{
	?>
...
if(strlen((string) $arResult["ERROR_MESSAGE"])>0)
{
	?>
	<div class="alert alert-danger blog-note-box blog-note-error">
...
if(strlen((string) $arResult["FATAL_MESSAGE"])>0)
{
	?>
	<div class="alert alert-danger blog-note-box blog-note-error">
		<div class="blog-error-text">
...
else
{
	if($arResult["imageUploadFrame"] == "Y")
	{
		...
		<script>
			...
			<?elseif(strlen((string) $arResult["ERROR_MESSAGE"]) > 0):?>
	...
	else
	{
		if($arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"])
		{
			...
			<div id="form_comment_" style="display:none;">
				...
				<div class="form blog-comment-fields">
					...
					if(strlen((string) $arResult["NoCommentReason"]) > 0)
					{
						?>
		...
		function ShowComment($comment, $tabCount=0, $tabSize=2.5, $canModerate=false, $User=[], $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = [])
		{
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				elseif($tabCount > 10)
					$paddingSize = 26 * 5 + 1.5 * 5 + $tabCount-10;
				...
				elseif(($tabCount+1) > 10)
					$paddingSizeNew = 26 * 5 + 1.5 * 5 + ($tabCount+1)-10;
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-info">
						...
						<div class="left_info">
							<?
							if (COption::GetOptionString("blog", "allow_alias", "Y") == "Y" && (strlen((string) $comment["urlToBlog"]) > 0 || strlen((string) $comment["urlToAuthor"]) > 0) && array_key_exists("ALIAS", $comment["BlogUser"]) && strlen((string) $comment["BlogUser"]["ALIAS"]) > 0)
								$arTmpUser = [
									"NAME" => "",
									"LAST_NAME" => "",
									"SECOND_NAME" => "",
							...
							elseif (strlen((string) $comment["urlToBlog"]) > 0 || strlen((string) $comment["urlToAuthor"]) > 0)
								$arTmpUser = [
							...
							if(strlen((string) $comment["urlToBlog"])>0)
							{
								?>
							...
							elseif(strlen((string) $comment["urlToAuthor"])>0)
							{
								?><div class="blog-author">
							...
							if(strlen((string) $comment["urlToDelete"])>0 && strlen((string) $comment["AuthorEmail"])>0)
							{
								?>
					...
					<div class="blog-comment-content">
						<?if(strlen((string) $comment["TitleFormated"])>0)
						{
							?>
						...
						<?else:?>
							<?if($comment["~POST_TEXT"]):?>
								...
								preg_match($pattern, (string) $comment["~POST_TEXT"], $matches);
						...
						if($comment["COMMENT_PROPERTIES"]["SHOW"] == "Y")
						{
							...
							?><div><?
							foreach ($comment["COMMENT_PROPERTIES"]["DATA"] as $arPostField)
							{
						...
						<div class="blog-comment-meta">
							...
							<?// hide comment button?>
							<?if (strlen((string) $comment["urlToShow"])>0):?>
							...
							<?// show comment button?>
							<?if (strlen((string) $comment["urlToHide"])>0):?>
							...
							<?if (strlen((string) $comment["urlToDelete"])>0):?>
								<?
							...
							<?if (strlen((string) $comment["urlToSpam"])>0):?>
					...
					<?
					if(strlen((string) $errorComment) <= 0 && (strlen((string) $_POST["preview"]) > 0 && $_POST["show_preview"] != "N") && (IntVal($_POST["parentId"]) > 0 || IntVal($_POST["edit_id"]) > 0)
							...
							|| (IntVal($_POST["edit_id"]) > 0 && IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y")))
					{
						...
						ShowComment($commentPreview, (IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y") ? $level : (1), 2.5, false, [], false, false, false, $arParams);
					}

					if(strlen((string) $errorComment)>0 && $bCanUserComment===true
					...
					<?
					if((strlen((string) $errorComment) > 0 || strlen((string) $_POST["preview"]) > 0)
						&& (IntVal($_POST["parentId"])==$comment["ID"] || IntVal($_POST["edit_id"]) == $comment["ID"])
		...
		function RecursiveComments($sArray, $key, $level=0, $first=false, $canModerate=false, $User = null, $use_captcha = null, $bCanUserComment = null, $errorComment = null, $arSumComments = null, $arParams = null)
		{
			if(!empty($sArray[$key]))
			{
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				<a name="0"></a>
				<?
				if(strlen((string) $arResult["COMMENT_ERROR"]) > 0 && strlen((string) $_POST["parentId"]) < 2
			...
			if($arResult["CanUserComment"])
			{
				...
				<?
				if((strlen((string) $arResult["COMMENT_ERROR"])>0 || strlen((string) $_POST["preview"]) > 0)
					&& IntVal($_POST["parentId"]) == 0 && strlen((string) $_POST["parentId"]) < 2 && IntVal($_POST["edit_id"]) <= 0)
				{
					?>
					<script>
		...
		if($arResult["is_ajax_post"] != "Y" && $arResult["NEED_NAV"] == "Y")
		{
			...
			if(!$arResult["CommentsResult"][0] && !$arResult["ajax_comment"] && !strlen((string) $arResult["COMMENT_ERROR"])):?>
