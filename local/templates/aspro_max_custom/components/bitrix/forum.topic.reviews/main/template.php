...
if (LANGUAGE_ID == 'ru'):
	$path = str_replace(["\\", "//"], "/", __DIR__."/ru/script.php");
	include($path);
endif;?>
		...
		if ($arParams["SHOW_RATING"] == "Y")
		{
			...
			<div class="review-rating rating_vote_graphic">
				...
				$arRatingParams = [
						...
						"PATH_TO_USER_PROFILE" => strlen((string) $arParams["PATH_TO_USER"]) > 0? $arParams["PATH_TO_USER"]: $arParams["~URL_TEMPLATES_PROFILE_VIEW"]
					];
				if (!isset($res['RATING']))
					$res['RATING'] = [
							"USER_VOTE" => 0,
				...
				$APPLICATION->IncludeComponent( "bitrix:rating.vote", $arParams["RATING_TYPE"], $arRatingParams, $component, ["HIDE_ICONS" => "Y"]);
				?>
			</div>
		<?
		}
...
<div>
		...
		<span style="display:none" itemprop="itemReviewed" itemscope itemtype="http://schema.org/Thing">
			<meta itemprop="name" content="<?=strip_tags((string) $arResult['ELEMENT']['PRODUCT']['NAME'])?>" />
		</span>
...
<?			}
			if ($arParams["SHOW_RATING"] == "Y")
			{
				...
				$arRatingParams = array_merge($arRatingParams, $res['RATING']);
				$GLOBALS["APPLICATION"]->IncludeComponent( "bitrix:rating.vote", $arParams["RATING_TYPE"], $arRatingParams, $component, ["HIDE_ICONS" => "Y"]);
				?>
			</span><?
			}
?>
...
if (strlen((string) $arResult["NAV_STRING"]) > 0 && $arResult["NAV_RESULT"]->NavPageCount > 1):
?>
