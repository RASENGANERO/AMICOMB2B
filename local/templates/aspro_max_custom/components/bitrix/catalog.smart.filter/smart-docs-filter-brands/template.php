					...
					<div class="smartfilter">
						<div class="bx-filter-section-brands bx_filter_parameters">
							...
							foreach($arResult["ITEMS"] as $key=>$arItem)
							{
								...
								$isFilter=true;
								?>
								<div class="bx-filter-section-brands bx_filter_parameters_box <?=$class;?> <?=(isset($arItem['PROPERTY_SET']) && $arItem['PROPERTY_SET'] == 'Y' ? ' set' : '');?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code=<?=strtolower($arItem["CODE"]);?> data-property_id="<?=$arItem["ID"]?>" data-property_name="<?=$arItem["NAME"]?>">
									...
									<div class="bx-filter-section-brands bx-elements-filter-show bx_filter_block  <?=(!$arItem["IS_PROP_INLINE"] && $arItem["DISPLAY_TYPE"] != 'A' ? '' : '');?> <?=(!$arItem["IS_PROP_INLINE"] && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block scrollblock" : "");?>" <?=$style;?>>
										...
										<?foreach($docsFilterSections as $val => $section):?>
