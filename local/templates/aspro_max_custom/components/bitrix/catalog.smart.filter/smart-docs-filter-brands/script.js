...
function show_all_param(event){
  let s = document.getElementsByClassName('filter-input-custom');
  if (String(event.currentTarget.getAttribute('show-par')) == "0") {
    for(let i=0; i<s.length; i++) {
      if (s[i].classList.contains('hide-parameter')){
        s[i].classList.remove('hide-parameter');
      }
    }
    event.currentTarget.setAttribute('show-par','1');
    event.currentTarget.textContent = 'Скрыть';
  }
  else{
    for(let i=0; i<s.length; i++) {
      if (i>7) {
        s[i].classList.add('hide-parameter');
      }
    }
    event.currentTarget.setAttribute('show-par','0');
    event.currentTarget.textContent = 'Показать все';
  }
}
document.addEventListener("DOMContentLoaded", ()=>{
  if(document.getElementById('show-all-param')!=null){
    let paramBtn = document.getElementById('show-all-param');
    paramBtn.addEventListener('click',show_all_param);
  }
});