		...
		<div class="slide-block">
			<div class="slide-block__body">
				<div class="bx_filter_section bordered rounded3">
						...
						<div class="bx_filter_parameters">
							...
							foreach($arResult["ITEMS"] as $key => $arItem){
							    if(isset($arItem["ASPRO_FILTER_SORT"]) && $arItem["VALUES"]){
									...
									?>
									<div class="bx_filter_parameters_box bx_sort_filter <?=$class;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ?: "N");?>" data-prop_code="<?=strtolower((string) $arItem["CODE"]);?>" data-property_id="<?=$arItem["ID"]?>">
							...
							foreach($arResult["ITEMS"] as $key=>$arItem)
							{
								...
								if(isset($arItem["PRICE"])):
									if ($arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"] <= 0)
										continue;
									?>
									<div class="bx-filter-elements bx_filter_parameters_box active <?=(isset($arResult['PRICE_SET']) && $arResult['PRICE_SET'] == 'Y' ? ' set' : '');?>" data-property_name="<?=$arItem["NAME"]?>">
										<span data-f="<?=Loc::getMessage('CT_BCSF_SET_FILTER')?>" data-fi="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TI')?>" data-fr="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TR')?>" data-frm="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TRM')?>" class="bx_filter_container_modef"></span>
							...
							foreach($arResult["ITEMS"] as $key=>$arItem)
							{
								...
								$isFilter=true;
								?>
								<div class="bx_filter_parameters_box <?=$class;?> <?=(isset($arItem['PROPERTY_SET']) && $arItem['PROPERTY_SET'] == 'Y' ? ' set' : '');?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ?: "N");?>" data-prop_code=<?=strtolower((string) $arItem["CODE"]);?> data-property_id="<?=$arItem["ID"]?>" data-property_name="<?=$arItem["NAME"]?>">
									<span data-f="<?=Loc::getMessage('CT_BCSF_SET_FILTER')?>" data-fi="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TI')?>" data-fr="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TR')?>" data-frm="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TRM')?>" class="bx_filter_container_modef"></span>
									<?if(!$arItem["IS_PROP_INLINE"]){?>
										<div class="bx_filter_parameters_box_title colored_theme_hover_bg-block" >
											<div>
												...
												<div class="char_name">
													<div class="props_list">
														<?if($arParams["SHOW_HINTS"] !== 'N'){
															...
															<?if( $arItem["FILTER_HINT"] && !str_contains( (string) $arItem["FILTER_HINT"],'line')){?>
									...
									<div class="bx-elements-filter-show bx_filter_block  <?=(!$arItem["IS_PROP_INLINE"] && $arItem["DISPLAY_TYPE"] != 'A' ? '' : '');?> <?=(!$arItem["IS_PROP_INLINE"] && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block scrollblock" : "");?>" <?=$style;?>>
										<div class="bx_filter_parameters_box_container <?=($arItem["DISPLAY_TYPE"]=="G" ? "pict_block" : "");?>">
										<?
										...
										<?foreach($arItem["VALUES"] as $ar):?>
											...
											<div class="<?= $checkVisible < 7 ? '' : 'hide-parameter '?>filter-input-custom">
												<input
													type="checkbox"
								...
								<div class="bx_filter_block">
									<div class="bx_filter_parameters_box_container">
										<div class="bx_filter_popup_result right muted777" id="modef_mobile" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?>>
											<?echo Loc::getMessage("CT_BCSF_FILTER_COUNT", ["#ELEMENT_COUNT#" => '<span data-f="'.Loc::getMessage('CT_BCSF_SET_FILTER').'" data-fi="'.Loc::getMessage('CT_BCSF_SET_FILTER_TI').'" data-fr="'.Loc::getMessage('CT_BCSF_SET_FILTER_TR').'" data-frm="'.Loc::getMessage('CT_BCSF_SET_FILTER_TRM').'" id="modef_num_mobile">'.intval($arResult["ELEMENT_COUNT"]).'</span>']);?>
										...
										<div class="bx_filter_popup_result right muted777" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?>>
											<?echo Loc::getMessage("CT_BCSF_FILTER_COUNT", ["#ELEMENT_COUNT#" => '<span id="modef_num">'.intval($arResult["ELEMENT_COUNT"]).'</span>']);?>
											<a rel="nofollow" href="<?echo str_replace('/filter/clear/apply/', '/', $arResult["FILTER_URL"]);?>" class="btn btn-default" title="<?echo Loc::getMessage("CT_BCSF_FILTER_SHOW")?>"><?=CMax::showIconSvg("more", SITE_TEMPLATE_PATH.'/images/svg/catalog/arrow_gotochosenfilters.svg', '', '', true, false);?></a>
										</div>
										...
										<div class="filter-bnt-wrapper">
											<button class="hide-btn-filter bx_filter_search_button btn btn-default round-ignore" type="submit" id="set_filter" name="set_filter" data-href=""><?=Loc::getMessage("CT_BCSF_SET_FILTER")?></button>
										</div>
									</div>
								</div>
							</div>
					...
					<?php
					$keyCount = array_keys($arResult['ITEMS'])[0];
					$countElementsFilter = count($arResult['ITEMS'][$key]['VALUES']);
					?>
					<div class="buttons-filter-container">
					 	<?if ($countElementsFilter >= 7):?>
							<button id="show-all-param" show-par="0" class="show-all-parameters title pull-left btn-lg has-ripple">Показать все</button>
						<?endif;?>
						<button class="reset-result title pull-left btn-lg has-ripple" data-del-url="<?=$arParams["CLEAR_URL"];?>">Сбросить</button>
					</div>
				</div>
			</div>
		</div>
