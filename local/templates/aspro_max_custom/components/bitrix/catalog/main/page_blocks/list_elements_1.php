...
<?if($arSeoItem):?>
	<?ob_start();?>
		...
		<?if($arSeoItem["PROPERTY_TIZERS_VALUE"]):?>
			<?$GLOBALS["arLandingTizers"] = ["ID" => $arSeoItem["PROPERTY_TIZERS_VALUE"]];?>
...
<?$isAjax="N";?>
<?if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower((string) $_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest"  && isset($_GET["ajax_get"]) && $_GET["ajax_get"] == "Y" || (isset($_GET["ajax_basket"]) && $_GET["ajax_basket"]=="Y")){
	$isAjax="Y";
}?>
<?if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower((string) $_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest" && isset($_GET["ajax_get_filter"]) && $_GET["ajax_get_filter"] == "Y" ){
	$isAjaxFilter="Y";
}
...
<?if($itemsCnt):?>
	<?if($arParams['AJAX_MODE'] == 'Y' && str_contains((string) $_SERVER['REQUEST_URI'], 'bxajaxid')):?>
	...
	<?}?>
	<div class="right_block1 clearfix catalog1 <?=strtolower((string) $viewFilter);?>" id="right_block_ajax">
...
<?endif;?>
			...
			<?if(!$arSeoItem):?>
				<?if(
					...
					!str_contains((string) $_SERVER['REQUEST_URI'], 'PAGEN')
				):?>
					<?ob_start();?>
...
<?if($itemsCnt):?>
			...
			<?}?>
				...
				?>
				<?if (($sort === 'CATALOG_PRICE_2') && ($sort_order === 'asc')) {
					$arParamsCustom['ELEMENT_SORT_FIELD'] = $arParams["ELEMENT_SORT_FIELD2"];
					$arParamsCustom['ELEMENT_SORT_ORDER'] = $arParams["ELEMENT_SORT_ORDER2"];
					$arParamsCustom['ELEMENT_SORT_FIELD2'] = $sort;
					$arParamsCustom['ELEMENT_SORT_ORDER2'] = $sort_order;
				}
				else {
					$arParamsCustom['ELEMENT_SORT_FIELD'] = $sort;
					$arParamsCustom['ELEMENT_SORT_ORDER'] = $sort_order;
					$arParamsCustom['ELEMENT_SORT_FIELD2'] = $arParams["ELEMENT_SORT_FIELD2"];
					$arParamsCustom['ELEMENT_SORT_ORDER2'] = $arParams["ELEMENT_SORT_ORDER2"];
				}
				$APPLICATION->IncludeComponent(
...
<?endif;?>
			...
			<?if($isAjax=="N"){?>
				...
				<?if($arParams["BLOG_IBLOCK_ID"]):?>
					...
					$GLOBALS[$filterName] = [
						[
							'LOGIC' => 'OR',
							[ "ID" => $linkedArticles ],
							[ "PROPERTY_LINK_GOODS_SECTIONS" => $section['ID'] ],
						],
					];

					if($sectionParent) {
						$GLOBALS[$filterName][0][] = [ "PROPERTY_LINK_GOODS_SECTIONS" => $sectionParent['ID'] ];
					}
					if($sectionRoot) {
						$GLOBALS[$filterName][0][] = [ "PROPERTY_LINK_GOODS_SECTIONS" => $sectionRoot['ID'] ];
					}

					if ($arParams["FILTER_NAME"] && $arParams["FILTER_NAME"] == "arRegionLink" && $arRegion) {
						$GLOBALS[$filterName]["PROPERTY_LINK_REGION"] = $arRegion['ID'];
					}
					$blogsCount = CMaxCache::CIblockElement_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["BLOG_IBLOCK_ID"])]], $GLOBALS[$filterName], []);
					if($blogsCount):
					?>
						<div class="linked-blog-list <?=$linkedArticlesPos?>" data-desktop_row="<?=$linkedArticlesRows?>" data-mobile_row="<?=$linkedArticlesRowsMobile?>">
							<?$APPLICATION->IncludeComponent(
								...
								[
									...
									"TITLE" => ($arParams["BLOCK_BLOG_NAME"] ?: GetMessage("TAB_BLOG_NAME")),
									"SHOW_TITLE" => "Y",
								],
								false, ["HIDE_ICONS" => "Y"]
							);?>
				...
				$bannersCount = CMaxCache::CIblockElement_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($linkedBannersIblock)]], $GLOBALS[$filterName], []);
				if($bannersCount):
				?>
...
$bBitrixAjax = (str_contains((string) $_SERVER["QUERY_STRING"], "bxajaxid"));
...
<?if($itemsCnt):?>
	...
	*/?>
	<?if($bBitrixAjax)
	{
		$page_title = $arValues['SECTION_META_TITLE'] ?: $arSection["NAME"];
		if($page_title){
...
<?else:?>
	<?if(!$section):?>
		<?\Bitrix\Iblock\Component\Tools::process404(
			trim((string) $arParams["MESSAGE_404"]) ?: GetMessage("T_NEWS_NEWS_NA")
			,true
	...
	<?else:?>
		...
		if($arParams["SET_TITLE"] !== 'N'){
			$page_h1 = $arValues['SECTION_PAGE_TITLE'] ?: $arSection["NAME"];
			if($page_h1){
