...
$section = CMaxCache::CIBlockSection_GetList(['CACHE' => ["MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], CMax::makeSectionFilterInRegion($arSectionFilter), false, ["ID", "IBLOCK_ID", "NAME", "DESCRIPTION", "PICTURE", "DETAIL_PICTURE", "UF_SECTION_DESCR", "UF_OFFERS_TYPE", 'UF_FILTER_VIEW', 'UF_LINE_ELEMENT_CNT', 'UF_TABLE_PROPS', 'UF_SECTION_BG_DARK', 'UF_LINKED_BLOG', 'UF_BLOG_BOTTOM', 'UF_BLOG_WIDE', 'UF_BLOG_MOBILE', 'UF_LINKED_BANNERS', 'UF_BANNERS_BOTTOM', 'UF_FILTER_VIEW', 'UF_BANNERS_WIDE', 'UF_BANNERS_MOBILE', $arParams["SECTION_DISPLAY_PROPERTY"], $arParams["SECTION_BG"], "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN"]);
CMax::AddMeta([
...
if ($section) {
	...
	if(strlen((string) $section["DESCRIPTION"])){
		$arSection["DESCRIPTION"] = $section["DESCRIPTION"];
	}
	if(strlen((string) $section["UF_SECTION_DESCR"])){
	...
	$iSectionsCount = CMaxCache::CIBlockSection_GetCount(['CACHE' => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], CMax::makeSectionFilterInRegion($arSubSectionFilter));
	...
	$itemsCnt = CMaxCache::CIBlockElement_GetList(["CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], CMax::makeElementFilterInRegion($arElementFilter), []);
	...
	$viewFilter = \Aspro\Functions\CAsproMax::processRetriveOptionValue('FILTER_VIEW', strtoupper((string) $viewFilter));

	if ($viewTableProps) {
        $rsViews = CUserFieldEnum::GetList([], ['ID' => $viewTableProps]);
        if ($arView = $rsViews->Fetch()) {
            $typeTableProps = strtolower((string) $arView['XML_ID']);
        }
    }
}
...
<div class="main-catalog-wrapper flexbox<?= $arTheme["SIDE_MENU"]["VALUE"] === 'LEFT' ? ' flexbox--row-reverse' : ' flexbox--row' ; ?> flexbox--justify-space-between flexbox--align-start<?= CMax::GetFrontParametrValue("LEFT_BLOCK_CS_TYPE") === 'NORMAL' ? ' flexbox--gap flexbox--gap-32' : '' ; ?>">
	<div class="section-content-wrapper <?=($bShowLeftBlock ? 'with-leftblock' : '');?>">
		...
		<?$bContolAjax = (isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower((string) $_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest" && isset($_GET["control_ajax"]) && $_GET["control_ajax"] == "Y" );?>
		<?// section elements?>
