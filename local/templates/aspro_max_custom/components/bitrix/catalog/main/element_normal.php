...
?>
<div class="main-catalog-wrapper details js_wrapper_items flexbox<?= $arTheme["SIDE_MENU"]["VALUE"] === 'LEFT' ? ' flexbox--row-reverse' : ' flexbox--row' ;
?> flexbox--justify-space-between flexbox--align-start flexbox--gap flexbox--gap-32 <?=$bShowCollapsedBlock ? ' show-collapsed' : ''
?><?=$bShowTableOneColumn ? ' show-one-column' : ''?>">
...
if(!$typeTmpDetail || !$typeTmpPictureDetail){
			if($arSection["DEPTH_LEVEL"] > 2){
				$sectionParent = CMaxCache::CIBlockSection_GetList(['CACHE' => ["MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], ['GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]], false, ["ID", "IBLOCK_ID", "NAME", "UF_ELEMENT_DETAIL", "UF_PICTURE_RATIO"]);
				...
				if(!$typeTmpDetail || !$typeTmpPictureDetail){
					$sectionRoot = CMaxCache::CIBlockSection_GetList(['CACHE' => ["MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], ['GLOBAL_ACTIVE' => 'Y', "<=LEFT_BORDER" => $arSection["LEFT_MARGIN"], ">=RIGHT_BORDER" => $arSection["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1, "IBLOCK_ID" => $arParams["IBLOCK_ID"]], false, ["ID", "IBLOCK_ID", "NAME", "UF_ELEMENT_DETAIL", "UF_PICTURE_RATIO"]);
...
if($typeTmpPictureDetail){
			$rsTypes = CUserFieldEnum::GetList([], ["ID" => $typeTmpPictureDetail]);
			if($arType = $rsTypes->GetNext()){
...
?>
		...
		<?php 
$isWideBlock = ($arParams["DIR_PARAMS"]["HIDE_LEFT_BLOCK"] ?? "");
?>
		<?php 
...
$arParams["BIGDATA_EXT_BOTTOM"] = $arParams["BIGDATA_EXT_BOTTOM"] ?: "bigdata_bottom_1";
$arParams["BIGDATA_EXT"] = $arParams["BIGDATA_EXT"] ?: "bigdata_1";
...
?>
		<?php 
$arParams["DETAIL_EXPANDABLES_TITLE"] = ($arParams["DETAIL_EXPANDABLES_TITLE"] ?: GetMessage("EXPANDABLES_TITLE"));
$arParams["DETAIL_ASSOCIATED_TITLE"] = ($arParams["DETAIL_ASSOCIATED_TITLE"] ?: GetMessage("ASSOCIATED_TITLE"));
$arTab = $arAllValues = $arSimilar = $arAssociated = $arNeedSelect = [];
...
?>
		<script type="text/javascript">
			...
			BX.addCustomEvent(window, "onAjaxSuccess", function(e){
				var arAjaxPageData = 
<?=CUtil::PhpToJSObject($arAdditionalData, true, true, true);
?>;
