...
if(array_key_exists("display", $_REQUEST) || (array_key_exists("display", $_SESSION)) || $arParams["DEFAULT_LIST_TEMPLATE"]){
	if($_REQUEST["display"] && (in_array(trim((string) $_REQUEST["display"]), $arDisplays))){
		$display = htmlspecialcharsbx(trim((string) $_REQUEST["display"]));
		$_SESSION["display"]=htmlspecialcharsbx(trim((string) $_REQUEST["display"]));
	}
	elseif($_SESSION["display"] && (in_array(trim((string) $_SESSION["display"]), $arDisplays))){
...
<div class="filter-panel sort_header view_<?=$display?> <?=($bShowCompactHideLeft && $bHideLeftBlock ? 'show-compact' : '' );?> <?=(!$bShowSortInFilter ? 'show-normal-sort' : '' );?>">
	<?if('Y' === $arParams['USE_FILTER']):?>
		...
		<div class="filter-panel__filter pull-left filter-<?=strtolower((string) $arTheme['FILTER_VIEW']['VALUE']);?>  <?=($bHideLeftBlock && !$bShowCompactHideLeft ? 'filter-panel__filter--visible' : '');?>">
	...
	<!--noindex-->
		<div class="filter-panel__sort pull-left hidden-xs">
			...
			if(in_array("PRICE", $arSorts)){
				...
				if($arSortPrices == "MINIMUM_PRICE" || $arSortPrices == "MAXIMUM_PRICE"){
					$arAvailableSort["PRICE"] = ["PROPERTY_".$arSortPrices, "asc"];
				}
				else{
			...
			$defaulSortButtons = ["SORT","POPULARITY", "NAME", "PRICE", "QUANTITY", "CUSTOM"];
			$propsInSort = [];
			$propsInSortName = [];
			foreach($arSorts as $sort_prop){
				if(!in_array($sort_prop, $defaulSortButtons)){
					$arAvailableSort['PROPERTY_'.$sort_prop] = ['PROPERTY_'.$sort_prop, "desc"];
					$propsInSort[] = $sort_prop;
				}
			}
			foreach($propsInSort as $propSortCode){
					$dbRes = CIBlockProperty::GetList([], ['ACTIVE' => 'Y', 'IBLOCK_ID' => $arParams['IBLOCK_ID'], 'CODE' => $propSortCode]);
			...
			if(in_array("CUSTOM", $arSorts) && !array_key_exists($sortElementField, $arAvailableSort) ){
				$arAvailableSort[$sortElementField] = ["CUSTOM", ToLower($arParams["ELEMENT_SORT_ORDER"])];
			}			
			...
			if((array_key_exists("sort", $_REQUEST) && array_key_exists(ToUpper($_REQUEST["sort"]), $arAvailableSort)) || (array_key_exists("sort", $_SESSION) && array_key_exists(ToUpper($_SESSION["sort"]), $arAvailableSort)) || $arParams["ELEMENT_SORT_FIELD"]){
				...
				elseif($_SESSION["sort"]){
					...
					$sort = (str_starts_with($sort, 'SCALED_PRICE_')) ? 'PRICE' : $sort;					
				}
			}
						...
						<span>
							<?if($sort_order && $sort):?>								
								<?if( in_array($sort, array_keys($propsInSortName)) ):?>
									<?=\Bitrix\Main\Localization\Loc::getMessage('SORT_TITLE_PROPETY', ['#CODE#' => $propsInSortName[$sort]]).\Bitrix\Main\Localization\Loc::getMessage('SECT_ORDER_'.$sort_order)?>
								<?else:?>
									...
									<div class="dropdown-select__list-item font_xs">
										...
										<?endif;?>
											<?if( in_array($key, array_keys($propsInSortName)) ):?>
												<span><?=\Bitrix\Main\Localization\Loc::getMessage('SORT_TITLE_PROPETY', ['#CODE#' => $propsInSortName[$key]]).\Bitrix\Main\Localization\Loc::getMessage('SECT_ORDER_'.$value)?></span>
											<?else:?>
			...
			if($sort == "CATALOG_AVAILABLE"){
				$sort = CMax::checkVersionModule('20.100.0', 'main') ? "CATALOG_AVAILABLE" : "";
			}
			?>
		</div>
		...
		<?if($display == 'block'):?>
			<div class="filter-panel__view controls-linecount pull-right">
				...
				{
					if($_REQUEST["linerow"] && (in_array(trim((string) $_REQUEST["linerow"]), $arLineCount)))
					{
						$linerow = htmlspecialcharsbx(trim((string) $_REQUEST["linerow"]));
						$_SESSION["linerow"]=htmlspecialcharsbx(trim((string) $_REQUEST["linerow"]));
					}
					elseif($_SESSION["linerow"] && (in_array(trim((string) $_SESSION["linerow"]), $arLineCount)))
					{
						$linerow = $_SESSION["linerow"];
					}
					elseif($arParams["LINE_ELEMENT_COUNT"] && (in_array(trim((string) $arParams["LINE_ELEMENT_COUNT"]), $arLineCount)))
					{
