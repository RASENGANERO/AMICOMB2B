			...
			<form method="post">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="userContact" id="contactInput" value=
						"<?=!empty($_GET['contact']) ? htmlspecialcharsbx(urldecode((string) $_GET['contact'])): ''?>">
				</div>
				<div class="form-group">
...
{
	$successNotify = strpos((string) $_GET['result'], 'Ok') ? true : false;
	...
	$arJSParams = [
		...
		'NOTIFY_MESSAGE' => urldecode((string) $_GET['message']),
	];
	?>
	<script type="text/javascript">
...
<!--		</div>-->
	...
	if (!empty($arItem['OFFERS']) && isset($skuTemplate[$arItem['ID']]))
	{
		...
		<div class="bx_catalog_item_scu" id="<?= $arItemIDs['PROP_DIV']; ?>"><?
			foreach ($skuTemplate[$arItem['ID']] as $propTemplate)
			{
				...
				unset($valueCount);
				echo '<div>', str_replace(['#ITEM#_prop_', '#WIDTH#'], [$arItemIDs['PROP'], $fullWidth], $rowTemplate['START']);
				foreach ($propTemplate['ITEMS'] as $value => $valueItem)
				{
					echo str_replace(['#ITEM#_prop_', '#WIDTH#'], [$arItemIDs['PROP'], $itemWidth], $valueItem);
				}
