...
if (!empty($arResult['ITEMS']))
{
	...
	//
	$skuPropList = []; // array("id_catalog" => array(...))
	$skuPropIds = []; // array("id_catalog" => array(...))
	$skuPropKeys = []; // array("id_catalog" => array(...))
	...
	foreach($arResult['CATALOGS'] as $catalog)
	{
		...
		if($offersCatalogId > 0 && $offersPropId > 0)
			$sku = ["IBLOCK_ID" => $offersCatalogId, "SKU_PROPERTY_ID" => $offersPropId, "PRODUCT_IBLOCK_ID" => $catalogId];
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		{
			$arSKUPropIDs = $skuPropIds[$arItem['IBLOCK_ID']] ?? [];
			$arSKUPropList = $skuPropList[$arItem['IBLOCK_ID']] ?? [];
			$arSKUPropKeys = $skuPropKeys[$arItem['IBLOCK_ID']] ?? [];
			...
			foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arOneRow = [
					...
					'AVAILIABLE' => str_replace('store_view', 'store_view_of', CMax::GetQuantityArray($totalCount, [], "Y")),
