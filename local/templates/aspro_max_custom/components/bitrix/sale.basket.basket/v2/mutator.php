...
$servicesIblockId = Option::get("aspro.max", "SERVICES_IBLOCK_ID", CMaxCache::$arIBlocks[SITE_ID]["aspro_max_content"]["aspro_max_services"][0]);
$catalogIblockId = Option::get("aspro.max", "CATALOG_IBLOCK_ID", CMaxCache::$arIBlocks[SITE_ID]["aspro_max_catalog"]["aspro_max_catalog"][0]);
$bCache = Option::get("aspro.max", "SERVICES_CACHE", 'N') === 'Y';
$cacheTime = Option::get("aspro.max", "SERVICES_CACHE_TIME", '36000');
$showOldPrice = Option::get("aspro.max", "SERVICES_SHOW_OLD_PRICE", 'Y');
$countInAnnounce = Option::get("aspro.max", "SERVICES_COUNT_IN_ANNOUNCE", '2');
$priceType = explode(",", Option::get("aspro.max", "SERVICES_PRICE_TYPE", 'BASE'));
$cacheGroups = Option::get("aspro.max", "SERVICES_CACHE_GROUPS", 'N');
$convertCurrency = Option::get("aspro.max", "SERVICES_CURRENCY", 'N');
$priceVat = Option::get("aspro.max", "SERVICES_PRICE_VAT_INCLUDE", 'Y');
$bConvertCurrency = $convertCurrency === 'N';
$bUseFastView =  CMax::GetFrontParametrValue('USE_FAST_VIEW_PAGE_DETAIL') !== 'NO';
$bServicesRegionality = Option::get("aspro.max", "SERVICES_REGIONALITY", 'N') === 'Y' 
						&& Option::get("aspro.max", "USE_REGIONALITY", 'N') === 'Y'
						&& Option::get("aspro.max", "REGIONALITY_FILTER_ITEM", 'N') === 'Y';
...
foreach ($this->basketItems as $row)
{
	...
	if($row['DELAY'] !== 'Y'){		
		...
		$row["LINK_SERVICES"] = [];
		if(count($link_services_in_basket) > 0){	
		...
		$arElement = CMaxCache::CIBLockElement_GetList(['CACHE' => ["MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($catalogIblockId)]], $arElementFilter, false, false, ["ID", "IBLOCK_ID", "PROPERTY_SERVICES"]);
		...
		$linkServices = CAsproMax::getLinkedItems($itemForServ, "SERVICES", $arParamsForServ);
	...
	$typeStickers = Option::get('aspro.max', 'ITEM_STICKER_CLASS_SOURCE', 'PROPERTY_VALUE', $rowData['LID']);
	$parentItemIDs = [
		'ID' => $row['PRODUCT_ID'],
		...
		while ($arProp = $rsProps->Fetch()) {
			if ($arProp['VALUE_ENUM']) {
				$arProps[] = [
					...
					'CODE' => strtolower((string) $arProp['VALUE_XML_ID']),
					'CLASS' => 'sticker_'.($typeStickers === 'PROPERTY_VALUE' ? CUtil::translit($arProp['VALUE_ENUM'], 'ru') : strtolower((string) $arProp['VALUE_XML_ID']))
				];
			}elseif($arProp['VALUE']){
				$arProps[] = [
					'VALUE' => $arProp['VALUE'],
					'CODE' => strtolower((string) $arProp['CODE']),
					'CLASS' => 'sticker_'.strtolower((string) $arProp['CODE']),
				];
			}
		}
	}
	...
	if ($row['NOT_AVAILABLE'])
	{
		foreach ($rowData['SKU_BLOCK_LIST'] as $blockKey => $skuBlock)
		{
			if (!empty($skuBlock['SKU_VALUES_LIST']))
			{
				if ($notSelectable)
				{
					foreach ($skuBlock['SKU_VALUES_LIST'] as $skuValue)
					{
