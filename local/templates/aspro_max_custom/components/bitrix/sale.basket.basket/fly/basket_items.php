...
if ($normalCount > 0):
	...
	?>
	<?foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
		if (in_array($arHeader["id"], ["TYPE", "DISCOUNT"])) {continue;} // some header columns are shown differently
					...
					if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y"):
						...
						<div class="item <?=($isServices ? 'hidden' : '')?>" data-id="<?=$arItem["ID"]?>" product-id="<?=$arItem["PRODUCT_ID"]?>" data-iblockid="<?=$arItem["IBLOCK_ID"]?>" <?if($arItem["QUANTITY"]>$arItem["AVAILABLE_QUANTITY"]):?>data-error="no_amounth"<?endif;?> <?if($isServices):?>data-parent_product_id="<?=$idParentProduct?>"<?endif;?>>
							<div class="wrap clearfix">
								<div class="image">
									<?if( strlen((string) $arItem["PREVIEW_PICTURE"]["SRC"])>0 ){?>
										<?if (strlen((string) $arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb"><?endif;?>
										...
										<?if (strlen((string) $arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
									<?}elseif( strlen((string) $arItem["DETAIL_PICTURE"]["SRC"])>0 ){?>
										<?if (strlen((string) $arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb"><?endif;?>
								...
								<div class="body-info">
									<div class="description">
										<div class="name">
											<?if(strlen((string) $arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?endif;?><?=$arItem["NAME"]?><?if (strlen((string) $arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
										</div>
											...
											<div class="props">
												<?foreach ($arItem["PROPS"] as $val) {
														if (is_array($arItem["SKU_DATA"])) {
															$bSkip = false;
															foreach ($arItem["SKU_DATA"] as $arProp) { if ($arProp["CODE"] == $val["CODE"]) { $bSkip = true; break; } }
															if ($bSkip) continue;
										...
										<?if (is_array($arItem["SKU_DATA"]) && $arItem["PROPS"]):?>
											<div class="props">
												<?foreach ($arItem["SKU_DATA"] as $propId => $arProp):
													...
													foreach ($arProp["VALUES"] as $arVal) { if (isset($arVal["PICT"]) && !empty($arVal["PICT"])) { $isImgProperty = true; break; } }
										...
										<div class="prices <?=( $bTypeColumn ? 'notes' : '' );?>">
											<div class="cost prices clearfix">
												...
												<?}else{?>
													...
													<input type="hidden" name="item_price_<?=$arItem["ID"]?>" value="<?=$arItem["PRICE"]?>" />
												<?}?>
												<?if (strlen((string) $arItem["NOTES"]) > 0 && $bTypeColumn):?>
													...
													if ($arItem['CHECK_MAX_QUANTITY'] == 'Y') {
														...
														if ($arRegion) {
															...
															if ($arStores) {
																...
																$arFilter = ['ID' => $arStores];

																$rsStore = CCatalogStore::GetList([], array_merge($arFilter, ['PRODUCT_ID' => $arItem['PRODUCT_ID']]), false, false, $arSelect);
									...
									<div class="services_in_basket buy_services_wrap" data-parent_product="<?=$arItem["PRODUCT_ID"]?>">
										...
										<?if($linkServices):?>
											...
											if($bServicesRegionality && isset($arRegion['ID'])){
												$GLOBALS['arBuyServicesFilterBasket'][] = [ "PROPERTY_LINK_REGION" => $arRegion['ID'] ];
											}										
											?>									
											<?$APPLICATION->IncludeComponent(
												...
												[
													...
													"COMPACT_MODE" => 'Y',
													"SHOW_ALL_IN_SLIDE" => 'Y',
													"SERVICES_IN_BASKET" => is_array($arItem["LINK_SERVICES"]) ? $arItem["LINK_SERVICES"] : [],
													"PLACE_ID" => 'fly_basket',
													"COMPATIBLE_MODE" => "Y",
												],
												false, ["HIDE_ICONS"=>"Y"]
...
<?else:?>
	<div class="cart-empty">
		...
		<div class="cart-empty__info">
			<?$APPLICATION->IncludeFile(SITE_DIR."include/empty_fly_cart.php", [], ["MODE"      => "html", "NAME"      => GetMessage("SALE_BASKET_EMPTY"),]);?>
		</div>
	</div>
<?endif;?>
