...
$title_delay = ($delayCount ? GetMessage("BASKET_DELAY_COUNT", ["#PRICE#" => $arResult["DELAY_PRICE"]["SUMM_FORMATED"]]) : GetMessage("EMPTY_BLOCK_DELAY"));

$arCounters = CMax::updateBasketCounters(['READY' => ['COUNT' => $normalCount, 'TITLE' => $title_basket, 'HREF' => $arParams["PATH_TO_BASKET"]], 'DELAY' => ['COUNT' => $delayCount, 'TITLE' => $title_delay, 'HREF' => $arParams["PATH_TO_BASKET"].'#delayed'], 'COMPARE' => ['COUNT' => $compareCount, 'HREF' => $arParams["PATH_TO_COMPARE"]], 'PERSONAL' => ['HREF' => $arParams["PATH_TO_AUTH"]], 'FAVORITE' => ['COUNT' => \Aspro\Max\Itemaction\Favorite::getCount()]]);

$arParams['INNER'] ??= false;
?>
<?if($arParams['INNER'] !== true && $_SERVER['REQUEST_METHOD'] !== 'POST'):?>
	<div class="basket_fly loaded<?if (strlen((string) $arResult["ERROR_MESSAGE"]) > 0):?> basket_empty<?endif;?>">
<?endif;?>
	<div class="wrap_cont">
	...
	<?
		...
		if ($delayCount) { $arMenu[] = ["ID"=>"DelDelCanBuy", "TITLE"=>GetMessage("SALE_BASKET_ITEMS_DELAYED"), "COUNT"=>$delayCount, "FILE"=>"/basket_items_delayed.php"]; }
		...
		if ($naCount) { $arMenu[] = ["ID"=>"nAnCanBuy", "TITLE"=>GetMessage("SALE_BASKET_ITEMS_NOT_AVAILABLE"), "COUNT"=>$naCount, "FILE"=>"/basket_items_not_available.php"]; }
