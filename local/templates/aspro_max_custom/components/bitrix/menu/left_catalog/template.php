...
<?if( !empty( $arResult ) ){
	...
	$classMenuPosition = $arTheme["MENU_POSITION"]["VALUE"] == 'COMBINED' ? 'm_line' : 'm_' . strtolower((string) $arTheme["MENU_POSITION"]["VALUE"]);
	...
	<div class="menu_top_block catalog_block <?=$bShowOverlay ? 'dark_overlay' : ''?>">
		<ul class="menu dropdown">
			<?foreach( $arResult as $arItem ){?>
				<li class="full <?=($arItem["CHILD"] ? "has-child" : "");?> <?=($arItem["SELECTED"] ? "current opened" : "");?> <?=$classMenuPosition?> v_<?=strtolower((string) $arTheme["MENU_TYPE_VIEW"]["VALUE"]);?>">
					<a class="icons_fa <?=($arItem["CHILD"] ? "parent" : "");?>" href="<?=$arItem["SECTION_PAGE_URL"]?>" >
						<?if($arItem["CHILD"]):?>
							...
							<?if(strtolower((string) $arTheme["MENU_TYPE_VIEW"]["VALUE"]) == "bottom" ):?>
						...
						<?endif;?>
						<?
						$arItem['IMAGES'] = ($arItem['PARAMS']['SECTION_ICON'] ?? ($arItem['PARAMS']['PICTURE'] ?: $arItem['IMAGES']));
						...
						<?if($arItem["IMAGES"] && $arTheme["LEFT_BLOCK_CATALOG_ICONS"]["VALUE"] == "Y"){?>
							<span class="image">
								<?if(str_contains((string) $arItem["IMAGES"]["src"], ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
					...
					<?if($arItem["CHILD"]){?>
						<div class="dropdown-block <?=strtolower((string) $arTheme["MENU_TYPE_VIEW"]["VALUE"]) == 'bottom' ? 'dropdown' : ''?>">
							<div class="dropdown">
								<ul class="left-menu-wrapper">
									
									<?foreach($arItem["CHILD"] as $arChildItem){?>
										<?
										$arChildItem['IMAGES'] = ($arChildItem['PARAMS']['SECTION_ICON'] ?? $arChildItem['PARAMS']['PICTURE'] ?? $arChildItem['IMAGES']);
											...
											<?if($arChildItem["IMAGES"] && $arTheme['SHOW_CATALOG_SECTIONS_ICONS']['VALUE'] == 'Y' && $arTheme["MENU_TYPE_VIEW"]["VALUE"] !== 'BOTTOM'){?>
												<span class="image colored_theme_svg">
													<a href="<?=$arChildItem["SECTION_PAGE_URL"];?>">
														<?if(str_contains((string) $arChildItem["IMAGES"]["src"], ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
															<?=\Aspro\Functions\CAsproMax::showSVG([
