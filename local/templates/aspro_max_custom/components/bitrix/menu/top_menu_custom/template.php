...
<?if($arResult):?>
	<?if (!function_exists('showSubItemss')) {
		function showSubItemss($arParams = [
			...
			$arParams['ITEM']["CHILD"] ??= [];
			?>
			<?if($arParams['HAS_PICTURE']):
				$arSubItemImg = ( $arParams['ITEM']['PARAMS']['SECTION_ICON'] ?? $arParams['ITEM']['PARAMS']['PICTURE'] );
				$arImg = CFile::ResizeImageGet($arSubItemImg, ['width' => 60, 'height' => 60], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);

				if(is_array($arImg)):?>
					<div class="menu_img <?=$arParams['HAS_ICON'] ? 'icon' : ''?>">
						<a href="<?=$arParams['ITEM']["LINK"]?>" class="noborder img_link colored_theme_svg">
							<?if(str_contains((string) $arImg["src"], ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
		...
		<table>
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					$arItem["CHILD"] ??= [];
					$arItem["BANNERS"] ??= false;
					$arItem["TEXT"] ??= false;

					if(!$arItem['TEXT']) continue;
					
					$bWideMenu = (isset($arItem['PARAMS']['CLASS']) && str_contains($arItem['PARAMS']['CLASS'], 'wide_menu'));
					$arItem['bManyItemsMenu'] = $bManyItemsMenu;
					if(!$bWideMenu) {
						$arItem['bManyItemsMenu'] = false;
					}?>

					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($arItem["PARAMS"]["CLASS"] ?? "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							<?
							if (str_replace('/','',$arItem['LINK']) !== 'TZ') {
								if (!in_array($arItem["LINK"],$linksHide)) {
								?>
								<a class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" href="<?=$arItem["LINK"]?>">
									<div>
										...
										<?endif;?>
										<?=$arItem["TEXT"]?>
										<?if(isset($arItem["PARAMS"]["CLASS"]) && str_contains($arItem["PARAMS"]["CLASS"], "catalog")):?>
							...
							?>
							<a class="hide-url <?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>">
								<div>
									<?if(isset($arItem["PARAMS"]["ICON"]) && $arItem["PARAMS"]["ICON"]):?>
										<?=CMax::showIconSvg($arItem["PARAMS"]["ICON"], SITE_TEMPLATE_PATH.'/images/svg/'.$arItem["PARAMS"]["ICON"].'.svg', '', '');?>
									<?endif;?>
							...
							?>	
								<a class="btn-tz btn btn-default btn-lg animate-load has-ripple" data-event="jqm" data-param-form_id="TZ_FORM" data-name="send_tz"><?=$arItem["TEXT"]?></a>
							<?
							}
							?>
																...
																<a href="<?=$arChild['LINK']?>" class="menu-navigation__sections-item-link font_xs <?=($arChild["SELECTED"] ? "colored_theme_text" : "dark_link")?> <?=($bShowImg ? " menu-navigation__sections-item-link--image" : "");?><?=($arChild['CHILD'] ? " menu-navigation__sections-item-dropdown" : "");?>">
																	...
																	<!--<?if($bShowImg){?>
																		<span class="image colored_theme_svg ">
																			<?$imageID = ($arChild['PARAMS']['SECTION_ICON'] ?? $arChild['PARAMS']['PICTURE']);?>
																			<?$image = CFile::GetPath($imageID);?>
																			<?if(str_contains($image, ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
																				<?=\Aspro\Functions\CAsproMax::showSVG([
																					'PATH' => $image
																				]);?>
																			<?else:?>
												...
												<div class="right-content">
													<?if($arItem["BANNERS"]):?>
														...
														if($GLOBALS['arRegionLink']) {
															$GLOBALS['rightBannersFilter'] = array_merge( $GLOBALS['arRegionLink'], ['ID' => $arItem["BANNERS"]] );
														} else {
															$GLOBALS['rightBannersFilter'] = ['ID' => $arItem["BANNERS"]];
														}
										...
										<ul class="menu-wrapper <?='menu-type-'.$MENU_TYPE?>" >
											<?foreach($arItem["CHILD"] as $arSubItem):?>
												...
												<?if($MENU_TYPE == 2 && $bWideMenu):?>
													...
													?>
													<li class="<?=($arSubItem["SELECTED"] ? "active" : "")?> <?=$bIcon ? 'icon' : ''?>">
														<?if($bHasPicture && $bWideMenu):
															$arSubItemImg = ( $arSubItem['PARAMS']['SECTION_ICON'] ?? $arSubItem['PARAMS']['PICTURE'] );
															$arImg = CFile::ResizeImageGet($arSubItemImg, ['width' => 80, 'height' => 80], BX_RESIZE_IMAGE_PROPORTIONAL);
															if(is_array($arImg)):?>
												...
												<?else: // type 1?>
													...
													$bIcon = (isset($arSubItem['PARAMS']['SECTION_ICON'])) && $arSubItem['PARAMS']['SECTION_ICON'];
													?>
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> parent-items">
														<?if($arItem['bManyItemsMenu']):?>
																		...
																		<div class="right-content">
																			<?if($bRightBanners):?>
																				...
																				if(isset($GLOBALS['arRegionLink']) && $GLOBALS['arRegionLink']) {
																					$GLOBALS['rightBannersFilter'] = array_merge( $GLOBALS['arRegionLink'], ['ID' => $arSubItem["PARAMS"]["BANNERS"]] );
																				} else {
																					$GLOBALS['rightBannersFilter'] = ['ID' => $arSubItem["PARAMS"]["BANNERS"]];
																				}
																				$APPLICATION->IncludeComponent(
																					"bitrix:news.list",
																					"banners",
																...
																<div class="subitems-wrapper">
																	<ul class="menu-wrapper" >
																		<?foreach($arSubItem["CHILD"] as $arSubItem2):?>
																			...
																			?>
																			<li class="<?=($arSubItem2["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem2["SELECTED"] ? "active" : "")?>">
																				<?=showSubItemss([
																					'HAS_PICTURE' => false,
																					'HAS_ICON' => $bIcon,
																					'WIDE_MENU' => true,
