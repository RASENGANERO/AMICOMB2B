...
if($arResult){
	...
	if($bRightBanner) {
		...
		$arBannerFilter = ['!PROPERTY_SHOW_MENU' => false, 'ACTIVE' => 'Y', 'IBLOCK_ID' => $bannerIblockId];
		$arBannerSelect = ['ID', 'PROPERTY_SHOW_MENU'];
		$arBanners = CMaxCache::CIblockElement_GetList(["SORT" => "ASC", "CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($bannerIblockId)]], $arBannerFilter, $arBannerSelect);
	}

	if($bRightBrand) {
		$brandIblockId = CMaxCache::$arIBlocks[SITE_ID]["aspro_max_content"]["aspro_max_brands"][0];
		$arBrandFilter = ['PROPERTY_SHOW_TOP_MENU_VALUE' => 'Y', 'ACTIVE' => 'Y', 'IBLOCK_ID' => $brandIblockId];
		$arBrandSelect = ['ID', 'PROPERTY_SHOW_TOP_MENU', 'PREVIEW_PICTURE', 'NAME', 'DETAIL_PAGE_URL', 'IBLOCK_ID'];
		$arResult['BRANDS'] = CMaxCache::CIblockElement_GetList(["SORT" => "ASC", "CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($brandIblockId)]], $arBrandFilter, false, false, $arBrandSelect);
	}
			...
			foreach($arItem['CHILD'] as $key2=>$arItemChild)
			{
				if($bManyItemsMenu) {
					if($bRightBrand && $arItemChild['PARAMS']['BRANDS']) {
						$brandIblockId = CMaxCache::$arIBlocks[SITE_ID]["aspro_max_content"]["aspro_max_brands"][0];
						$arBrandFilter = ['ID' => $arItemChild['PARAMS']['BRANDS'], 'ACTIVE' => 'Y', 'IBLOCK_ID' => $brandIblockId];
						$arBrandSelect = ['ID', 'PREVIEW_PICTURE', 'NAME', 'DETAIL_PAGE_URL', 'IBLOCK_ID'];
						$arResult[$key]['CHILD'][$key2]['BRANDS'] = CMaxCache::CIblockElement_GetList(["SORT" => "ASC", "NAME" => "ASC", "CACHE" => ["TAG" => CMaxCache::GetIBlockCacheTag($brandIblockId)]], $arBrandFilter, false, false, $arBrandSelect);
					}
				}
			}
		}
