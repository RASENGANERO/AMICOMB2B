	...
	<div class="menu top top-mobile-menu">
		<ul class="top">
			<?foreach($arResult as $arItem):?>
				...
				$bShowImages = $bShowImagesInMobile && isset($arItem['PARAMS']['CLASS']) && (!str_contains($arItem['PARAMS']['CLASS'], 'no_icons_in_mobile')) && (str_contains($arItem['PARAMS']['CLASS'], 'catalog') || str_contains($arItem['PARAMS']['CLASS'], 'wide_menu'));
				?>
				...
				<li <?=($arItem['SELECTED'] ? ' class="selected"' : '')?>>
					<?if (str_replace('/','',$arItem['LINK']) === 'TZ'):?>
						<a class="dark-color animate-load has-ripple" data-event="jqm" data-param-form_id="TZ_FORM" data-name="send_tz"><?=$arItem["TEXT"]?></a>
					<?else:?>
					...
					<?if($bParent):?>
						<ul class="dropdown">
							<li class="menu_back">
								<a href="" class="dark-color" rel="nofollow"><?=CMax::showIconSvg('back_arrow', SITE_TEMPLATE_PATH.'/images/svg/return_mm.svg')?><?=GetMessage('MAX_T_MENU_BACK')?></a>
							</li>
							<li class="menu_title">
								...
								<li <?=($arSubItem['SELECTED'] ? ' class="selected"' : '')?>>
									<a class="dark-color<?=($bParent ? ' parent' : '')?> top-mobile-menu__link" href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>">
										<?if($bShowImages):?>
											...
											$arImg = CFile::ResizeImageGet($arSubItemImg, ['width' => 50, 'height' => 50], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
											?>
											<?if(is_array($arImg)):?>
											...
											<?foreach($arSubItem["CHILD"] as $arSubSubItem):?>
												...
												<li<?=($arSubSubItem['SELECTED'] ? ' class="selected"' : '')?>>
													<a class="dark-color<?=($bParent ? ' parent' : '')?> top-mobile-menu__link" href="<?=$arSubSubItem["LINK"]?>" title="<?=$arSubSubItem["TEXT"]?>">
														<?if($bShowImages):?>
															<?
															$arSubSubItemImg = ( $arSubSubItem['PARAMS']['SECTION_ICON'] ?? $arSubSubItem['PARAMS']['PICTURE'] );
															$arImg = CFile::ResizeImageGet($arSubSubItemImg, ['width' => 50, 'height' => 50], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
															?>
															<?if(is_array($arImg)):?>
																<span class="image top-mobile-menu__image colored_theme_svg">
																	<?if(str_contains((string) $arImg["src"], ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
															...
															<?foreach($arSubSubItem["CHILD"] as $arSubSubSubItem):?>
																<li<?=($arSubSubSubItem['SELECTED'] ? ' class="selected"' : '')?>>
																	<a class="dark-color top-mobile-menu__link" href="<?=$arSubSubSubItem["LINK"]?>" title="<?=$arSubSubSubItem["TEXT"]?>">
																		<?if($bShowImages):?>
																			<?
																			$arSubSubSubItemImg = ( $arSubSubSubItem['PARAMS']['SECTION_ICON'] ?? $arSubSubSubItem['PARAMS']['PICTURE'] );
																			$arImg = CFile::ResizeImageGet($arSubSubSubItemImg, ['width' => 50, 'height' => 50], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
																			?>
																			<?if(is_array($arImg)):?>
																				<span class="image top-mobile-menu__image colored_theme_svg">
																					<?if(str_contains((string) $arImg["src"], ".svg") && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
