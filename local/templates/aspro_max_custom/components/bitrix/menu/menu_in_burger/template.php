	...
	<div class="burger_menu_wrapper">
		...
		<?if($arResult['EXPANDED']):?>
			...
			<div class="top_link_wrapper">
				...
				<div class="menu-item <?=($arResult['EXPANDED']["CHILD"] ? "dropdown" : "")?> <?=($arResult['EXPANDED']["PARAMS"]["CLASS"] ?? "");?>  <?=($arResult['EXPANDED']["SELECTED"] ? "active" : "")?>">
		...
		<div class="bottom_links_wrapper row">
			<?foreach($arResult as $arItem):?>					
				<?$bShowChilds = $arParams["MAX_LEVEL"] > 1;?>
				<div class="menu-item col-md-4 unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($arItem["PARAMS"]["CLASS"] ?? "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
					<div class="wrap">
						<?if (str_replace('/','',$arItem['LINK']) === 'TZ'):?>
							<a class="dark-color animate-load has-ripple" data-event="jqm" data-param-form_id="TZ_FORM" data-name="send_tz">
								<div class="link-title color-theme-hover"><?=$arItem["TEXT"]?></div>
							</a>
						<?else:?>
									...
									<?foreach($arItem["CHILD"] as $arSubItem):?>
										...
										<div class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?>">
											<?if($bHasPicture && $bWideMenu):
												$arImg = CFile::ResizeImageGet($arSubItem['PARAMS']['PICTURE'], ['width' => 60, 'height' => 60], BX_RESIZE_IMAGE_PROPORTIONAL_ALT);
												if(is_array($arImg)):?>
													<div class="menu_img">
