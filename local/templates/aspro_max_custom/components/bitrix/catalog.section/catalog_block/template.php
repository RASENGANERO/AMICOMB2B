		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<?ob_start();?>
				...
				<div class="item-title">
					<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="<?=$bBigBlock && $bFonImg ? '' : 'dark_link'?> js-notice-block__title option-font-bold font_sm">
						<span><?=$arItem['NAME']//$elementName;?></span>
					</a>
				</div>
				<div class="item-title">
					<?if (empty($arItem["PROPERTIES"]["HIDE_SECTION"]["VALUE"])):?>
							...
							<?if(!$bBigBlock):?>
								...
								<?if($arParams['SHOW_GALLERY'] == 'Y' && $arParams['SHOW_PROPS'] != 'Y'):?>
									<?if($bUseSkuProps && $arItem["OFFERS"]):?>
										<?\Aspro\Functions\CAsproMaxItem::showSectionGallery( ['ITEM' => $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]], 'RESIZE' => $arResult['CUSTOM_RESIZE_OPTIONS']] );?>
									<?else:?>
