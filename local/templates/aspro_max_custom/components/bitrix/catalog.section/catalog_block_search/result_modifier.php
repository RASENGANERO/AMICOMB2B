...
if (!empty($arResult['ITEMS'])){
	...
	if ($arResult['MODULES']['catalog'])
	{
		...
		$bUseModuleProps = Option::get("iblock", "property_features_enabled", "Y") === "Y";

		if ($bUseModuleProps) {
			$arParams['OFFERS_CART_PROPERTIES'] = (array)PropertyCatalogFeature::getBasketPropertyCodes($arSKU['IBLOCK_ID'], ['CODE' => 'Y']);
		}
		...
		if ( $bUseModuleProps && $boolSKU && $featureProps = PropertyCatalogFeature::getOfferTreePropertyCodes( $arSKU["IBLOCK_ID"], ['CODE' => 'Y'] ) ) {
			$arParams['OFFER_TREE_PROPS'] = $featureProps;
		}
		if ( $boolSKU && $featureProps = PropertyFeature::getListPageShowPropertyCodes( $arSKU["IBLOCK_ID"], ['CODE' => 'Y'] ) ) {
		...
		if ($boolSKU && !empty($arParams['OFFER_TREE_PROPS']))
		{
			...
			if ($arFilterSKU && $arFilterSKU['OFFERS_ID']) {
				foreach ($arResult['ITEMS'] as $key => $arItem) {
					if ($arItem['OFFERS']) {
						$arResult['ITEMS'][$key]['OFFERS'] = array_filter($arItem['OFFERS'], fn($arValue) => in_array($arValue['ID'], $arFilterSKU['OFFERS_ID']));
					}
				}
			}
					...
					foreach ($arItem['OFFERS'] as &$arOffer) {
						foreach ($arOffer['DISPLAY_PROPERTIES'] as &$arProperty) {
							if (isset($arOffer['DISPLAY_PROPERTIES'][$arProperty['CODE']])) {
								...
								$valueID = $arProperty['PROPERTY_TYPE'] === PropertyTable::TYPE_LIST
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			...
			foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				if($arParams["USE_MAIN_ELEMENT_SECTION"] != "Y")
				{
					...
					{
						$arTmpUrl = explode("?", (string) $arOffer["DETAIL_PAGE_URL"]);
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					//{
						...
						if ($foundOffer && $intSelected == -1)
						{
							...
							$arItem['MIN_PRICE'] = ($arOffer['RATIO_PRICE'] ?? $arOffer['MIN_PRICE']);
					...
					if(!Property::checkFilterOfferByStore($arOffer, $arParams)){
		...
		if($arParams["SET_SKU_TITLE"] === "Y"){
			if(isset($arItem["OFFERS_SELECTED"])){
				$ipropValues = new ElementValues($arItem['OFFERS'][$arItem["OFFERS_SELECTED"]]['IBLOCK_ID'], $arItem['OFFERS'][$arItem["OFFERS_SELECTED"]]['ID']);
		...
		if($arParams['IBINHERIT_TEMPLATES']){
			IBInherited::modifyItemTemplates($arParams, $arItem);
		}
...
$arResult['ITEMS'] = $arResult['ORIGINAL_PARAMETERS']['GLOBAL_FILTER']['=ID'];
?>
