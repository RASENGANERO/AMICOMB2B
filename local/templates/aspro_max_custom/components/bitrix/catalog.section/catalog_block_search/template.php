...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			$col = match ($arParams["LINE_ELEMENT_COUNT"]) {
                '4' => $bBigBlock ? 6 : 3,
                '3' => 4,
                default => $bBigBlock ? 40 : 20,
            };
							...
							if( $arParams['REVIEWS_VIEW'] ):?>
								<div class="blog-info__rating--top-info">
									<div class="votes_block nstar with-text">
										<div class="rating-cust ratings">
											<?
											$valRate = CIBlockElement::GetProperty($arItem['IBLOCK_ID'], $arItem['ID'], 'sort','asc', ['CODE'=>'EXTENDED_REVIEWS_RAITING'])->Fetch();
											
											?>
											<?$message = $valRate['VALUE'] ? GetMessage('VOTES_RESULT', ['#VALUE#' => $valRate['VALUE']]) : GetMessage('VOTES_RESULT_NONE')?>
											<div class="inner_rating" title="<?=$message?>">
												<?if (empty($valRate['VALUE'])):?>
													<?for($i=0;$i<5;$i++):?>
												...
												<?else:?>
													<?for($i=1;$i<=5;$i++):?>
														<div class="item-rating <?=$i<=$valRate['VALUE'] ? 'filed' : ''?>">
															<?=CMax::showIconSvg("star", SITE_TEMPLATE_PATH."/images/svg/catalog/star_small.svg");?>
														</div>
													<?endfor;?>
												<?endif;?>
			...
			<?ob_start();?>
				...
				<div class="item-title">
					<a href="/catalog/<?=$arItem["CODE"]?>/" class="<?=$bBigBlock && $bFonImg ? '' : 'dark_link'?> js-notice-block__title option-font-bold font_sm">
						<span><?=$arItem['NAME']//$elementName;?></span>
					</a>
				</div>
				<div class="item-title">
						...
						<?else:?>
							...
							<?elseif(isset($arResult["PRICES"])):?>
								...
								if (intval($productPrice)<=0) {
									$productPrice = \CMax::GetFrontParametrValue('MISSING_GOODS_PRICE_DISPLAY_TEXT_VALUE');
								}
								else {
									$productPrice = number_format($productPrice, 0, '', ' ') . ' ₽/шт';
								}

								$quantityProduct = \Bitrix\Catalog\ProductTable::getList([
									'select' => ['QUANTITY'],
									'filter' => ['=ID'=>$arItem['ID']],
								])->fetch()['QUANTITY'];
							
								if (intval($quantityProduct)>0) {
									$quantityProduct = 'quan-green';
								}
								else{
									$quantityProduct = 'icon';
								}
								?>
								<div class="sa_block">
									<div class="item-stock">
										<span class="<?=$quantityProduct?> order"></span>
										<span class="value font_sxs"></span>
									</div>														
								</div>
								<div class="price_matrix_wrapper">
									<div class="prices-wrapper">
										<div class="price font-bold font_mxs"><?=$productPrice?></div>
									</div>
								</div>
						...
						<div class="image_wrapper_block js-notice-block__image <?=($arParams['SHOW_PROPS'] == 'Y' && $arItem['DISPLAY_PROPERTIES'] && !$bBigBlock ? ' with-props' : '');?>">
								...
								<?if(!$bFastViewFull):?><div class="like_icons block ce_cmp_hidden"><?endif;?>
									<?if(!$bComplect):?>
										<div class="fast_view_button <?=$bFastViewFull ? 'fast_view_button--full fast_view_button--center' : ''?>">
											<span title="<?=$fast_view_text?>" class="rounded3 <?=$bFastViewFull ? 'font_upper_xs' : 'colored_theme_hover_bg'?> " data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="<?=$arParams["IBLOCK_ID"];?>" data-param-id="<?=$arItem["ID"];?>" data-param-item_href="<?=urlencode((string) $arItem["DETAIL_PAGE_URL"]);?>" data-name="fast_view"><?=\CMax::showIconSvg($bFastViewFull ? "fw " : "fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/quickview".$typeSvg.".svg");?><?=$bFastViewFull ? $fast_view_text : ''?></span>
										</div>
									<?endif;?>
							...
							<?else:?>
								<?if($bFastViewFull && $arParams['USE_FAST_VIEW'] != 'N' && !$bComplect):?>
									<div class="fast_view_button fast_view_button--full">
										<span title="<?=$fast_view_text?>" class="rounded2 font_upper_xs" data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="<?=$arParams["IBLOCK_ID"];?>" data-param-id="<?=$arItem["ID"];?>" data-param-item_href="<?=urlencode((string) $arItem["DETAIL_PAGE_URL"]);?>" data-name="fast_view"><?=\CMax::showIconSvg("fw ", SITE_TEMPLATE_PATH."/images/svg/quickview".$typeSvg.".svg");?><?=$fast_view_text?></span>
									</div>
								<?endif;?>
							...
							<?if(!$bBigBlock):?>
								...
								<?if($arParams['SHOW_GALLERY'] == 'Y' && $arParams['SHOW_PROPS'] != 'Y'):?>
									<?
									$image = $arItem['PREIVIEW_PICTURE'] ?: $arItem['DETAIL_PICTURE'];	
									$image = \CFile::GetPath($image);
									if (empty($image)) {
										$image = SITE_TEMPLATE_PATH.'/images/svg/noimage_product.svg';
									}
									?>
									<a href="<?="/catalog/".$arItem["CODE"]."/"?>" class="thumb"><img class="img-responsive " src="<?=$image?>" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" /></a>
								<?else:?>
