	...
	foreach($arCategory["ITEMS"] as $i => $arItem)
	{
		if(isset($arItem["ITEM_ID"]))
		{
			...
			if (
				...
				&& !str_starts_with($arItem["ITEM_ID"], "S")
			)
			{
		...
		case "iblock":
			if (
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					...
					elseif ($arElement['OFFERS'] && Option::get('aspro.max', 'SHOW_FIRST_SKU_PICTURE', 'N') === 'Y') {
						$bFindPicture = false;
						...
						foreach ($arElement['OFFERS'] as $arOffer) {
							...
							if ($arOffer['DETAIL_PICTURE'] && !$arElement['PREVIEW_PICTURE']['ID'] && !$bFindPicture) {
								$arElement['PICTURE'] = CFile::ResizeImageGet($arOffer["DETAIL_PICTURE"], ["width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT], BX_RESIZE_IMAGE_PROPORTIONAL, true);
								$bFindPicture = true;
								break;
							}
						}
