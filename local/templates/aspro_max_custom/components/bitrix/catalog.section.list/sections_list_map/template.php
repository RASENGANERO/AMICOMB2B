...
<?if($arResult["SECTIONS"]){?>
	...
	<div class="catalog_section_list row items<?=($bSlick ? ' margin0' : '');?> flexbox type_<?=$arParams['TEMPLATE_TYPE']?>">
		...
		foreach( $arResult["SECTIONS"] as $arItems ){
			...
			$this->AddDeleteAction($arItems['ID'], $arItems['DELETE_LINK'], CIBlock::GetArrayByID($arItems["IBLOCK_ID"], "SECTION_DELETE"), ["CONFIRM" => GetMessage('CT_BNL_SECTION_DELETE_CONFIRM')]);
			$bSlide = false;
		?>
							...
							<td class="section_info toggle">
									...
									<li class="name<?=($bBigBlock ? ' text-center' : '');?>">
										...
										<?if($arItems["ELEMENT_CNT"]):?>
											<?if($bBigBlock || $bSlideBlock):?>
												<span class="element-count2 muted font_xs"><?=\Aspro\Functions\CAsproMax::declOfNum($arItems["ELEMENT_CNT"], [Loc::getMessage('COUNT_ELEMENTS_TITLE'), Loc::getMessage('COUNT_ELEMENTS_TITLE_2'), Loc::getMessage('COUNT_ELEMENTS_TITLE_3')])?></span>
											<?else:?>
									...
									<?if($arItems["SECTIONS"]):?>
										...
										<?foreach($arItems["SECTIONS"] as $arItem):?>
											<li class="sect<?=(++$i > $iVisibleItemsMenu ? ' collapsed' : '');?><?=($bSlideBlock ? ' col-sm-6 font_sm' : ' font_xs');?>">
												<a href="<?=$arItem["SECTION_PAGE_URL"]?>" class="<?=($bSmallBlock ? 'muted777' : '')?>"><?=$arItem["NAME"]?><?=($arItem["ELEMENT_CNT"]?'&nbsp;<span class="'.($bSmallBlock ? 'dark-color' : '').'">'.$arItem["ELEMENT_CNT"].'</span>':'');?></a>
											</li>
											<?
											$checkSections = \Amikomnew\SubsectionsTree::checkCountSection($arItem['ID']);
											?>
											<?if ($checkSections !== false):?>
												<?
												$arSectionChilds = \Amikomnew\SubsectionsTree::getSections($arItem['ID']);
												?>
												<ul class="childs-elements">
													<?
														foreach($arSectionChilds as $arItemChild):
													?>
														<li class="childs-map-list sect<?=(++$i > $iVisibleItemsMenu ? ' collapsed' : '');?><?=($bSlideBlock ? ' col-sm-6 font_sm' : ' font_xs');?>">
															<a href="<?=$arItemChild['URL']?>" class="<?=($bSmallBlock ? 'muted777' : '')?>"><?=$arItemChild["NAME"]?><?=($arItemChild['CNT']?'&nbsp;<span class="'.($bSmallBlock ? 'dark-color' : '').'">'.$arItemChild['CNT'].'</span>':'');?></a>
														</li>
													<?endforeach;?>
												</ul>
											<?endif;?>
								...
								<?if($arParams["SECTIONS_LIST_PREVIEW_DESCRIPTION"]!="N"):?>
									<?if($arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]):
										$arSection = $section = CMaxCache::CIBlockSection_GetList(['CACHE' => ["MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])]], ['GLOBAL_ACTIVE' => 'Y', "ID" => $arItems["ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]], false, ["ID", $arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]]);?>
