...
if($arItems){
	...
	<?if(count($arAllSections) > 1):?>
		<?
		$arSections = CMaxCache::CIBlockSection_GetList(['NAME' => 'ASC', 'CACHE' => ["MULTI" => "N", "GROUP" => ["ID"], "TAG" => CMaxCache::GetIBlockCacheTag($catalogIBlockID)]], ["ID" => $arSectionsID, "IBLOCK_ID" => $catalogIBlockID], false, ["ID", "IBLOCK_ID", "NAME"]);
			...
			<div class="slide-block">
				...
				<div class="slide-block__body">
					<ul class="menu dropdown">
						...
						<?if($cntMore > 0):?>
							<div class="item"><span class="item_title colored more_items with_dropdown"><?=GetMessage('MORE_SECTIONS')?> <?=Aspro\Functions\CAsproMax::declOfNum($cntMore, [GetMessage('MORE_SECTIONS0'), GetMessage('MORE_SECTIONS1'), GetMessage('MORE_SECTIONS2')])?></span></div>
						<?endif;?>
					</ul>
				</div>
			</div>
