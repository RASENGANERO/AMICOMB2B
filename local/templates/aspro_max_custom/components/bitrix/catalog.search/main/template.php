...
<?$isAjax="N";?>
<?if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower((string) $_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest"  && isset($_GET["ajax_get"]) && $_GET["ajax_get"] == "Y" || (isset($_GET["ajax_basket"]) && $_GET["ajax_basket"]=="Y") || isset($_GET["control_ajax"])){
	$isAjax="Y";
}?>
<?if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower((string) $_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest" && isset($_GET["ajax_get_filter"]) && $_GET["ajax_get_filter"] == "Y"  && !isset($_GET["control_ajax"])){
	$isAjaxFilter="Y";
}?>
<?
use Amikomnew;
...
<?if($arParams["FILTER_NAME"] == '' || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", (string) $arParams["FILTER_NAME"])){
...
if(!strlen((string) $searchQuery)){
	$searchQuery = $_GET['q'];
}

$elemsArticle = Amikomnew\BrainSearch::getElementsArticle($searchQuery);
$elemsBrand = Amikomnew\BrainSearch::getElementsIsBrand($searchQuery);
	...
	if(($landingID = intval($_REQUEST['ls'])) > 0){
		...
		if(!strlen((string) $searchQuery)){
			...
			if($arElement = $dbRes->Fetch()){
				...
				if(strlen($query = $arQuery ? trim(htmlspecialchars_decode((string) $arQuery[0])) : '')){
...
if($arLanding){
	...
	if(strlen((string) $arLanding['PROPERTY_URL_CONDITION_VALUE'])){
		$urlCondition = ltrim(trim((string) $arLanding['PROPERTY_URL_CONDITION_VALUE']), '/');
	...
	if(strlen((string) $arLanding['PROPERTY_REDIRECT_URL_VALUE']) && !strlen((string) $urlCondition)){
		if(!isset($_REQUEST['ls'])){
...
<div class="top-content-block <?=$APPLICATION->ShowViewContent('top_class');?>">
	...
	<?if($arLanding && ($arLanding["DETAIL_PICTURE"] || strlen((string) $arLanding["PREVIEW_TEXT"]) || $arLanding["PROPERTY_FORM_QUESTION_VALUE"]) || $arLanding["PROPERTY_TIZERS_VALUE"]):?>
		...
		<?if(strlen((string) $arLanding["PREVIEW_TEXT"])):?>
			<div class="seo_block">
		...
		<div class="seo_block">
					...
					<tbody>
						<tr>
							...
							<td class="col-md-3 col-sm-4 col-xs-5 valign btns-col">
								<div class="btns">
									<span><span class="btn btn-default btn-sm animate-load" data-event="jqm" data-param-form_id="ASK" data-name="question"><span><?=(strlen((string) $arParams['S_ASK_QUESTION']) ? $arParams['S_ASK_QUESTION'] : GetMessage('S_ASK_QUESTION'))?></span></span></span>
								</div>
							</td>
						</tr>
					</tbody>
			...
			<?if($arLanding["PROPERTY_TIZERS_VALUE"]):?>
				<?$GLOBALS["arLandingTizers"] = ["ID" => $arLanding["PROPERTY_TIZERS_VALUE"]];?>
				<?$APPLICATION->IncludeComponent(
					"bitrix:news.list",
					"front_tizers",
					[
	...
	<?if($arLanding &&strlen((string) $arLanding['PROPERTY_H3_GOODS_VALUE'])):?>
...
<div class="main-catalog-wrapper flexbox<?= CMax::GetFrontParametrValue("LEFT_BLOCK_CS_TYPE") === 'NORMAL' ? ' flexbox--gap flexbox--gap-32' : '' ; ?><?= $arTheme["SIDE_MENU"]["VALUE"] === 'LEFT' ? ' flexbox--row-reverse' : ' flexbox--row' ; ?> flexbox--align-start">
	<div class="section-content-wrapper <?=($arElements && !$bHideLeftBlock ? 'with-leftblock' : '');?> <?=($bShowFilter ? 'with-filter' : '');?> js-load-wrapper<?=($arTheme["LAZYLOAD_BLOCK_CATALOG"]["VALUE"] === "Y" ? ' with-load-block' : '')?>">
		...
		$arElements = array_merge($arElements,$elemsArticle,$elemsBrand);
		...
		if (!empty($arElements))
		{
			...
			if($arSKU)
			{
				...
				{
					$arTmp = CIBlockElement::GetProperty($arSKU['IBLOCK_ID'], $value, ["sort" => "asc"], ["ID"=>$arSKU['SKU_PROPERTY_ID']])->Fetch();
					if($arTmp['VALUE'])
			...
			if($arLanding && $arCustomFilter){
				if($bReplaceElementsByCustomFilter){
					$GLOBALS[$arParams["FILTER_NAME"]] = [$arCustomFilter] + $arrFilter;
				}
				else{
				...
				<div class="inner_wrapper">
					<div class="ajax_load cur <?=$display?>" data-code="<?=$display?>">
						<?$arConfigTransfer = [
							...
							'CURRENT_BASE_PAGE' => $arLanding && strlen((string) $arLanding['PROPERTY_URL_CONDITION_VALUE']) ? $canonicalUrl : null,
							"IBINHERIT_TEMPLATES" => $arLanding ? $arIBInheritTemplates : [],
						];
		...
		<?}else{
			...
			if($arParams["USE_BIG_DATA_IN_SEARCH"] == "Y"){
				$APPLICATION->IncludeComponent("bitrix:catalog.bigdata.products", "main", [
					...
					"PAGE_ELEMENT_COUNT" => ($arParams['RECOMEND_IN_SEARCH_COUNT'] ?: 10),
					...
					"RCM_TYPE" => ($arParams['BIG_DATA_IN_SEARCH_RCM_TYPE'] ?? ''),
					"DISPLAY_WISH_BUTTONS" => $arParams["DISPLAY_WISH_BUTTONS"],
		...
		<?if($arLanding):?>
			<div class="group_description_block bottom muted777">
				<?if(strlen((string) $arLanding["DETAIL_TEXT"])):?>
				...
				if($arLanding["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"])
					$APPLICATION->SetPageProperty("title", strip_tags(htmlspecialchars_decode((string) $arLanding["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"])));
				else
				...
				if($arLanding["IPROPERTY_VALUES"]["ELEMENT_META_DESCRIPTION"])
					$APPLICATION->SetPageProperty("description", strip_tags(htmlspecialchars_decode((string) $arLanding["IPROPERTY_VALUES"]["ELEMENT_META_DESCRIPTION"])));
