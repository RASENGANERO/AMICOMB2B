...
if(array_key_exists("display", $_REQUEST) || (array_key_exists("display", $_SESSION)) || $arParams["DEFAULT_LIST_TEMPLATE"]){
	if($_REQUEST["display"] && (in_array(trim((string) $_REQUEST["display"]), $arDisplays))){
		$display = trim((string) $_REQUEST["display"]);
		$_SESSION["display"]=trim((string) $_REQUEST["display"]);
	}
						...
						<span>
							<?if($sort_order && $sort):?>
								<?if( in_array($sort, array_keys($propsInSortName)) ):?>
									<?=\Bitrix\Main\Localization\Loc::getMessage('SORT_TITLE_PROPETY', ['#CODE#' => $propsInSortName[$sort]]).$bSort = $sort !== "RANK" ?\Bitrix\Main\Localization\Loc::getMessage('SECT_ORDER_'.$sort_order) : "";?>
								<?else:?>
		...
		<?if($display == 'block'):?>
			<div class="filter-panel__view controls-linecount pull-right">
				...
				<?if(array_key_exists("linerow", $_REQUEST) || (array_key_exists("linerow", $_SESSION)) || $arParams["LINE_ELEMENT_COUNT"])
				{
					...
					{
						$linerow = trim((string) $_REQUEST["linerow"]);
						$_SESSION["linerow"]=trim((string) $_REQUEST["linerow"]);
					}
