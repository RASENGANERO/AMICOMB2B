	...
	<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $indexItem < 2)):?>
					<div class="<?=$arItem['ELEMENT_CLASS_B2B']?> item-wrapper-b2b item-wrapper col-md-<?=$col;?> col-sm-6 col-xs-6 col-xxs-12 clearfix <?=$arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'];?> <?=$dop_class;?><?=($bSlider ? ' item-width-261' : '');?>" data-ref="mixitup-target">
				<?endif;?>
					...
					<?if($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $indexItem == 1):?>
						...
						<?$bHalfWrapper = true;?>
					<?endif;?>
					<div class="brand-container-item item <?=$bHalfWrapper ? '' : 'bg-white'?> <?=($bBordered ? ' bordered box-shadow rounded3' : '');?><?=(!$bImage ? ' no-img' : '');?><?=(($arResult['HAS_TITLE_FON'] == 'Y' || $useImageAlternate) ? ' long' : '');?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		...
		<div class="bottom_nav_wrapper<?=($bSlider ? ' hidden-slider-nav' : '');?>">
			<div class="bottom_nav animate-load-state<?=($arResult['NAV_STRING'] ? ' has-nav' : '');?>" <?=($arParams['IS_AJAX'] ? "style='display: none; '" : "");?> data-parent=".item-views" data-scroll-class=".swipeignore.mobile-overflow" data-append="<?=($arParams['HALF_BLOCK'] != 'Y' ? '.items > .row' : '.items > .row > .item-wrapper.line_img .half-wrapper .mCSB_container');?>">
				<?if($arParams['DISPLAY_BOTTOM_PAGER']):?>
					<?//$arResult['NAV_STRING']?>
					<div class="buttons-pager-b2b">
						<?if($arParams['NEWS_COUNT'] < $arParams['COUNT_ALL_DISCOUNT']):?>
							<button id="show-all-discount-brand" class="all-brands-b2b-btn" show-brand-cnt="<?=$arParams['NEWS_COUNT']?>">Все скидки</button>
						<?endif;?>
						<a class="all-brands-b2b-url" href="/brands/">Все бренды</a>
					</div>
