...
<?if($arResult['ITEMS']):?>
	...
	<?endif;?>
			...
			?>
				...
				<div class="news-brands-container">
					<span class="main-news-brands-text">Акции и новости бренда <?=$arParams['BRAND_NAME']?></span>
					<div class="news-brands-container-items" data-brand-id="<?=$arParams['BRAND_ID']?>" data-brand-count="<?=$arParams['BRAND_ELEM_COUNT']?>">
						<?foreach($arResult['ITEMS'] as $i => $arItem):?>
							...
							<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $indexItem < 2)):?>
								<div class="news-brands-item item-wrapper col-lg-20 col-xs-6 col-xxs-12 clearfix <?=$arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'];?> <?=$dop_class;?><?=($bSlider ? ' item-width-261' : '');?>" data-ref="mixitup-target" data-id-element="<?=$arItem['ID']?>">
							<?endif;?>
